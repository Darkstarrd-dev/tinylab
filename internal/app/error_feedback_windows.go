...
func FeedbackFatalError(configDir, msg string) {
	...
	messageBox(0, msg, "TinyLab 启动失败", mbOK|mbIconError)
}

// feedbackPortConflict shows a dialog asking the user how to handle the port
// conflict. If the owner is another TinyLab instance, it offers a "Yes/No"
...
func feedbackPortConflict(configDir string, port int, owner PortOwner) (shouldKill bool) {
	if owner.IsTinyLab {
		msg := fmt.Sprintf("检测到另一个 TinyLab 实例正在运行（PID %d，路径 %s）。\n\n是否关闭它并接管端口 %d？", owner.PID, owner.Path, port)
		writeErrorLog(configDir, msg)
		result := messageBox(0, msg, "TinyLab 启动失败", mbYesNo|mbIconQuestion)
		shouldKill = (result == idYes)
		if shouldKill {
			writeErrorLog(configDir, fmt.Sprintf("用户确认关闭另一个 TinyLab (PID %d) 并接管端口 %d", owner.PID, port))
		} else {
			writeErrorLog(configDir, "用户选择不关闭另一个 TinyLab，启动终止")
		}
	} else {
		msg := fmt.Sprintf("端口 127.0.0.1:%d 被 %s (PID %d, 路径 %s) 占用。\n\n请关闭该程序或修改 config.yaml 中的 port 字段，然后重新启动 TinyLab。", port, owner.Name, owner.PID, owner.Path)
		writeErrorLog(configDir, msg)
		messageBox(0, msg, "TinyLab 启动失败", mbOK|mbIconError)
	}
	return shouldKill
