...
| 输出 | 方案（当前工作树实现） | 细节 |
|---|---|---|
| GIF | gif.js 编码（`gif-editor-export.js::renderGifBlobFromModal`） | 导出弹窗「Preview」触发 `renderGifBlobFromModal`：`ensureGifJs` 确保 vendor 就绪；`new GIF({workers: 4, quality: clamp(1..10), width, height, workerScript: GIF_WORKER_URL})`，encoder 存 `currentEncoder`（可中止）；色键透明开启且 `transparencyReady` 时设 `transparent: MATTE_NUM`（`#FF00FF` matte）；逐帧 `renderCompositedFrame(…, {applyTransparency, matte})` + `delay = max(1, round(slice.delay))`；finished → 缓存 `previewCache.gif`（key=`gifConfigKey(outW,outH,quality,trans)`）+ `setHandoff({name:'Slice_<ts>.gif', mime:'image/gif', kind:'image', format:'gif', blob})`；60s `gifRenderTimeout` fail-safe 超时 abort+恢复 |
