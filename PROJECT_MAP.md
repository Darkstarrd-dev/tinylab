...
| 新增/修改智能助理契约与小精灵交互 | PROJECT_MAP §10.25/§13l/§18.2 | `internal/assistant/`（`assistant.go`/`scheduler.go`/`reactor.go`/`llm_classifier.go`/`semantics.json`/`reactions.json`）+ `internal/api/assistant/`（`handler.go`/`events.go`/`jobs.go`/`todos.go`/`schema.go`/`sheet.go` spritesheet 服务（sheet-preview 注册/服务、sheet-image 按 action 名）/`register.go`）+ `config.AssistantConfig.Actions[]AssistantAction`（Settings Action 化：Name/SpritesheetPath/Cols/Rows/FrameStart/FrameEnd/Fps）+ `web/static/sprite.js`（L1 Dock/Modal + L2 角色漫游与漫画气泡 + 可拖拽 dock）+ `web/static/settings/settings_assistant.js`（模型选择器 stacked overlay + Action 列表/Add/Remove/Edit + canvas 网格帧选编辑弹窗）+ `web/static/sprite-pet.html`（L3 独立宠物页面，经 `/api/assistant/sheet-image/{name}` 消费 actions 动画）+ `host_webview_windows.go`（`openPetWindow` + 托盘菜单"释放…
