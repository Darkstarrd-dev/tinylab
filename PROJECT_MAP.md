...
> **最后核对（2026-08-09，打开 .html 内容为空问题解决与同名覆盖升级）：** (1) 打开 .html 文件内容为空修复：定位并解决了同名文件在 `EditorWorkspace.putFile` 中被拒并中断导致的文本未载入问题，在 `importWorkspaceFile` 中重构为智能覆盖与更新，确保选择任何外部 `.html` / `.md` 文件时能够 100% 将代码写入并载入；(2) 磁盘最新文件最高优先级：在 `loadFile` 中对关联 `externalPath` 的外部文件赋予物理文件读取最高优先级，自动刷新消除空草稿打扰，保证源码在左侧编辑区 100% 完好无损完整高亮展现。
