...
| 新增/修改智能助理契约与小精灵交互 | PROJECT_MAP §10.25/§13l/§18.2 | `internal/assistant/`（`assistant.go`/`scheduler.go`/`reactor.go`/`semantics.json`/`reactions.json`）+ `internal/api/assistant/`（`handler.go`/`events.go`/`jobs.go`/`todos.go`/`schema.go`/`register.go`）+ `web/static/sprite.js`（L1 Dock/Modal + L2 角色漫游与漫画气泡）+ `web/static/sprite-pet.html`（L3 独立透明置顶宠物页面）+ `host_webview_windows.go`（`openPetWindow` + 托盘菜单"释放桌面小精灵"）+ `web/static/style.css` + `web/static/i18n.js` |
