...
> - 新增 / 移除 `docs/` 下的事实基线文档
>
> **最后核对（2026-08-22，Gallery 渐进加载：tree 先显 + 首图即显 + 压缩包流式展开）：** `web/playground/static-pg/gallery/gallery-io.js` 加载管线改为三阶段渐进——`walkDir` 纯遍历（zip 收集为 `zipfile` 条目不再 inline 上传）、松散文件先 `appendItems`/`appendVideoItems` 即显 tree+首图、压缩包经 `processZipFilesProgressive`/`processBackendZipsProgressive`（并发 6，LRU 保护）逐包 manifest 到达即 `appendProgressiveItems` 增量上屏；`processBackendFileList` 增 `zipJobs` 参数；新增助手 `fetchZipManifest`/`pushZipPackItems`。基准 harness：`autoresearch.sh` + `autoresearch/bench.mjs`（Node VM 驱动真实 gallery-io.js），ttfi 11272ms→80ms。详见 `docs/playground-architecture.md` 顶部最后核对与 §16 变更维护清单。
