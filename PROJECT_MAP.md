...
> - 新增 / 移除 `docs/` 下的事实基线文档
>
> **最后核对（2026-08-22，Gallery 渐进加载 + 主图空白修复）：** (1) `web/playground/static-pg/gallery/gallery-io.js` 加载管线三阶段渐进——`walkDir` 纯遍历（zip 收集为 `zipfile` 条目）、松散文件先显 tree+首图、压缩包经 `processZipFilesProgressive`(blob 包并发 6)/`processBackendZipsProgressive`(`BACKEND_ZIP_CONCURRENCY`=12) 流式展开，增量上屏经 `queueProgressiveItems`(120ms 合并)；基准 ttfi 11272ms→71ms、总耗时→956ms。(2) **主图空白修复（92cd29b 回归）**：`getVideoStreamURL` grantId 直链分支补 `kind==='backend'` 守卫——zip 条目的 `.webp/.gif` 条目名命中 `isVideoExt` 后误走无 rel 的 grant 直链 → 主图空白、缩略图正常；修复后回落 blob 渲染，真实视频直链不变。(3) `web/static/style.css` `.gallery-vol-slider-vert` 弃用 `appearance:slider-vertical` 改 `writing-mode:vertical-lr;direction:rtl`。详见 `docs/playground-architecture.md` 顶部最后核对与 §16 变更维护清单。
...
> **最后核对（2026-08-19，桌面小精灵透明窗口 + spritesheet 渲染）：** (1) `host_webview_windows.go::openPetWindow`（`//go:build tray && webview && windows`）新增 Win32 透明：`WS_EX_LAYERED`（`gwlExstyle=-20`）+ `SetLayeredWindowAttributes(hwnd, petColorKey=RGB(255,0,255), 0, LWA_COLORKEY)`——宠物页 body 背景设为该 color key（`sprite-pet.html` `.pet-container` `background:#FF00FF`），被 keyed out 透明，只留 sprite/气泡（非 key 像素）。修复"释放小精灵无效果"（不透明方框）根因。新增 proc `procSetLayeredWindowAttributes` 与常量 `gwlExstyle`/`wsExLayered`/`lwaColorkey`/`petColorKey`。per-pixel alpha 抗锯齿为后续优化。(2) `web/static/sprite-pet.html` 新增 canvas spritesheet 渲染器（`#pet-sprite` canvas + `initPetSprite`：fetch `/api/settings` 取 `assistant.spritesheetPath`/`spritesheetFps`，`new Image`+`ctx.drawImage` 按帧步进，无配置则保留 CSS face fallback）——消费 item 1 的 spritesheet 配置。`go build -tags "tray webview"` 通过；bench `pet_release_wired` 83.3→100%。
...
| 新增/修改智能助理契约与小精灵交互 | PROJECT_MAP §10.25/§13l/§18.2 | `internal/assistant/`（`assistant.go`/`scheduler.go`/`reactor.go`/`llm_classifier.go`/`semantics.json`/`reactions.json`）+ `internal/api/assistant/`（`handler.go`/`events.go`/`jobs.go`/`todos.go`/`schema.go`/`register.go`）+ `web/static/sprite.js`（L1 Dock/Modal + L2 角色漫游与漫画气泡 + 可拖拽 dock）+ `web/static/sprite-pet.html`（L3 独立透明置顶宠物页面）+ `host_webview_windows.go`（`openPetWindow` + 托盘菜单"释放...
