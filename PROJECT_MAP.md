...
> **最后核对（2026-08-11，Batch Project 显式进出与手动生成计数）：** Image 侧栏 Batch Project 按钮在 Batch UI 激活期间变为可点击的 `Return`（i18n `pgBatchReturn`），点击走统一 `pgImageBatchCloseUI()`（关 SSE/reconnect 再恢复 Image 布局），保留 draft/plan/transform/active-project/后端执行；再次**显式**点击 Batch Project 由 `pg-image-batch.js` 的 `pgImageBatchRestore()`（替代已删除的 `pgImageBatchOnEnter`）按 内存执行项目 → 内存规划态 → localStorage active-project → localStorage draft 顺序恢复；`pg-lifecycle.js` `renderPlayground` 与 `pg-ui.js` `pgSetMode` 不再自动重入 Batch（绝不因持久化状态自动进入）。Batch 激活期间侧栏 Clear Chat 隐藏（普通 Image 模式保留）。Batch Project/Return 按钮右侧新增手动生成计数数字输入（默认 1）：状态 `w.config.imgSubmitCount`（`pg-core.js` `PG_DEFAULT_CFG` 默认 1），读写缝 `pgGetImageSubmitCount()`/`pgOnImageSubmitCount()`（`pg-ui.js`，clamp 1..99、pgSave 持久化、不进 API body），与 Batch Planning quantity（默认 4）完全无关；多图循环由另一工作流消费该缝。涉及 `pg-image-batch.js`/`pg-ui.js`/`pg-lifecycle.js`/`pg-core.js`/`pg-i18n.js`（`pgBatchReturn`/`pgBatchReturnTip`/`pgImageSubmitCountTip`）/`playground.css`（`.pg-batch-entry`/`.pg-image-submit-count`）。
> **最后核对（2026-08-11，GIF 编辑器最后 5 处硬编码英文标签 i18n 化）：** `web/static/gif-editor/gif-editor.js`/`gif-editor-export.js` 最后 5 处硬编码英文标签全部改走 `t()`（en/cn 双语齐全）——`#gif-open-export-btn` title「Export settings」→ 新键 `gifEditorExportSettingsTitle`（'Export Settings'/'导出设置'）；`#gif-reload-btn` title「Reset workspace」→ 既有键 `gifEditorResetWorkspace`（'Reset Workspace'/'重置工作区'）；`#gif-timeline-zoom-range` title「Zoom」→ 既有键 `gifTimelineZoom`（'Zoom:'/'倍率:'）；`#gif-modal-preview-img` alt「Export preview」→ 新键 `gifEditorExportPreviewAlt`（'Export preview'/'导出预览'）；`#gif-modal-preview-btn`「Preview」→ 新键 `gifEditorPreviewBtn`（'Preview'/'预览'，含 `data-i18n`）。`web/static/i18n.js` en+cn GIF 段新增 3 对键、复用 2 个既有键；详见 `gif_implented.md` 顶部最后核对。
> **最后核对（2026-08-11，Batch 生命周期审计修正）：** 依 `image_batch_project_redesign_plan.md` 审核结论修正 Batch 前端生命周期/隐私契约：(1) 执行项目引用持久化 `tinyrouter.playground.imageBatchActiveProject.v1`（`{schemaVersion,projectId,savedAt}`，create 后写、Close 不清除、`pgImageBatchOnEnter` snapshot-first 重入）；(2) 统一 Close 入口 `pgImageBatchCloseUI()`=cleanup+exitUI；模式切换先退出 Batch 恢复 Image 布局再切 mode/windows；(3) Stop 显式 after-current/immediate 双按钮；(4) viewer Prompt×Variant 双层导航；(5) 新建项目清理旧引用；(6) `apiPostTrace` 的 `responseRawBody` 脱敏后 256 KiB 截断。后端协议/schema 未变；详见 `docs/playground-architecture.md` §6.1/§7.2/§15、`image_batch_project_flow_review.md` §15.4。
...
| 新增/修改 GIF 编辑器页面与 Gallery 动画支持 | gif_implented.md（实施入口）、playground | `web/static/gif-editor/gif-editor-state.js`、`web/static/gif-editor/gif-editor-import.js`、`web/static/gif-editor/gif-editor-timeline.js`、`web/static/gif-editor/gif-editor-playback.js`、`web/static/gif-editor/gif-editor-export.js`、`web/static/gif-editor/gif-editor.js`（GIF 帧编辑器 SPA 页面模块与 5 个子模块，模块职责/导出函数见 §18.2；入口 `renderGifEditor(container)`/`cleanupGifEditor()`，第 6 导航按钮 `data-page="gif"`）、`web/static/vendor/gif.js/`（`gif.js`+`gif.worker.js`+LICENSE，MIT 编码器；`gif.worker.js` 为 gif.js 运行时 workerScript，非 `<script>` 标签）、`web/static/vendor/gifuct-js/`（`gifuct-js.js`+LICENSE，MIT 解码器 esbuild bundle）、`web/static/{index.html,index-nopg.html}`（第 6 按钮 + `gif.js`→`gifuct-js.js`→state→import→timeline→playback→export→`gif-editor.js` 脚本顺序）、`web/static/{app.js,auth.js,i18n.js,style.css}`（switch `case 'gif'`+cleanup、`gif`/`gifEditor*`/`gifImport*`/`gifTimeline*` 键、`gif-*` 样式含 `.gif-timeline-item` 全套与 GIF 段末尾窄视口权威规则；旧 `.gif-result-overlay` 已随修复轮移除）、`internal/feature/feature.go`（GIF feature `StaticFiles` 清单：6 模块 + 3 vendor 文件，`DependsOn: archive`）、`web/playground/static-pg/gallery/gallery-state.js`（`SUPPORTED_VIDEO_EXTS` 增 gif/webp + `ANIMATED_IMG_EXTS`/`isAnimatedImg`）、`gallery-layout.js`（video pane 增 `<img id="gallery-main-anim">`，`autoBalanceFullscreenSplitRatio` 测活动媒体）、`gallery-video.js`（`renderActiveVideo` 动画分支 + 竞态守卫 + `applyVideoPaneMode`/`replayAnim`/`stopAnim`）、`gallery-fullscreen.js`（动画短路 seek/音量、Space=重播）、`gallery-tree.js`（anim src 清理）、`internal/api/gallery/fs_handlers.go`（`galleryVidExts` 增 `.gif`/`.webp`）、`internal/gallery/gallery.go`（`SupportedExts` 增 gif）、`internal/gallery/zip.go`（`contentTypeForExt` `.gif`→`image/gif`）、`internal/api/router.go`（`/vendor/*` 主静态回退）、`web/static/media-bridge.js`（导出结果登记/「Open in Gallery」生产者，见 :736）、`internal/api/gallery/edit_handlers.go`（`galleryEditUploadTemp` 保留请求 basename、`galleryEditZipOutputs` cleanUp 全路径释放）、`docs/playground-architecture.md`。**2026-08-11 优化**：Gif Editor 导航文案修正大小写；`auth.js` 补 `mousemove` 焦点跟随修复双高亮 Bug；GIF Editor 1:1 正方形拖入容器；清理冲突与重复监听器修复 Crop/Split 面板展开；侧边栏两端全对齐；Set Latency、Batch Frame Delete、Reduce Frame 全重构为全宽展开面板；底部导航栏合并为单行（左侧缩放滑动条/Ctrl+滚轮联动 + 中间播放控制 + 右侧帧数）；滚动条高度增高 4 倍（14px）；Crop 微调输入框重构为 Left/Right/Top/Bottom 4 边边距；Export Sprite Sheet 在单图导入时隐匿，增 Start Frame 输入框（默认 0），拼图生成后先更新弹窗预览并提供「💾 保存序列图」按键确认保存或取消；恢复视频 Overlay 面板（第 7 个全宽展开按钮，集成文字/字幕与图片/水印叠加控制）；提升 Spinner 浮层 z-index 至 20000 解决导入/导出遮挡与黑屏问题，视频点击 Import 时立即关闭设置弹窗；补全 modalPreviewMime/modalPreviewFilename 变量声明修复 strict 模式 JavaScript ReferenceError；单图导入时只显示 Crop(1)、Set Transparency(2)、Split Sheet(3)、Set Latency(4) 共 4 个面板（隐匿 Overlay 等）；Split Sheet 拖拽 Scale 滑块在右侧保持舞台自适应与图片中心轴居中呈现，并实时反映分辨率精细度与像素点颗粒感改变。**2026-08-11 修复**：(1) 补全 Overlay 面板 scope radio (Current/All/Range) 与范围输入框，修复视频/GIF 导入时 Overlay 不起作用的 Bug；(2) Split Sheet 增加 Even/Uneven 切换开关，Uneven 模式下按单元格像素分辨率 + Gap 切割，超出图片边界的部分抛弃；(3) Crop 默认边距初始归 0（`pW=0, pH=0`）；(4) 显式禁用 Canvas imageSmoothingEnabled 并叠加 CSS `pixelated`，显示真实硬边缘点阵；(5) 重构 `draw()` 维护稳定原图基准并移除 `onSplitUIChange` 中高频重算视口的 `resetView()`，彻底消除 Scale 调节时的画面跳动与偏位；(6) 按方案 A 修复 Scale 缩放后 Gap 偏小 Bug：Gap/Margin 作为目标缩放画布（256×256）上的真实像素，不再乘 Scale 缩小，同时主舞台 100% 精准对齐映射。**2026-08-11（GIF 修复轮）**：导出弹窗（export modal）取代旧结果 overlay——`previewCache` 按格式/config key 隔离、`setHandoff` + `openResultInGallery` **惰性** MediaBridge 登记、`closeExportModal`/`tinyrouter:modal-close` 中止 encoder/清缓存；ZIP 帧资产（`releaseRegistered`）上传/打包失败与成功均释放、结果资产归 MediaBridge token，legacy `upload-temp` 保留 `frame_NNN.png` basename + 服务端 cleanUp 全路径释放；导入 `draftGeneration` 事务代数 + gap/margin 输出像素 + GIF 切边帧 delay 裁剪 + commit 清 source 备份；时间线 zoom 20%–300% 与舞台 zoom 解耦（Ctrl/滚轮只调时间线）；图片源 Edge Crop/Grid Slice 折叠面板、Split Sheet Uneven 像素模式 `state.splitMode` 持久化、裁剪后刷新 `processedImg`；cleanup 注册表持久化 + `bindStateEvents` 生命周期；窄视口响应式布局（见 §18.2 样式行）；MediaBridge `deliverPendingImports` 逐资产投递 + 拒绝/跳过重试；i18n 后置修复 `t(key,args,fallback)` 占位符透传 + 四确认弹窗参数化 + `web/gif-editor-i18n-args.test.js` 零依赖 Node 合同测试；i18n smoke-gap 补键（二次修复）：15 新键 + 9 CN 修正 + 5 CN 补齐（详见 gif_implented.md 顶部）+ Split Sheet/Overlay/时间线 aria 接线 + Import Modal Even/Uneven、X/Y、单元格宽高标签接线（5 处残留 GridSplitTitle/Scale/CenterGap/OuterMargin/EnableOuterGap 已接线，Import Modal i18n 全量完成）+ 测试扩展 smoke-flagged 断言（14 `check()`）。 |
