...
| `forward_retry.go` | `(h *Handler) forwardWithRetry`：重试循环 + body 改写（替换 `model` 字段、注入 `stream_options`、调 `backfillThoughtSignatures`）；**非流式 keep-alive 延迟刷新**（20s 宽限期后首字节 `\n` + 5s ticker ` ` 字节，`keepAliveDone`/`keepAliveStopped` channel 同步退出，§8.7 见 proxy-architecture.md，**H-8 bug 区，勿改语义**）；+ `broadcastRequestStart`/`broadcastTTFT`/`broadcastTokens` 三个事件广播辅助 |
