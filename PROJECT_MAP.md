...
| 修改 Image Batch Project 前端生命周期/SSE/重入 | playground | `pg-image-batch.js`（`pgImageBatchCloseUI` 统一 Close、`pgImageBatchOnEnter` snapshot-first 重入、active-project/draft localStorage key、显式 Stop、Prompt×Variant 导航）、`pg-ui.js`（`pgSetMode` 先退出 Batch 再切 mode）、`pg-lifecycle.js`、`web/static/api.js`（`responseRawBody` 脱敏）、`docs/playground-architecture.md` §6.1/§7.2/§15、`image_batch_project_flow_review.md` §15.4 |
