...
| Music 模块（融合 5 家：MusicFree/LX/Listen1/Azusa/Jamendo，见 docs/music-implementation-plan.md） | docs/music-implementation-plan.md（蓝本）、config-registry-state | `internal/config/types.go`（`MusicDir`/`ResolveMusicDir`）、`internal/api/settings/register.go`（GET `musicDir`/`configDir`，PATCH `musicDir` presence-aware）、`web/static/download.js`（Path Settings `musicDir:true` 行 Default Music Dir `{configDir}/Musics`）、`web/static/settings/settings_modal.js`（全量 sections 含 `musicDir`）、`web/static/i18n.js`（`music/musicDir/musicDirDesc` en+cn）、`web/static/index.html`+`index-nopg.html`（Gallery 下拉 `gallery-nav-wrap`/`#gallery-menu` Gallery/Music）、`web/static/app.js`（`GALLERY_TOOLS`/`galleryActiveTool`/`galleryMenuOpen`+`renderGalleryWithMenu`+下拉 wiring+ESC/hover）、`web/static/music/music.js`（解耦独立，当前占位，后续 host/player/providers） |
