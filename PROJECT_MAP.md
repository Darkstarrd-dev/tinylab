...
> - 模块职责发生迁移（文件/目录改属）
> - 新增 / 移除 `docs/` 下的事实基线文档
> **最后核对（2026-08-29，Demo 游戏插件架构落地：磁盘插件 + TRGames 宿主 + Phaser v4）：** (1) 新增 `internal/api/games/`（`GET /api/games` 列表 + `GET/PUT /api/games/{id}/state` 存档 KV + `SeedGames` 启动播种），`internal/config` 新增 `GamesDir` 字段与 `ResolveGamesDir` 三段式，`web/games.go`（无 tag）`//go:embed all:games` 内嵌默认游戏集；`internal/api/router.go` 注册 `/games/*` 磁盘静态（StripPrefix+no-store、不鉴权、位于 serveUI 通配前）+ 启动 seed（目录已存在永不覆盖）。(2) 新增 `web/static/demo-games.js` 前端宿主：`window.TRGames` 插件注册表、Phaser 懒加载、entry 脚本 mtime 缓存击穿注入、host adapter（saveState/loadState/sheetImageUrl/llmChat）、Demo 页游戏区 UI（选择/启动/停止/重载）；`assistant-demo.js` 新增 `__ademo.setPaused` 暂停缝（游戏运行时冻结测试台物理/SM）；`style.css` 末尾 `:has(.dg-root)` 双栏布局门；`app.js` demo 路由追加 `renderDemoGames`/`cleanupDemoGames`；`i18n.js` `demoGames*` en+zh。(3) vendor `web/static/vendor/phaser/`（Phaser 4.2.1 UMD + README/SHA-256 + MIT LICENSE）；seed 游戏 `web/games/survivor/`（吸血鬼幸存者式原型，v4 overlap 回调参数顺序无关写法）。改游戏代码经 Demo 页「重载」生效，不重编译不重启。架构基线文档 `docs/gamedemo-progress.md`（新增，权威）。
