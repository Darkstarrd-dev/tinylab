...
> - 模块职责发生迁移（文件/目录改属）
> - 新增 / 移除 `docs/` 下的事实基线文档
> **最后核对（2026-08-30，Game Designer：Demo 第三工具=复用 TextEditor 工厂的 games 作用域编辑 + Phaser 预览）：** `internal/api/editor/register.go` 加 `gamesDir()/baseDir(r)/resolveFileIn(root, fileId)`，`?root=games` 时 `tree/open/save/rename/delete` 均走 `ResolveGamesDir` 解析的 games 根，缺省仍走 `docDir`（零行为变更，旧单测全绿）；`delete` 对顶层游戏目录走 `PathGuard->RemoveAll`（仅当 `root=games` 且父= games 根时递归）。`web/static/demo-designer.js`（`GameDesigner.render/cleanup`，IIFE 无框架）：复用 `EditorLayout.create/renderTree/updateTitle/updateStatus/setPreview` 与 `edFileExt`、`promptModal/confirmModal`、`__dgames.{idRe,registry,makeHost,loadPhaser,injectScript}`；`GET/POST /api/editor/{tree,open,save,delete}?root=games`，新建项目写 `game.json{ id=目录名, title, version:"0.1.0", entry:"main.js"}`+`main.js` 模板（`TRGames.register({id,launch})->Phaser.Game{parent: host.container}` + `__trgame` 缝），新建文件/删除项目/文件，`Ctrl+S` 存；右侧 `Preview` 改 `div.dgn-stage`，`Run/Reload` 经 `loadPhaser->URL.createObjectURL(blob)+injectScript(blob)->registry校验->makeHost->launch` 的 Blob 热注入闭环（偏好内存中的 textarea 值，未保存也能预览），`Stop` 清 `destroy/dispose`。`web/static/demo-games.js __dgames` 加 `loadPhaser/injectScript`。`web/static/app-demo.js` `DEMO_TOOLS=[ademo,tilemap,design]`（`design:{labelKey: design}`/`demoHasTool(design)`/`demoToolLifecycle(design:suspend)`/`renderDemoWithMenu` 分支），`web/static/app-router.js` 加 `case 'design'` 与离开清理，两个 `index*.html` 在 `#demo-menu` 加 `data-demo-tool=design` 的 `Game Designer` 按钮、`demo-games.js` 后加 `demo-designer.js` 脚本，`web/static/i18n.js` 加 `design/designer*` en+cn。`web/static/style-editor.css` 加 `.dgn-root` 作用域规则（藏 `ed-format-tools/ed-toggles-group/#ed-toc/.ed-status-right/.ed-action-open` 等 markdown 控件）。工具内增删改的游戏经 `GET /api/games` 立即出现在 `Demo->Games` 下拉（磁盘即事实源，listGames 每次扫盘+mime 击穿）。
...
| 样式 | `style.css`（共享层 ~1830 行：变量/主题/布局/头部/卡片·按钮·表单/表格·徽章/modal·toast·skeleton/utilities·主题变体/style 维度，顶部 `@import` 6 个 per-page 层）+ `style-settings.css`（~605 行：Settings 布局 + QuickSlot + Theme Picker）+ `style-monitor.css`（~618 行：Stats & Models + Logs & Console + Usage/Quota Monitor + Info Modal）+ `style-download.css`（~946 行：Download 页面）+ `style-utility.css`（~2016 行：Gallery + FileTransfer + GIF Editor + TR Page）+ `style-demo.css`（~557 行：Sprite Assistant + Demo Shell）+ `style-music.css`（预留层 ~65 行，Music 专用选择器归属地 via `@import`，当前无独立覆写均复用共享 token；**2026-08-29 拆分**：原 6442 行单体按页面拆 6 文件，共享层 `@import` 聚合，无 HTML 改动，`go build` 通过；历史增量：**2026-08-10** 增 `.gallery-meta-overlay` 等 7 条规则，z-index:10——位于 `.gallery-delete-overlay`(5) 之上、`.gallery-video-hover-ctrl`(20) 之下，主题 token 复用；**2026-08-11** GIF 段末尾增权威窄视口规则——≤768px `.gif-workspace` 纵向滚动/`.gif-stage-area` `min-height:260px`/`.gif-timeline-area` `min-height:180px`/`.gif-sidebar` `max-height:38vh`，≤480px 导出弹窗 footer 换行 + sprite 按钮整行，≤380px 弹窗宽度自适应；3 处 `--accent-color`→`--accent` token 修正；**2026-08-12** `.gif-thumb-wrapper` 透明背景改主题感知棋盘格——暗色 `#262626`/`#3a3a3a`、亮色 `#f0f0f0`/`#d0d0d0`，12px 对角双线性渐变层） |
...
| 修改前端页面/资产 | PROJECT_MAP §18 | `web/static/<page>.js`、`web/static/utility/editor/*`（Editor：`editor-state.js`/`editor_workspace.js`/`editor_commands.js`/`editor_markdown.js`/`editor_layout.js`/`editor.js`/`editor_shell.js`/`editor-logs.js`/`tilemap_editor.js`（首个 Editor 类别：TileMap Editor，Tiled JSON→Phaser，`renderEditor` 包裹注入类别 Tab）；独立 Text Review wrapper 与 wizard）、`web/static/vendor/utility-editor/*`（各依赖许可证）、`web/static/index.html`、`web/static/index-nopg.html`、`internal/feature/feature.go` 的 RootStatic manifest |
...
| 新增/修改路径设置弹窗/浏览初始目录 | fsbrowse（`POST /api/browse` 后端）、config-registry-state、fsutil | `web/static/download.js`（`openPathSettingsModal` 共享弹窗 + `Image Dir`/`Log Dir`/`Games Dir`/`Music Dir` 默认路径 Placeholder 提示 + `fasBrowsePicker` 初始目录 + `browsePickerOpen` 锁 + `trapHandler` 键盘陷阱；调用方 `gamesDir:true` 新增 Default Games Dir 行，`musicDir:true` 新增 Default Music Dir 行（`{configDir}/Musics`），PATCH 走 `musicDir`/`gamesDir`；Settings 全量 sections 现含 `musicDir`）、`web/static/settings/settings.js`（Settings 侧栏 Path 行）+`settings_modal.js`（`openPathModal` 全量 sections 含 `gamesDir`+`musicDir`）、`web/static/i18n.js`（`pathSettings`/`imageDir`/`logDir`/`gamesDir`/`gamesDirDesc`/`musicDir`/`musicDirDesc`/`useProxyHint` 键）、`web/static/index-nopg.html`（补加载 `download.js`）、`web/playground/static-pg/gallery/gallery-edit.js`（齿轮按钮改调 `openPathSettingsModal`）、`web/playground/static-pg/playground/pg-stream.js`（`pgAutoSaveImageArtifact` 自动回置 `savedPath`/`savedFilename` 并刷新 `pgRefreshImageModalMeta`）、`web/playground/static-pg/playground/pg-modal.js`（`pgCopyImage` 经 `<canvas>` …
...
| Music 模块（融合 5 家：MusicFree/LX/Listen1/Azusa/Jamendo，见 docs/music-implementation-plan.md + music-architecture.md） | docs/music-implementation-plan.md（蓝本，Stage 1–5 全量）、docs/music-architecture.md（基线，§2–§3 全量）、config-registry-state | `web/static/music/host.js`（`MusicHost` 沙箱+`loadFromSource/loadFromURL` + `jamendo`/`local`）、`web/static/music/providers/bilibili.js`（Azusa `bvid→cid→playurl` 经 `GET /api/music/bilibili/search`+`POST /api/music/bilibili/resolve`）+ `example-generic.js`（Listen1/LX 热加载模板，`var plugin={id}`/`module.exports` 契约）+ `web/static/music/player.js`（`MusicPlayer<audio>` 队列/随机/循环+`playAt/next/prev/seek`）+ `web/static/music/music.js`（多源 `selectedProviders` 多选徽标 + Jamendo/Bilibili 搜索→Play/+Queue/↓落盘 + Local file 入队 + Now Playing/进度 + Queue + Library `Play`→`/api/music/file` + `→mp3`→`file→transcode→blobURL` + playlists bar `Musics/playlists.json` + `m3u` 导入导出/远端 Fetch）+ `internal/api/music/{register,transcode,playlists}.go`（`GET /api/music/library`、`POST /api/music/proxy` 透传、`POST /api/music/download` 原子落盘 `(1)` 去重 + `Base` 防穿越 + `tmp+Rename` + `GET /api/music/bilibili/search`/`POST /api/music/bilibili/resolve`→Bilibili 官方 API 映射 + `GET /api/music/file` `ServeContent`/`Accept-Ranges` + `POST /api/music/transcode?format=mp3|opus` `ffmpeg pipe:0→pipe:1` + `lookupFFmpeg` + `GET/PUT /api/music/playlists`/`POST /api/music/playlists/import`/`GET/POST /api/music/m3u`→`Musics/playlists.json`/`#EXTM3U` + `tmp+Rename`）+ `internal/api/music/{transcode,playlists}.go` + `internal/api/router.go`（`music.NewHandler` 挂 `r.Route("/music")` 于 `auth Group` 内、`securityHeaders connect-src` 放行 `https://api.jamendo.com https://api.bilibili.com`）、`internal/config/types.go`（`MusicDir`）、`internal/config/paths.go`（`ResolveMusicDir`）、`internal/api/settings/register.go`（GET `musicDir`/`configDir`，PATCH `musicDir`）、`web/static/download.js`（`musicDir:true` 行）、`web/static/settings/settings_modal.js`（全量 `musicDir:true`）、`web/static/i18n.js`（`music/musicDir/musicDirDesc`）、`web/static/index.html`+`index-nopg.html`（Gallery 下拉 `gallery-nav-wrap`/`#gallery-menu` + `demo-nav-wrap`/`#demo-menu`，music `host→player→providers→music` 顺序加载）、`web/static/app.js`（`GALLERY_TOOLS/renderGalleryWithMenu/galleryToolLifecycle/preserveGalleryState` + `DEMO_TOOLS` 同批次） |
