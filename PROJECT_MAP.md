...
| `forward_retry.go` | `(h *Handler) forwardWithRetry`：重试循环 + body 改写（替换 `model` 字段、注入 `stream_options`、调 `backfillThoughtSignatures`）；**非流式不再 keep-alive 刷新**（原 20s 宽限期首字节 `\n` + 5s ticker ` ` 字节路径已于 H-8 修复中移除——它隐式提交 HTTP 200 并破坏全 key 耗尽时的 502 状态；现非流式响应在最终响应/错误前不向 `w` 写任何字节，`writeError(w,502)` 恒生效；300s 服务器 `WriteTimeout` 上限不变）；+ `broadcastRequestStart`/`broadcastTTFT`/`broadcastTokens` 三个事件广播辅助 |
