...
> - 模块职责发生迁移（文件/目录改属）
> - 新增 / 移除 `docs/` 下的事实基线文档
> **最后核对（2026-08-29，Move 分组 + Mirror 镜像：共享 8 向 + idle / 右侧镜像自动配对）：** (1) `internal/config/types.go` 新增 `AssistantAction.Mirror bool`（`mirror,omitempty`，持久化到 actions）；(2) `web/static/settings/settings_assistant.js`：`__assistantMoveActions`/`__assistantMoveMirrorPairs` + `__assistantHasAction`/`__assistantAutoMirrorFor`（新增/编辑时对 `move_*` 左右 3 对自动补右侧 `mirror:true` 镜像，仅缺失时创建，已存在可覆盖）；`__assistantActionPresets.move` = Move 9 项（idle + 8 向），pet/platformer/topdown 抽离 move（pet=6/platformer=2/topdown=1）；`assistantActionGroupOf` Move 先判、Other 兜底 + 5 桶渲染（`t('assistantGroupMove')`）；别名表 `__assistantSMAliases` 8 个 `move_*` 共享别名 + legacy `walk_*` 双轨回退链；状态机面板 `__assistantSMGroups`：Move(9)/Pet(6)/Platformer(2)/Topdown(1)；编辑器 `assistantEditAction` 镜像行改为 `toggle-switch`、预览 `assistantDrawSheet` 按帧位移单帧翻转托管、加载走 `sheet-image` 防 `file://` 禁载；`web/static/settings/settings_modal.js` +1 按钮 `assistantPresetMove`。(3) `web/static/sprite-pet.js`/`assistant-demo.js` 的 petSM/ademoSM 新增 `move_*` 别名 + `states[].mirror` + `_mirrorPending`/`isMirrorPending` 分支（右向回退到左像时渲染镜像），ademo 的 `ademoMotionEvent` 指向 `move_*` 方向事件；(4) `internal/api/settings/register.go` 的 `GET /api/settings` 暴露 `gamesDir`/`configDir`，PATCH 的 `gamesDir` 变更触发目录创建→顶层条目移动（`Rename` 失败回退递归拷贝、已存在跳过不覆盖、空旧目录移除、空新目录 `SeedGames(web.Games)`）；`web/static/download.js` Path Settings 加 `gamesDir` 行 + payload、`i18n.js` en/cn 新增 `gamesDir/gamesDirDesc/gamesDirMoved` + Move 预设/分组键。验证：`go vet` + `node --check` ×4 + `node web/assistant-demo.test.js`（Move 8 向 + walk 兼容、motionEvent/SCroller、类型/缩放/i18n）。
