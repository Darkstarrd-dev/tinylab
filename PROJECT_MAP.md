...
| Music 模块（融合 5 家：MusicFree/LX/Listen1/Azusa/Jamendo，见 docs/music-implementation-plan.md + music-architecture.md） | docs/music-implementation-plan.md（蓝本）、docs/music-architecture.md（基线）、config-registry-state | `web/static/music/host.js`（`MusicHost` 沙箱+`jamendo`/`local` providers + `loadFromSource/search/getMediaSource`）、`web/static/music/player.js`（`MusicPlayer<audio>` 队列/随机/循环+`playAt/next/prev/seek`）、`web/static/music/music.js`（Jamendo 搜索→Play/+Queue/↓落盘 + Local file 入队 + Now Playing/进度 + Queue + Library）、`internal/api/music/register.go`（`GET /api/music/library`、`POST /api/music/proxy` 透传、`POST /api/music/download` 原子落盘 `(1)` 去重 + `Base` 防穿越 + `tmp+Rename`）、`internal/api/router.go`（`music.NewHandler` 挂 `r.Route("/music")` 于 `auth Group` 内、`securityHeaders connect-src https://api.jamendo.com`）、`internal/config/types.go`（`MusicDir`）、`internal/config/paths.go`（`ResolveMusicDir`）、`internal/api/settings/register.go`（GET `musicDir`/`configDir`，PATCH `musicDir`）、`web/static/download.js`（`musicDir:true` 行）、`web/static/settings/settings_modal.js`（全量 `musicDir:true`）、`web/static/i18n.js`（`music/musicDir/musicDirDesc`）、`web/static/index.html`+`index-nopg.html`（Gallery 下拉 `gallery-nav-wrap`/`#gallery-menu`，`host→player→music` 顺序加载）、`web/static/app.js`（`GALLERY_TOOLS/renderGalleryWithMenu/galleryToolLifecycle/preserveGalleryState`） |
