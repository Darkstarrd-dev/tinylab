...
| 文件 | 职责 |
|---|---|
| `register.go` | `Handler` + `Register` + `galleryListZip`/`galleryGetZipEntry`/`galleryDeleteZipEntry`/`galleryDeleteZipSession`（`DELETE /api/gallery/zip/{sessionId}`，整会话删除）/`galleryTouchSession`（`POST /api/gallery/zip/{sessionId}/touch`，刷新 LRU 位置）/`galleryConvertTiff`/`galleryStartReview`/`galleryReviewStatus`/`galleryCancelReview`/`galleryGeneratePrompt`/`galleryOpenDir`/`galleryListDir`/`galleryServeFile`/`galleryDeleteFs`/`galleryZipFromPath`/`galleryZipWriteback`/`galleryPastePaths`/`galleryOpenFolder`（`POST /api/gallery/open-folder {path}`：在系统文件管理器中打开路径所在目录，复用 `fsutil.OpenInFileManager`，供编辑完成后"打开目录"按钮用）/`galleryEditFfmpegStatus`/`galleryEditProbe`/`galleryEditSubtitleUpload`/`galleryEditStart`/`galleryEditStatus`/`galleryEditCancel`/`galleryEditExtractZipEntry`/`galleryEditUploadTemp`/`galleryEditZipOutputs`/`galleryEditZipWriteback`（媒体编辑 ffmpeg 端点，委托 `internal/mediaedit`；`extract-zip-entry`/`upload-temp` 支持批量转换——解压单条 zip 条目/上传 FSAA blob 到临时文件供转码；`zip-outputs` 打包多条转换结果为含原文件名的 zip（`zipName` 经 `filepath.Base`+`.zip` 强制）；`zip-writeback` 将转码后的多条临时文件以其原 zip 内路径替换回磁盘归档（`{archivePath, entries:[{zipPath, filePath}]}`），调用 `gallerylib.ReplaceZipEntries` + `fsutil.AtomicWrite` 原子回写 + best-effort 清理临时输入，实现 replace-original convert-all/单图 zip 真正原位）。`cleanZipPathNormalize` 内部 helper 复刻包内 `cleanZipPath` 做 zipPath 清洗 |
...
| `manager.go` | `Manager`（`sync.Map` 存 job）：`Start`（验证输入→构建 args→探测时长→选输出路径：覆盖→原文件、非覆盖无 OutputDir→同目录 `{base}_{desc}.{ext}` 去重、非覆盖有 OutputDir→`relocateOutput(OutputDir, outStem+ext)`，`outStem` 优先 `req.OutputName` 否则 `InputPath` stem →`generateID`→后台 `runJob`）/`Get`/`Cancel`/`ProbeMedia` |
| `args_test.go` | 参数构建器测试（格式/质量/缩放/裁剪/字幕/兼容性校验/输出路径去重） |
| `manager_test.go` | 集成测试（需 ffmpeg，否则 skip）：探针、图片转码、覆盖模式、取消、job snapshot、`OutputName` 命名（`TestManager_TranscodeImage_OutputName`/`_Dedup`——构造 `gallery-edit-upload-XXXX.png` 临时输入 + `OutputName="vacation_photo"` → `vacation_photo.webp`，二次同 stem → `_2`） |
...
| `zip_replace.go` | `ReplaceZipEntries(data, replacements map[string][]byte) ([]byte, Manifest, error)`：zip 条目替换/原位回写核心——按已清洗 zipPath 替换命中条目内容、未命中条目字节级保留（含 Method/Modified/Extra/comment、归档注释），输出新归档字节 + 新 Manifest。被 `internal/api/gallery/register.go` `galleryEditZipWriteback` 调用（replace-original convert-all/单图 zip 路径），支持 Store+Deflate（及任何 stdlib 支持的方法）。调用方负责 zipPath 清洗（handler 内 `cleanZipPathNormalize` 复刻包内 `cleanZipPath`：`\\`→`/`、trim 前导 `/`、`path.Clean`）。不依赖 `zip_delete.go` |
...
| 新增/修改 Gallery 媒体编辑 | playground | `internal/mediaedit/`（types/binary/probe/args/executor/manager + 测试）、`internal/api/gallery/register.go`（`mediaJobs` + `resolveFfmpeg` + 9 个 edit handler：ffmpeg-status/probe/subtitle-upload/start/status/cancel/**extract-zip-entry/upload-temp/zip-outputs/zip-writeback**）、`internal/api/router.go`（`pgJSFiles` 加 `gallery-edit.js`）、`web/static/index.html`（script 标签加载顺序）、`docs/playground-architecture.md`（§4.2 表 + §16 小节）。**输出命名**：`StartRequest.OutputName`（可选，无扩展名 stem）+ `manager.go` OutputDir 非覆盖分支 + **同路径分支**（`OutputName!="" && !Overwrite && OutputDir==""` → `relocateOutput(dir(InputPath), OutputName+"_"+desc+ext)`）+ `buildArgs` 的 `ext`，避免临时输入名泄漏进保存文件/zip 内条目名；**Same Path**（原 Replace Original File，label 已改 en/zh）：非覆盖同目录新建 `<原名>_converted.<ext>`（不破坏原文件），Convert all + Same Path 可开 sequential rename；**压缩包名**：`galleryEditZipOutputs` 的 `zipName`（`filepath.Base` + `.zip` 强制）；**批量转换兄弟匹配**：`gallery-edit.js` `_getSiblingImages` 按 kind 分组 + `_resolveBatchInput` 逐条解析临时磁盘路径；**批量 UX 选项**：rename/normalise 开关 → `_padNum`（自动扩位）+ `_captureBatchCfg`/`_refreshBatchUXVisibility`（renorm 行不再受 `!isOverwrite` gate 限制）；**视频 rename 对等**：共享 dest block 新增 `ge-dest-rename` 输入框（仅"另存到目录"显示），`_startJob` 读其值覆盖 `origStem` 作为 `OutputName`；**视频缩放滑块**：`ge-vid-scale` 改 `<input type="range">` + 实时 WxH 预览；**trim 跨片段约束**：`_startTrimDrag.onMove`/`_moveNearestHandle` 新增 prevEnd/nextStart 约束防组间重叠；**replace-original 守卫 + zip 原位回写**：`!canReplace` 拒绝 + `/edit/zip-writeback`（`zip_replace.go` `ReplaceZipEntries` + `fsutil.AtomicWrite`）；**打开目录**：`POST /api/gallery/open-folder` 复用 `fsutil.OpenInFileManager`，完成结果区仅保留 Open Folder（移除 Show in Gallery）；**i18n**：`pg-i18n.js` `geBatchProgress`/`geBatchDone` `%s`→`{0}/{1}` + `geRename*`/`geRenorm*`/`geBatchOpenFolder`/`geBatchOpenError`/`geNoDiskPath`/`geBatchDoneAll`/`geBatchFiles`/`geExtracting` + `geReplaceOriginal` 改 "Same Path"/"同路径"（en+zh） |
