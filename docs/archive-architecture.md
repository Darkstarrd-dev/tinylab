...
- 规则：绝对路径永不过桥；`galleryState` 只由 Gallery 代码写；token 跨页面切换/cleanup 存活直到消费/释放/TTL（10 分钟，60s 懒清扫，`unref`）；pendingImports 队列不被 `cleanupGallery` 清空（切走再切回仍投递）。**2026-08-11：** 投递为**逐资产**语义——`importedCount>0` 才消费 token，拒绝/跳过资产留队下次重试。
...
- **GIF 编辑器**（`web/static/gif-editor/gif-editor-export.js`）：三条导出路径（GIF blob / PNG Sprite blob / ZIP pack 受控 URL）统一走**导出弹窗（export modal）**，导出成功后 `setHandoff({asset})` 启用弹窗内「Open in Gallery」按钮（`gifEditorOpenGallery`）→ `openResultInGallery()` **首次点击才** `MediaBridge.register(handoff.asset)`（惰性，预览不镜像字节）完成后 `openGallery`；ZIP 的 handoff 为受控 URL（`kind:'archive'`）；`gif-editor.js` 不再创建下载 `<a>` 再 click（保存走 `triggerSaveFileWithPicker`，交接走 MediaBridge）。
...
| `web/static/gif-editor/gif-editor-export.js` | export modal `setHandoff`/`openResultInGallery` + 「Open in Gallery」惰性登记（ZIP 帧资产失败/成功均释放、结果资产归 token） |
...
| 修改 MediaBridge 契约 | `web/static/media-bridge.js` + `web/media-bridge.test.js` + `i18n.js`（mediaBridge* 键）；加载顺序：两个 index.html 最先加载；**2026-08-11** `deliverPendingImports` 逐资产投递 + 拒绝/跳过重试（消费语义：`importedCount>0` 才 `consume`） |
