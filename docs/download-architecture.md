...
>
> **最后核对（2026-08-24，/api/browse 端点搬迁）：** 通用 OS 文件/目录选择器端点 `POST /api/browse`（`browseSystemPath` + `resolveBrowseInitialDir`）已从 `internal/api/download/register.go` 迁出至新包 `internal/api/fsbrowse/`，并在 `internal/api/router.go` 的 /api 组内**无 feature 门控**挂载——assistant 与 gallery 前端也消费该端点，不应随 `feature.Enabled(feature.Download)` 门控。URL、请求/响应契约与默认 exe 过滤器行为不变；本文下方涉及 `browseSystemPath` 的历史条目均指该处理器，现归属 fsbrowse 包。Download 模块自身不再拥有任何 browse 路由。
