...
>
> **文档名说明：** 文件名 `docs/gif_implented.md` 为用户指定（原文拼写）；文档内标题使用规范拼写 "implemented"。
>
>
> **最后核对（2026-08-29，Flood Fill 选区 Erode/Erode Smooth 滑块）：** Flood Fill 面板在 Tolerance 下方新增两个滑块——**Erode**（`#gif-trans-erode`，-10..+10，默认 0 居中：正值按像素扩展本次洪水选区、负值收缩）与 **Erode Smooth**（`#gif-trans-erode-smooth`，0..20，默认 0：选区边缘羽化像素数，边缘两侧产生部分 alpha 过渡带）。实现在 `gif-editor-transparency.js` 的 `erodeSmoothMask(imgData, alphaBefore, erode, smooth)`：`applyPipeline` flood 分支在跑种子/角点洪泛前快照 alpha，`erode != 0 || erodeSmooth > 0` 时由 **alpha 差分**恢复本次选区掩码（预先透明像素永不复活），跑 1/√2 权重 chamfer 距离变换得到**半像素偏移的符号距离场**（不偏移则直线边 chamfer 取整距，羽化永远落在 0/1 两端）——硬边缘规则 `s + erode > 0`（一次遍历同时完成膨胀/腐蚀），羽化 `r = clamp(0.5 + (s+erode)/smooth)`；最终 alpha = round(alphaBefore·(1−r))，RGB 不动 → 腐蚀取消的像素从快照精确还原。`state.trans` 新增 `erode`/`erodeSmooth`（liveTransParams/committed/Cancel 恢复/reset 全链路，`transParamChanged` 把两滑块 DOM 值写回 state 以支持面板重开回显），中英 i18n 键 `gifEditorErode`/`gifEditorErodeTitle`/`gifEditorErodeSmooth`/`gifEditorErodeSmoothTitle`。Node 合同测试新增 7 组 erode/smooth 断言（31/31），headless Chrome 冒烟 14/14（舞台像素级断言：+2 吃掉红块 2px 边缘、-2 还原边缘、smooth=4 的 96/32/0/159/223/255 羽化斜坡、Apply committed、Cancel/Disable 复位）。
>
> **最后核对（2026-08-11，导出弹窗/交接契约与事务化导入修复轮）：** 本轮为 GIF 修复轮（`gif-editor-export.js`/`import.js`/`state.js`/`timeline.js`/`gif-editor.js`、`media-bridge.js`、`style.css`、`internal/api/gallery/edit_handlers.go`），核心事实如下，与 §4.5/§10.1/§10.2 一并核对：
>
> - **导出弹窗（export modal）取代旧结果 overlay（M2 已移除 `showResultModal`/`lastResultAsset`/`#gif-result-overlay`）**：GIF 与精灵图预览写入独立 `previewCache`（按渲染参数 config key 隔离，`gifConfigKey`/`spriteConfigKey`）；「Save as ZIP/GIF/Sprite」保存前按 MIME+config key 重查缓存，绝不把 PNG 存成 .gif 或复用不同参数下的预览；gif.js encoder 存入 `currentEncoder`（可中止），新增 60s fail-safe `gifRenderTimeout`（超时 abort+恢复）；`closeExportModal`/`handleModalCloseEvent` 监听 `tinylab:modal-close`，中止 encoder、revoke 预览 objectURL、清空缓存与 handoff。
>
> - **「Open in Gallery」惰性登记**：导出成功后 `setHandoff(asset)` 仅启用弹窗内 `#gif-modal-open-gallery-btn`；`openResultInGallery()` 首次点击才 `MediaBridge.register(handoff.asset)`（预览不镜像字节到服务端），完成后 `MediaBridge.openGallery(assetId)`。
> - **ZIP 帧资产释放纪律（Archive 主路线）**：`exportZipViaArchive` 用 `registered[]` 逐 id 跟踪已成功上传的帧；`releaseRegistered()` 在**上传失败、pack 失败与成功**三条路径都释放帧资产（部分拒绝批次不孤儿化），打包结果资产**不在前端释放**——归 MediaBridge token 持有至 Gallery 消费或服务端 TTL。
> - **legacy ZIP 契约**：`exportZipLegacy` 走 `upload-temp?name=frame_NNN.png`（服务端保留请求 basename，`galleryEditUploadTemp` 以请求名注册 asset，不再折叠成 upload.png）→ `zip-outputs {assetIds, zipName, cleanUp:true}`；服务端 `galleryEditZipOutputs` 的 cleanUp 在**成功与所有失败路径**都释放输入帧资产（defer 全路径）。
> - **事务化导入**：`draftGeneration` 随 cancel/open/cleanup 递增，Image/GIF/FileReader/视频 `loadedmetadata` 等异步元数据读取校验代数防陈旧写入；commit 成功后清 `source.image`/`source.rawImage`（Split Sheet 不会回滚到旧图）；GIF 范围选帧的切边帧 delay 按重叠区间裁剪（提交播放时长==endMs−startMs）；gap/margin 为输出像素不再乘 scalePercent；`cancelDraft` 同时 bump 两个 generation 并强制 `hideSpinner`。
> - **可重复清理（lifecycle）**：`cleanupModules` 的 cleanup 注册表持久化（不随 teardown 清空），state 模块经 `registerModule('state', {cleanup: cleanupStateEvents})` 在每次 teardown 移除 window 级键盘块并清 `isExtracting`/spinner——重复 enter/leave 不累积监听、不残留全局键盘拦截。
> - **时间线 zoom 20%–300%**：`setZoom` clamp 0.2–3.0（原 50%–200%），与舞台 zoom（`core.state.scale`/`updateTransform`）彻底解耦；Ctrl/Meta+滚轮在时间线滚动区只调 `timeline.zoom`，舞台缩放由 gif-editor.js 舞台 wheel 监听独占。
> - **响应式 UI**：`style.css` GIF 段**末尾**新增权威窄视口规则（≤768px：`.gif-workspace` 纵向滚动、`.gif-stage-area` `min-height:260px`、`.gif-timeline-area` `min-height:180px`、`.gif-sidebar` `max-height:38vh`；≤480px 导出弹窗 footer 换行 + sprite 保存按钮整行；≤380px 弹窗宽度自适应），修复原「移动端时间线偏紧」遗留项；另 3 处 `--accent-color`→`--accent` token 修正。
> - **还原的图片工具与状态持久化**：图片源新增「Edge Crop & Grid Slice」折叠面板（`#gif-image-tools-toggle-btn`/`#gif-image-tools-panel`，`updateSourcePanels` 用 `display:block/none` 取代 `gif-hidden` 类）；Split Sheet Even/Uneven 模式在 `pageTemplate` 按 `core.state.splitMode` 持久化（uneven=像素 Cell W/H，默认 64、max 9999）；图片裁剪/缩放/透明后刷新 `processedImg`（Split Sheet 与 `splitImage`/`updateSplitSummary` 的源优先级：`processedImg` → 首帧 canvas → source 备份）。
> - **MediaBridge 逐资产投递**：`deliverPendingImports` 改为逐资产串行投递——`galleryImportAssets([asset])` 解析 `importedCount>0` 才 `consume(id)`；被拒（rollback）/跳过（bytes 不可解析）的资产留在 pendingImports 队列，下次 `renderGallery` 投递重试；队列原地 drain（`pendingImports.length=0`）后重排未消费项；`web/media-bridge.test.js` 增逐资产/重试用例。
> - **i18n**：导出弹窗按钮走 `gifEditorSaveAsZip`/`gifEditorSaveAsGif`/`gifEditorOpenGallery`/`gifEditorClose`（`gifEditorOpenGallery` 为「Open in Gallery」）；`gifEditorImageTools`/`gifEditorEdgeCropTitle`/`gifEditorCropTop/Bottom/Left/Right`/`gifEditorGridCols/Rows`/`gifEditorSliceBtn` 等键覆盖还原的图片工具面板。
> - **i18n 参数化确认文案（post-smoke 修复）**：模块内 `t(key, args, fallback)` 现把 args 数组透传给全局 `window.t(key, args)` 完成 `{0}/{1}/…` 占位符替换（转义后）；四个确认弹窗——删帧 `gifEditorConfirmDeleteRange`（`[start+1, end+1]`）、仅保留 `gifEditorConfirmKeepRange`（`[start+1, end+1]`）、全局缩放 `gifEditorResizeConfirm`（`[origW, origH, targetW, targetH]`）、间隔删除 `gifEditorIntervalDeleteConfirm`（`[interval, deletedCount, slices.length, kept.length]`）——全部改传字符串参数数组；`typeof args === 'string'` 时直接返回（旧式字符串 fallback 调用不视为替换数组，保持兼容）。修复 smoke 发现的删帧确认框字面渲染 `{0}` 的问题。配套零依赖 Node 合同测试 `web/gif-editor-i18n-args.test.js`（`node web/gif-editor-i18n-args.test.js`）：加载**真实** `web/static/i18n.js` + 全局 `t()` + 从 `gif-editor.js` 提取的**真实**消息表达式，断言四个确认键 en/cn 双语的 `{N}` 替换与 wrapper 回退兼容（缺失键回退、字符串 args 不替换）。本修复轮 `i18n.js` 共新增 **41 个 en 键（均带 cn）**：确认弹窗配套键（`gifEditorDeleteTitle`/`gifEditorDeleteConfirmBtn`/`gifEditorKeepTitle`/`gifEditorKeepConfirmBtn`/`gifEditorResizeTitle`/`gifEditorResizeBtn`/`gifEditorResizeConfirm`/`gifEditorIntervalDeleteConfirm`）、告警/通用键（`gifEditorAlertAllDeleted`/`gifEditorAlertIntervalInvalid`/`gifEditorAlertResizeEmpty`/`gifEditorApplyTo`/`gifEditorConfirmBtn`/`gifEditorCopiedToClipboard`/`gifEditorCopyFailed`/`gifEditorRenderFail`/`gifEditorResetTitle`/`gifEditorResetConfirmBtn`/`gifEditorResetView`/`gifEditorSplitSheet`/`gifEditorZoomIn`/`gifEditorZoomOut`）、图片工具面板键（`gifEditorImageTools`/`gifEditorEdgeCropTitle`/`gifEditorGridCols`/`gifEditorGridRows`），二次 smoke-gap 轮另增 15 键（见下条）。测试共 **14 个 `check()`**：8 个四确认键 en/cn 替换 + 3 个 wrapper 兼容（纯 fallback 解析/缺键回退/字符串 args 不替换）+ 3 个 smoke-flagged（`gifEditorSetLatency` en/cn 解析 + 五 CN 标签非英文断言）。
> - **i18n smoke-gap 补键（post-smoke 二次修复）**：`web/static/i18n.js` **新增 15 个 en 键**（均带 cn）——`gifEditorSetLatency`（'Set Latency'）、`gifEditorGridSplitTitle`（'Sprite Sheet Grid Split'）、`gifEditorEven`/`gifEditorUneven`（'Even'/'Uneven'）、`gifEditorSplitX`/`gifEditorSplitY`（'X (Horizontal Split):'/'Y (Vertical Split):'）、`gifEditorCellWidth`/`gifEditorCellHeight`（'Cell Width (px):'/'Cell Height (px):'）、`gifEditorCenterGap`（'Center Gap (px):'）、`gifEditorOuterMargin`（'Outer Margin (px):'）、`gifEditorEnableOuterGap`（'Enable outer border gap'）、`gifEditorSplitBtn`（'Split'）、`gifEditorTextColor`/`gifEditorStrokeColor`（'Text Color'/'Stroke Color'）、`gifEditorTimelineAria`（'GIF timeline'）。**CN 修正 9 处**（原为回退英文）：`gifEditorGlobalDelay`→'设置延迟'、`gifEditorDelRange`→'删除范围'、`gifEditorKeepRange`→'保留范围'、`gifEditorReduceFrame`/`gifEditorIntervalDeleteTitle`→'间隔删帧'、`gifEditorBatchDelayBtn`/`gifEditorIntervalDeleteBtn`→'应用'、`gifEditorExportTitle`→'导出'、`gifEditorBatchDelete`→'批量删除帧'。**CN 补齐 5 个此前缺失键**：`gifEditorMagicWandTitle`→'设置透明'、`gifEditorApplyTrans`→'应用'、`gifEditorCancelTrans`→'取消'、`gifEditorApplyDelay`→'应用'、`gifEditorCancelDelay`→'取消'。gif-editor.js 模板接线：Split Sheet 面板（GridSplitTitle、Even/Uneven、SplitX↔CellWidth 与 SplitY↔CellHeight 按模式动态切换、CenterGap、OuterMargin、EnableOuterGap、SplitBtn）、Overlay 面板（TextColor/StrokeColor）、时间线 `aria-label`（TimelineAria）。**Import Modal 全量接线（post-smoke 三次修复）**：`gif-editor-import.js` Import Modal 内 split 面板标题（`Sprite Sheet Grid Split`）、`Scale`、`Center Gap (px):`、`Outer Margin (px):`、`Enable outer border gap` 5 处标签原硬编码英文，现已随模板构建改走模块内 `t(key, fallback)`（`gifEditorGridSplitTitle`/`gifEditorScale`/`gifEditorCenterGap`/`gifEditorOuterMargin`/`gifEditorEnableOuterGap`），Import Modal i18n 全量完成；Even/Uneven 与 X/Y、单元格宽高标签此前已接线。`web/gif-editor-i18n-args.test.js` 扩展 smoke-flagged 断言（`gifEditorSetLatency` en/cn 解析 + 五个 CN 标签非英文）。
> - **i18n 硬编码标签清零（final five）**：GIF 编辑器最后 5 处硬编码英文标签全部接入 `t()` 且 en/cn 双语齐全——`#gif-open-export-btn` title「Export settings」→ 新键 `gifEditorExportSettingsTitle`（'Export Settings'/'导出设置'）；`#gif-reload-btn` title「Reset workspace」→ 既有键 `gifEditorResetWorkspace`（'Reset Workspace'/'重置工作区'）；`#gif-timeline-zoom-range` title「Zoom」→ 既有键 `gifTimelineZoom`（'Zoom:'/'倍率:'）；`#gif-modal-preview-img` alt「Export preview」→ 新键 `gifEditorExportPreviewAlt`（'Export preview'/'导出预览'）；`#gif-modal-preview-btn`「Preview」→ 新键 `gifEditorPreviewBtn`（'Preview'/'预览'，含 `data-i18n`）。`web/static/i18n.js` en+cn GIF 段新增 3 对键、复用 2 个既有键；涉及 `web/static/gif-editor/gif-editor.js`、`gif-editor-export.js`。
>
> **变更维护：** 涉及文档——`PROJECT_MAP.md` §18.2/§24、`docs/gif_upgrade.md`（zoom 范围 50%–200%→20%–300%）、`docs/playground-architecture.md`（§14 源码锚点 + MediaBridge 投递 + `/edit/upload-temp|zip-outputs` 契约行）、`docs/archive-architecture.md`（§7/§8/§11 MediaBridge 与 GIF 生产者）。
>
> **最后核对（2026-08-12，透明引擎重构：即时预览 / 缩略图同步 / 区域去背）：** 色键透明从"勾选才应用 + 破坏性烘焙"改为**非破坏提交模型**——slice canvas 永不烘焙，所有消费者（舞台预览、缩略图、导出）对原始帧跑 `GifEditorTransparency` 管线：面板打开时用 live DOM 参数即时预览（无需先点应用），Apply 把 live 参数冻结进 `state.trans.committed`，缩略图/导出只认 committed 快照（`isTransparencyEnabled` 现要求 `trans.committed` 非空）。新纯函数模块 `web/static/gif-editor/gif-editor-transparency.js`（`window.GifEditorTransparency`，零 DOM 依赖，Node VM 可测）提供三种去色策略 + 共享管线：**颜色去背**（redmean 感知色距替代旧 RGB box 容差，0–100 容差映射 0–765）、**区域去背**（flood fill 扫描线从种子点扩散，只删与种子同色**连通**区域，纯色背景精灵图内部同色像素幸存；已透明像素可穿越，后续种子能越过已删背景；角点预设 `cornerFloodToAlpha` 一键去四角背景）、**软边去背**（GIMP Color-to-Alpha：部分 alpha 渐变 + 颜色去污染，保抗锯齿边缘，GIF 单透明索引不支持部分 alpha，仅 PNG/精灵图/ZIP/预览生效）。变换类操作（全局裁剪/全局缩放/网格切片/Split Sheet）在替换 canvas 前 `materializeTransparency()` 把 committed 移除烘焙进新帧并清空 committed（flood 种子坐标随几何失效）。新增 i18n 键 11 个（模式/种子/角点/软边/提示，en+cn）。测试：`web/gif-editor-transparency.test.js`（24 项纯函数契约）+ 既有 3 测试全绿 + headless 浏览器冒烟 18/18（live 预览、应用后缩略图透明、区域去背保留角色、导出 compose matte、取消/禁用语义）。**后续修正：** 缩略图透明背景由纯黑（`.gif-thumb-wrapper{background:#000}`）改为**主题感知棋盘格**（暗色 `#262626`/`#3a3a3a`、亮色 `#f0f0f0`/`#d0d0d0`，12px 对角双渐变层，`style.css` 4562 行附近），headless 计算样式断言 5/5（暗/亮基色 + 渐变层 + 瓦片尺寸 + 缩略图 alpha 保留）。
>
> **变更维护：** 涉及文档——`PROJECT_MAP.md` §18.2/§24、`docs/gif_upgrade.md`（§4.1 状态表 + script 顺序 + node --check 清单）。
>
...
>
> **已落地（2026-08-06，升级重构）：** 按照 `docs/gif_upgrade.md` 方案落地受控模块拆分与 Import Modal / Timeline Toolbar 升级：(1) 在 `web/static/` 拆分为 `gif-editor-state.js`、`gif-editor-import.js`、`gif-editor-timeline.js`、`gif-editor-playback.js`、`gif-editor-export.js` 以及 `gif-editor.js` 入口；(2) 文件选择/拖放/粘贴统一进入 Import Modal（支持比例、FPS、双手柄起止时间范围、实际帧数与播放时长精确统计联动，取消不影响现有工程）；(3) 时间线独立横向滚动区，支持 50%–200% 动态缩放；(4) 底部新增 First/Prev/Play/Pause/Next/Last 帧焦点与按 delay 自动播放控制，全套 Modal & Toolbar 主题响应式支持；(5) 两个 HTML 入口脚本引入顺序、i18n、style.css 及文档全部同步完成。
---
...
| 色键透明 | 2009-2077, 2662-2687 → 2026-08-12 重构为 `gif-editor-transparency.js` 纯函数管线 + `gif-editor.js` 会话层（`state.trans` committed 快照、`materializeTransparency`、`previewTransParams`） | 颜色去背（redmean 容差）+ 区域去背（flood fill 种子/角点预设）+ 软边（GIMP C2A）；面板打开即时预览，Apply 冻结 committed，缩略图/导出只认 committed；取色 EyeDropper 或 canvas 像素 |
...
- **class/id 全前缀化**：所有原 class 加 `gif-`（`gif-workspace`/`gif-sidebar`/`gif-stage`/`gif-timeline`/`gif-slice-item`…），所有 id 加 `gif-`（`gif-drop-zone`/`gif-file-input`/`gif-export-btn`…≈80 个）。理由：原页面用 `.btn/.btn-primary/.hidden/.modal-overlay` 等与 TinyLab `style.css` 全局类直接冲突（TinyLab 有 `.btn/.btn-primary/.btn-danger/.modal-*` 体系）。
...
| GIF | gif.js 编码（`gif-editor-export.js::renderGifBlobFromModal`） | 导出弹窗「Preview」触发 `renderGifBlobFromModal`：`ensureGifJs` 确保 vendor 就绪；`new GIF({workers: 4, quality: clamp(1..10), width, height, workerScript: GIF_WORKER_URL})`，encoder 存 `currentEncoder`（可中止）；透明开启（`isTransparencyEnabled`：checkbox && `transparencyReady` && **`trans.committed` 非空**——只导出已应用快照，不用 live 面板参数）时设 `transparent: MATTE_NUM`（`#FF00FF` matte）；逐帧 `renderCompositedFrame(…, {applyTransparency: transEnabled, matte})`（composeFrame 对 boolean 解析为 committed 快照，对帧原始 canvas 跑管线；不再按 `srcImg` 二次套用当前 keyColor）+ `delay = max(1, round(slice.delay)…` |
|---|---|---|
| 序列帧 ZIP | **Archive 主路线 + legacy 回退**（`gif-editor-export.js::exportZipViaArchive`/`exportZipLegacy`） | 先经 `MediaBridge.archiveStatus()` 探测：Archive 可用 → `exportZipViaArchive`：逐帧 `POST /api/archive/assets?name=frame_NNN.png`（原始 PNG body，每批 50 串行）→ `POST /api/archive/pack` JSON `{assetIds, format:'zip', name}` → 返回 `{assetId}` → 结果以受控 URL `/api/archive/assets/{id}` 展示并 `setHandoff({kind:'archive', format:'zip', url})`。**帧资产释放纪律**：`registered[]` 逐 id 跟踪成功上传，`releaseRegistered()` 在**上传失败、pack 失败与成功**三条路径都释放帧资产；打包结果资产**不在前端释放**——归 MediaBridge token 持有至 Gallery 消费或服务端 TTL。Archive 缺失 → `exportZipLegacy`：`POST /api/gallery/edit/upload-temp?name=frame_NNN.png`（**服务端保留请求 basename**，不再折叠成 upload.png）→ `POST /api/gallery/edit/zip-outputs {assetIds, zipName, cleanUp:true}`（服务端 cleanUp 在成功与所有失败路径都释放输入帧资产） |
| 精灵图 | 前端 canvas 合成（`gif-editor-export.js::exportSprite`） | rows×cols/Start Frame 经 `readSpriteParams()` 读 `#gif-modal-sprite-rows`/`#gif-modal-sprite-cols`/`#gif-modal-sprite-start-frame`（均 ≥1 / ≥0，模板默认 1×5）；`renderCompositedFrame` 逐格绘制 → 单次 `toBlob('image/png')` → 缓存 `previewCache.png`（key=`spriteConfigKey(outW,outH,rows,cols,startFrame)`）+ `setHandoff({name:'SpriteSheet_<ts>.png', mime:'image/png', kind:'image', format:'png', blob})` |
>
> 三条导出路径统一走**导出弹窗（export modal）**（旧 `showResultModal`/`#gif-result-overlay`/`lastResultAsset` 已随 M2 移除）：GIF 与精灵图预览写入独立 `previewCache`（按渲染参数 config key 隔离），「Save as ZIP / Save as GIF / Save as Sprite」保存前按**格式（MIME）+ config key** 重查缓存——绝不把 PNG 字节存成 .gif、不复用不同 W/H/quality/拼图参数下的预览；「Open in Gallery」（`#gif-modal-open-gallery-btn`，导出成功后启用）经 `openResultInGallery()` **惰性** `MediaBridge.register(handoff.asset)`（首次点击才登记，预览不镜像字节到服务端）后 `MediaBridge.openGallery(assetId)`；`closeExportModal`/`cleanup` 中止在途 `currentEncoder`、清除 `gifRenderTimeout`、`revokePreviewUrl` 并清空缓存与 handoff（监听 `tinylab:modal-close`，具名并在 cleanup 移除）；不触碰 `galleryState`、不传绝对路径。弹窗文案走 i18n（`gifEditorSaveAsZip`/`gifEditorSaveAsGif`/`gifEditorOpenGallery`/`gifEditorClose`）。
...
| 新增 | `web/static/gif-editor-export.js`（GIF/ZIP/精灵图三导出 + export modal `previewCache`/`setHandoff`/`openResultInGallery`/`closeExportModal`/MediaBridge 惰性登记 + ZIP 帧资产释放纪律） |
...
| 新增 | `web/gif-editor-i18n-args.test.js`（零依赖 Node 合同测试：模块内 `t(key,args,fallback)` 的 `{N}` 占位符替换 + 字符串 fallback 兼容；**2026-08-11 二次扩展** smoke-flagged 键覆盖——`gifEditorSetLatency` en/cn 解析 + 五个 CN 标签非英文断言（globalDelay/delRange/keepRange/even/uneven），共 **14 个 `check()`**（8 确认键 en/cn + 3 wrapper 兼容 + 3 smoke-flagged）；置于 embed 目录之外，不进入生产静态树） |
...
- **模块拆分与加载顺序**：`gif-editor-state.js`（`window.GifEditorCore` 注册表 + `resetSlices`/`releaseSource`/`showSpinner`/`hideSpinner`/`bindStateEvents`）→ `gif-editor-import.js` → `gif-editor-timeline.js` → `gif-editor-playback.js` → `gif-editor-export.js` → `gif-editor.js`（入口 `renderGifEditor`/`cleanupGifEditor` + 模板/cacheDom/`applyPageI18n`/`registerCoreCommands`），`index.html` 与 `index-nopg.html` 顺序一致；`media-bridge.js` 先于生产者加载。**2026-08-11：** cleanup 注册表持久化（`cleanupModules` 不再清空 `cleanupFns`，幂等 cleanup 每次 teardown/re-render 都执行）；`bindStateEvents`/`cleanupStateEvents` 把 window 级键盘块改为生命周期绑定（render 加、teardown 移），state 模块经 `registerModule('state', {cleanup})` 自登记，重复 enter/leave 不累积监听、不残留 `isExtracting`/spinner。
- **Import Modal**：文件选择/拖放/粘贴统一入口；GIF 用 gifuct-js `parseGIF`+`decompressFrames` + disposal 1/2/3 合成（上一帧 disposal 先应用、disposal-3 绘制前快照）；**delay 按 ms 直用（gifuct-js 已是 ms）**；`fpsChanged` 前默认保留源帧 delay；视频 seek 抽帧；比例（10–300%）、FPS（1–60）、双手柄起止滑块 + 毫秒输入联动、实际帧数/时长统计、120ms 去抖预览；commit 事务 + `commitGeneration` 防陈旧提交；取消保留旧工程；200MB 单文件上限。**2026-08-11：** `draftGeneration` 随 cancel/open/cleanup 递增，Image/GIF/FileReader/视频 `loadedmetadata` 等异步元数据读取均校验代数（陈旧读取静默中止）；commit 成功后清 `source.image`/`source.rawImage`（Split Sheet 不回滚到旧图）；GIF 范围选帧的切边帧 delay 按重叠区间裁剪（提交播放时长==endMs−startMs）；gap/margin 为输出像素不再乘 scalePercent；`cancelDraft` bump 两个 generation 并强制 `hideSpinner`。
- **时间线**：虚拟化窗口（`TL_BUFFER=4`、FIFO 缩略图缓存 `THUMB_CACHE_MAX=256`、rAF 节流）、**20%–300% 倍率**（`setZoom` clamp 0.2–3.0）、容器委托的选中/复制/删除/delay 输入/拖拽排序；style.css「GIF Editor Upgrade Styles」含 `.gif-timeline-item`/`.gif-thumb-wrapper`/`.gif-timeline-info`/`.gif-timeline-item-actions`/`.btn-copy-slice`/`.btn-del-slice` 全套规则。**2026-08-11：** 时间线 zoom 与舞台 zoom（`core.state.scale`/`updateTransform`）彻底解耦——Ctrl/Meta+滚轮在时间线滚动区只调 `timeline.zoom`（`setZoom(getZoom()+delta)`），舞台缩放由 gif-editor.js 舞台 wheel 监听独占；`updateZoomDisplay` 读 `getZoom()` 而非 `core.state.scale`。
...
- **1:1 编辑功能（还原）**：色键透明（取色/容差/取消）、边缘裁剪 + 网格切片（`gif-slice-btn`→`runSlice`）、全局裁剪（`gif-global-crop-btn` + crop-panel 微调滑条 + gizmo）、图层（文字/贴图/作用范围/描边/缩放/删除）、批量延迟、删帧/仅保留范围、输出宽高/缩放（`gif-output-scale-slider`，100%=1.0x，可经 `gif-scale-percent` + `gif-apply-scale-btn` 百分比应用）/质量（1-10 滑块联动）、拼图 Sheet 行/列参数 + `gif-export-sprite-btn` 导出按钮；导出按钮初始 disabled、导入后启用。**2026-08-11：** 图片源新增「Edge Crop & Grid Slice」折叠面板（`#gif-image-tools-toggle-btn`/`#gif-image-tools-panel`，`updateSourcePanels` 改 `display:block/none` 取代 `gif-hidden` 类）；Split Sheet Even/Uneven 模式在 `pageTemplate` 按 `core.state.splitMode` 持久化（uneven=像素 Cell W/H，默认 64、max 9999）；图片裁剪/缩放/透明后刷新 `processedImg`（`splitImage`/`updateSplitSummary` 源优先级：`processedImg` → 首帧 canvas → source 备份）。
- **导出**：GIF（gif.js `workers:4`、quality clamp 1-10、色键开启时 matte `#FF00FF`、`GIF_WORKER_URL` 同源 worker、`ensureGifJs` 兜底、`currentEncoder` 可中止 + 60s fail-safe `gifRenderTimeout`）、序列帧 ZIP（Archive `assets→pack→受控 URL` 主路线 + upload-temp/zip-outputs 回退；**帧资产逐 id 跟踪，上传/打包失败与成功均释放，结果资产归 MediaBridge token**；legacy `upload-temp` 保留 `frame_NNN.png` basename、服务端 cleanUp 全路径释放）、PNG 精灵图；**导出弹窗（export modal）取代旧结果 overlay**——`previewCache` 按格式/config key 隔离预览，Save 前重查 MIME 防错存。
- **MediaBridge/Archive 交接**：导出结果统一 `setHandoff`，`openResultInGallery` 首次点击才 `MediaBridge.register`（惰性，预览不镜像字节）后 `openGallery`；`MediaBridge.deliverPendingImports` 逐资产投递——`importedCount>0` 才 `consume`，拒绝/跳过资产留队下次重试；导出峰值 1.5GB confirm 警告（`checkExportMemory` factor 3/1）；`resetSlices`/`releaseSource` 释放帧 canvas 与 source objectURL（含 `rawImage` 备份）；`tinylab:modal-close` 监听具名并在 cleanup 移除。
- **i18n**：模板 `[data-i18n]`/`[data-i18n-placeholder]` 每 render 由 `applyPageI18n` 应用；导出弹窗文案走 `gifEditorSaveAsZip`/`gifEditorSaveAsGif`/`gifEditorOpenGallery`/`gifEditorClose`（旧结果弹窗键 `gifEditorResultTitle`/`gifEditorDownloadGif` 已不再使用）；图片工具面板键 `gifEditorImageTools`/`gifEditorEdgeCropTitle`/`gifEditorCropTop/Bottom/Left/Right`/`gifEditorGridCols/Rows`/`gifEditorSliceBtn`；13 个缺失键（`gifEditorNoLayer`/`gifEditorConfirmReset`/`gifEditorAlert*` 等）已补 en/cn。**2026-08-11（post-smoke 修复）：** 模块内 `t(key, args, fallback)` 透传参数数组给全局 `window.t` 做 `{N}` 占位符替换（`gifEditorConfirmDeleteRange`/`gifEditorConfirmKeepRange`/`gifEditorResizeConfirm`/`gifEditorIntervalDeleteConfirm` 四确认弹窗改传 `[String(...)]`），字符串 args 直接返回保持旧式调用兼容；`web/gif-editor-i18n-args.test.js` 零依赖 Node 合同测试锁定 en/cn 替换与回退语义。**2026-08-11（post-smoke 二次修复）：** 新增 15 个 en 键（`gifEditorSetLatency`/`gifEditorGridSplitTitle`/`gifEditorEven`/`gifEditorUneven`/`gifEditorSplitX/Y`/`gifEditorCellWidth/Height`/`gifEditorCenterGap`/`gifEditorOuterMargin`/`gifEditorEnableOuterGap`/`gifEditorSplitBtn`/`gifEditorTextColor`/`gifEditorStrokeColor`/`gifEditorTimelineAria`）+ CN 修正 9 处（GlobalDelay/DelRange/KeepRange/ReduceFrame/IntervalDeleteTitle/BatchDelayBtn/IntervalDeleteBtn/ExportTitle/BatchDelete）+ CN 补齐 5 个缺失键（MagicWandTitle/ApplyTrans/CancelTrans/ApplyDelay/CancelDelay）；Split Sheet/Overlay/时间线 aria 标签全部接线；**Import Modal 全量标签接线**——`gif-editor-import.js` 模板构建走 `t(key, fallback)`（`gifEditorGridSplitTitle`/`gifEditorScale`/`gifEditorCenterGap`/`gifEditorOuterMargin`/`gifEditorEnableOuterGap`），Import Modal i18n 完成；测试扩展 smoke-flagged 键断言。
...
3. 低优先级遗留（审计 L 级，未逐一修复）：Firefox 拖拽未 `setData`（dragstart 可能不发起）；delay 输入无 60000ms 上界钳制且接受小数；播放到末帧停止而非循环；时间线条目在放大 ≥1.5 时高度可超 130px 轨道（`overflow-y:hidden` 裁剪）。这些不影响主流程。**2026-08-11：** 原「移动端时间线高度（`min-height:190px`）小视口偏紧」已修复——`style.css` GIF 段**末尾**新增权威窄视口规则（≤768px：`.gif-workspace` 纵向滚动、`.gif-stage-area` `min-height:260px`、`.gif-timeline-area` `min-height:180px`、`.gif-sidebar` `max-height:38vh`；≤480px 导出弹窗 footer 换行 + sprite 保存按钮整行；≤380px 弹窗宽度自适应），另 3 处 `--accent-color`→`--accent` token 修正。**2026-08-11（smoke-gap 三次修复，残留关闭）：** 原遗留的 5 处 Import Modal 硬编码标签——split 面板标题（`Sprite Sheet Grid Split`）、`Scale`、`Center Gap (px):`、`Outer Margin (px):`、`Enable outer border gap`——已在 `gif-editor-import.js` 模板构建改走模块内 `t(key, fallback)`（`gifEditorGridSplitTitle`/`gifEditorScale`/`gifEditorCenterGap`/`gifEditorOuterMargin`/`gifEditorEnableOuterGap`），Import Modal i18n 全量完成；Even/Uneven 与 X/Y、单元格宽高标签（`gifEditorEven`/`gifEditorUneven`/`gifEditorSplitX/Y`/`gifEditorCellWidth/Height`）此前已接线（`updateImportSplitLabels` 按模式动态切换）。不再遗留。
