...
> **最后核对（2026-08-09，audit_fix.md Phase A/E 落地）：** (1) **管理面认证状态机（F-04）**：`internal/api/auth/handler.go::AuthMiddleware` 现强制 setup-required——未设置密码（含旧 `PasswordEnabled=false` 配置）时 `/api/providers`、`/api/keys`、`/api/settings`、filetransfer/gallery/editor/archive 等全部管理路由返回 401 `setup_required`（`internal/api/api_test.go::TestSetupRequired_BlocksManagementRoutes` 覆盖），不再无条件放行；`POST /api/auth/setup`（`SetupHandler`）为 bootstrap（未保护时设置密码并签发 session+CSRF，已保护 409、空密码 400）。密码变更/登录/setup 响应均携带 `csrfToken`。`AuthStatusHandler` 返回 `setupRequired`。(2) **凭证最小化（F-04）**：Provider/Key 列表/创建/更新只返回公开 DTO——`ProviderDTO`（`internal/api/providers/register.go`，含 `keyCount`/`hasKey`、无 `Keys` 数组）、`KeyDTO`+`MaskKey`（`internal/api/keys/register.go`，≤8 字符全 `***`，长 key 仅末 4 位，不可逆）；Settings GET `anySearch` 只返回 `hasApiKey`。(3) **CSRF（F-05）**：`AuthMiddleware` 对 POST/PUT/PATCH/DELETE 校验 session-bound `X-CSRF-Token`（`csrfChecksPass`）+ Origin/Referer（`originAllowed`）+ JSON/multipart Content-Type（`requireJSONBody`）；`web/static/auth.js` 全局 fetch 包装自动注入 token。(4) **reload 收敛（F-17）**：`internal/api/settings/register.go::convergeRuntime(cfg)` 为单一运行时收敛点（QuickSlotOnly/LogRequests 原子、`ProxyHandler.SetRequestLogDir`、`validateProxyConfig`+`SetProxy`、`ServerCfgFn`/`UpstreamTimeoutFn`、`Selector.UpdateSettings`、`pushDownloadSettings`、`ArchiveSettingsFn`）；`reload` handler 先校验代理再 `Reg.Reload(cfg)`+`convergeRuntime(cfg)`（`converge_test.go`）。(5) **深拷贝（F-18）**：`registry.Registry::Config()` 现返回 `cloneConfig(*r.config)`（JSON round-trip 深拷贝，`deepcopy_test.go`）。(6) **state key 编码（F-21）**：`KeySnapshot` 新增结构化 `ProviderID`/`KeyID` + `state.EncodeSnapshotKey` 长度前缀编码（`state_key_test.go`），`Restore` 兼容 结构化→长度前缀→legacy `::`。(7) **解密失败（F-20）**：`finalizeConfig` 解密错误 → stderr 审计告警 + `k.IsActive=false`（`decrypt_fail_test.go`），`enc:` 密文绝不发给上游。(8) **Provider 私网 opt-in（F-06）**：`config.Provider.AllowPrivateNetwork bool`（`allowPrivateNetwork,omitempty`），供 `apibase.Deps::ManagementClient` 的 `internal/outbound.Policy` 使用。源码锚点均为稳定符号名（见 §22/§23）。
