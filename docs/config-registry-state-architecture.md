...
> **最后核对（2026-08-29，Music MVP Jamendo+Local+Musics 闭环）：** `web/static/music/host.js`（`MusicHost` 沙箱+`jamendo`/`local` 内置 provider + `loadFromSource` 并发 `search/getMediaSource`）、`web/static/music/player.js`（`MusicPlayer<audio>` 队列/随机/循环+`playAt/next/prev/seek/cycleLoop`）、`web/static/music/music.js`（Jamendo 搜索→播放→+Queue→↓落盘 Musics + Local `createObjectURL` 入队 + Now Playing/进度拖拽 + Library + `cleanup/suspend/resume`）、`internal/api/music/register.go`（`GET /api/music/library`、`POST /api/music/proxy` 透传（`Access-Control-Allow-Origin:*`）、`POST /api/music/download` 原子落盘 `(1)` 去重 + `filepath.Base` 防穿越 + `tmp+Rename`）、`internal/api/router.go`（`music.NewHandler` 挂 `r.Route("/music")` 于 `auth Group` 内、`securityHeaders connect-src` 放行 `https://api.jamendo.com`）。详见 `docs/music-implementation-plan.md` §2–§5 及 `docs/music-architecture.md`。
...
> **最后核对（2026-08-24，Assistant Action 化）：** `Config.Assistant` 由 `{Model, SpritesheetPath, SpritesheetFps}` 改为 `{Model, Actions []config.AssistantAction}`；`AssistantAction{Name,SpritesheetPath,Cols,Rows,FrameStart,FrameEnd,Fps}` 描述一个精灵动画（网格行主序 0 基帧范围 + fps），`finalizeConfig` 归一非法值（cols/rows≥1、frameEnd≥frameStart、fps≤0→8）。PATCH `assistantPatch` 改为 `{Model *string, Actions *[]config.AssistantAction}`——Actions 存在即整表替换，与 Settings 弹窗"编辑整个动作列表后一次保存"的交互一致。spritesheet 文件经 `internal/api/assistant/sheet.go` 服务：native picker 选出的绝对路径由 `POST /api/assistant/sheet-preview` 注册为 owner 绑定 1h TTL 随机 id（扩展名白名单 + 64MiB 上限），运行时宠物页按 action 名从 `GET /api/assistant/sheet-image/{name}` 加载。
> **最后核对（2026-08-09，密码保护可选性修复）：** 管理面认证恢复为可选模式：`internal/api/auth/handler.go::AuthMiddleware` 在 `PasswordEnabled=false` 时直接放行管理路由，`AuthStatusHandler` 返回 `setupRequired:false` 与已认证状态；仅开启密码保护后要求 session-bound CSRF、Origin/Referer 和 JSON/multipart Content-Type。前端 `app.js`/`api.js` 仅在保护开启且无有效会话时显示登录屏，不再把无密码状态当作强制 setup。`POST /api/auth/setup` 保留为用户主动启用密码的可选 bootstrap。`internal/api/auth/auth_test.go` 与 `internal/api/api_test.go` 覆盖无密码启动、管理页访问和主动设置密码流程。
...
> **最后核对（2026-08-19，小精灵助手配置）：** `Config.Assistant`（`AssistantConfig{Model,SpritesheetPath,SpritesheetFps}`，`yaml/json:"assistant,omitempty"`）新增。`Model` 为模型辅助意图分类所用模型 id（provider-prefix/model 或 combo 名），空=仅关键词 fallback；`SpritesheetPath`/`SpritesheetFps` 配置角色动画资产（`finalizeConfig` 给 `SpritesheetFps` 默认 8）。Settings GET 返回 `assistant` 对象；PATCH 接受 presence-aware `assistantPatch`（全指针字段，经 `applyAssistantUpdates` 逐字段合并），沿用 rotation/theme/archive 的 presence-aware 纪律——不整结构覆盖。不落盘运行时状态，`state.yaml` 无 assistant 段。前端 `web/static/settings/settings.js`（Settings 侧栏 Assistant 行）+ `settings_modal.js`（`openAssistantModal`/`saveAssistantModal`）+ `i18n.js`（`assistantSettings` 等 8 键 EN+CN）。涉及文件：`internal/config/types.go`、`defaults.go`、`internal/api/settings/register.go`、`web/static/settings/settings.js`、`settings_modal.js`、`web/static/i18n.js`。
...
| `Provider` | types.go:74-96 | `ID`/`Name`/`Prefix`/`BaseURL`/`APIType`/`IsActive`/`Keys`/`Models`/`RotationStrategy`/`StickyLimit`/`InjectStreamOpts`/`NormalizeStreamChunks`/`NIMConfig`/`HardLimit`/`UseProxy` 等 |
| `HardLimitSettings` | types.go | `RPMEnabled` / `RPM` / `TPMEnabled` / `TPM`（站点级滑动 60s 窗口 RPM/TPM 限流配置） |
...
| `AssistantConfig` | types.go（2026-08-19 新增；**2026-08-24 Action 化**，`Config.Assistant`，`yaml/json:"assistant,omitempty"`） | `Model` / `Actions []AssistantAction`——`Model` 为模型辅助意图分类所用模型 id（provider-prefix/model 或 combo 名），空=仅关键词 fallback。`AssistantAction{Name,SpritesheetPath,Cols,Rows,FrameStart,FrameEnd,Fps}` 定义一个精灵动画：spritesheet 按 Cols×Rows 网格分割、行主序 0 基编号（左上=0），播放 `[FrameStart,FrameEnd]` 帧于 Fps；`finalizeConfig` 归一 cols/rows≥1、frameStart≥0、frameEnd≥frameStart、fps≤0→8（旧顶层 `SpritesheetPath`/`SpritesheetFps` 字段已移除，旧 config 值被忽略）。PATCH 为 presence-aware `assistantPatch`（`Model *string` + `Actions *[]AssistantAction` 整表替换，经 `applyAssistantUpdates` 合并） |
...
| 文件 | 主要方法 | 行为要点 |
|---|---|---|
| `providers.go` | `ListProviders`(11-17)、`HasProvider`(20-29)、`GetProvider`(31-41)、`GetProviderByPrefix`(44-54)、`AddProvider`(56-70)、`UpdateProvider`(74-97)、`DeleteProvider`(99-116)、`UpdateProviderStrategy`(119-130)、`ReorderProvider`(133-168) | `AddProvider` 为每个 key 初始化 `KeyRuntimeState`（61-69）；`UpdateProvider` **仅更新标量字段**（含 `AllowPrivateNetwork` 写回，2026-08-22），**不触碰 Keys/Models**（注释 92-93）；`DeleteProvider` 在 `stateMu` 下清理该 provider 全部 key 的 state（103-107）；`ReorderProvider` 按避让/插值规则调整指定 provider 在列表中的索引。读返回副本（35-38、49-52） |
...
10. **`UpdateProvider` 不触碰 Keys/Models：** 仅更新标量字段（providers.go:74-97，注释 92-93；2026-08-22 起含 `AllowPrivateNetwork`）；key/model 需走各自 CRUD——批量整体替换 provider 会丢失 key 运行时状态指针关联（若 key ID 不变则 reload merge 仍能复用，但 UpdateProvider 本身不动 states）。
...
- `providers.go`：ListProviders/HasProvider/GetProvider/GetProviderByPrefix(11-54)、AddProvider(56-70)、UpdateProvider(74-97，含 UseCustomHeaders/CustomHeaders/AllowPrivateNetwork 写回)、DeleteProvider(99-116)、UpdateProviderStrategy(119-130)。
...
| FileTransfer 上传与路径容量 | `internal/filetransfer/upload.go`（`Upload`/`PathInfo`/`PasteClipboard`/`collectParts`/`appendLocalPath`/`appendLocalFile`）+ `internal/api/router.go`（`/api/filetransfer` 路由组：feature gate、认证与 600MiB body 上限，`fileTransferHandler.Register` 内含 `owner.Middleware`）+ `web/static/filetransfer.js`（multipart 上传、剪贴板容量、接收区域拖拽保护、Clear、XHR 进度）+ `web/static/i18n.js`/`style.css`；上传成功依赖外部临时服务，路由本身不保证外部服务可用。**2026-08-09（audit F-01）：** 浏览器只提交 multipart 文件或 `pathGrantId`（`h.grants.Resolve(ownerID, id, pathgrant.OpExport)`），raw `paths` 400/410 拒绝；`/paste` 服务端剪贴板 → export grants；`path-info` 只查已登记 grant；上限 500MiB 归档/2000 文件/1GiB 总输入/深度 32/扫描 30s |
| 修改管理面认证/CSRF | `internal/api/auth/handler.go`（`AuthMiddleware`/`SetupHandler`/`AuthStatusHandler`/`SessionStore`/`CSRFToken`/`csrfChecksPass`/`originAllowed`/`requireJSONBody`）+ `rate_limit.go`（`loginGuard`/`loginResponseWriter`）+ `internal/api/router.go`（全部 `/api/*` 组经 `authHandler.AuthMiddleware`）+ `config/types.go`（`SecurityConfig`）+ `web/static/auth.js`（全局 fetch 注入 `X-CSRF-Token`，仅保护开启时显示登录屏）+ `web/static/api.js`（401/403 仅触发登录屏）；`PasswordEnabled=false` 为直接访问模式，`setupRequired` 不再阻塞管理 UI。`auth_test.go`/`api_test.go` 覆盖可选保护、密码登录、主动 setup、CSRF 与关闭保护后的页面切换。
