...
| `AssistantConfig` | types.go（2026-08-19 新增，`Config.Assistant`，`yaml/json:"assistant,omitempty"`） | `Model` / `SpritesheetPath` / `SpritesheetFps`——全可选：`Model` 为模型辅助意图分类所用模型 id（provider-prefix/model 或 combo 名），空=仅关键词 fallback；`SpritesheetFps` 零值由 `finalizeConfig` 归一为 8。PATCH 为 presence-aware `assistantPatch`（全指针字段，经 `applyAssistantUpdates` 逐字段合并），不整结构覆盖 |
