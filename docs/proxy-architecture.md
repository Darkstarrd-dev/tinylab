...
> **最后核对（2026-09-05，RES=enc 哨兵 + delta/done 去重）：** 密文 reasoning（Responses `encrypted_content`，如 muse-spark 类模型）无明文可计，RES 恒 0 无法区分"无推理"与"有但不可见"；Responses 终态 `output_item.done`/`completed` 回带全文经 `"text":"` 又计一次，CT 约翻倍。(1) **enc 哨兵**——`stream_usage.go` 新增 `reasoningEncryptedSentinel=-1` + `sseHasEncryptedReasoning`（非空 `encrypted_content` 触发）+ `applyReasoningSentinel`（明文永远覆盖）+ `splitOutputTotal`（哨兵归零）；`stream.go`/`responses_translate.go` 双分支检测 + ticker/终态接哨兵；`UpdateTokensSplit` 哨兵只粘 0、明文覆盖、无信息 0 不清 enc；前端 `monitor_state.js` 新增 `resIsEnc/resNum/resDisplay`，RES 格渲染 `enc`，OUT/SPD 归零参与；merge/live-patch/tick 三处单调全部哨兵感知。(2) **delta+done 去重**——`countContentSplit` 对 native Responses 非 `*.delta` 事件直接归零（`isResponsesDeltaEvent` 判 `.delta"`）；纯透传的 delta 行不受影响，混合流 translated-first 规则不变。(3) 测试：`TestSSEHasEncryptedReasoning` + `TestCountContentSplit_NativeDoneEchoSuppressed` + `TestStreamResponse_EncryptedReasoningSentinel`（RES=-1、CT 单计、OUT==CT），全量 proxy/usage suite + vet + 4 个 monitor JS `node --check` 通过。
> **最后核对（2026-09-05，Recent Requests 11 列：OUT 拆 RES/CT + GT 跳变根治）：** 用户实测：llama 推理流增量全在 `reasoning_content`（`content:null` 直到正文），Responses 原生增量是顶层 `"delta":"..."`——旧 marker 两者全漏致 live OUT=0；GT 反复跳另有独立根因。(1) **RES/CT 分流**——`usage.Entry` 加 `ReasoningTokens/ContentTokens`；`sseContentLength` 加 `"delta":"`（Responses output_text），新增 `sseReasoningLength` 认 `reasoning_content/reasoning/reasoning_text`（与 CT 互斥，单测隔离）；`stream.go` 双分支计数 + `reasoningCharsTotal` atomic + ticker `effRes/effCt` + 终态 split；`responses_translate.go` 把 reasoning summary 折成 `reasoning_content` chat chunk 再统一计数；`recordUsage` 可变尾参 `split...`（~20 老调用点零改动）；`broadcastTokensSplit` + `UpdateTokensSplit`（单调）；前端 OUT 格→RES/CT 两格，Spd=`(RES+CT)/GT`，colspan 10→11，aggregate-only 事件 delta 归 CT。(2) **GT 跳变根治**——`EntryTracker.Refresh` 改写 `Timestamp`（请求起始）是真凶：每秒把 GT 锚点 `ts+ttft` 后移致回跳。改 `lastActive` 独立心跳 map，Timestamp 永不动；`SweepStale` 判心跳（旧 stale 测试改心跳语义）；fcm 锚点优先级保留（reasoning 即起算）。(3) 测试：RES/CT 隔离 + 推理流终态 `OUT=RES+CT` + Refresh 不动 Timestamp 三用例，全量 proxy  suite 通过。
...
> **最后核对（2026-08-15，内存泄漏与高水位线治理）：** (1) **Base64 图片占位**：`internal/proxy/recorder.go::captureBody` 与 `forward_retry.go` 处理请求与响应体时，检测并使用正则表达式将 `data:image/...;base64,...` 及 `"b64_json"` 替换为形如 `[image omitted: %d bytes]` 的轻量占位符，视觉多图请求不再将多兆大图存入 `usage.RingBuffer`、`EntryTracker` 或 SSE 广播；(2) **EntryTracker 孤儿在途请求定时清理**：`Handler` 新增 `SweepStaleEntries` 与 `StartEntryTrackerSweeper`，由 `internal/app/app.go` 启动后台 goroutine 每分钟主动清理 5 分钟未完结的 processing 条目、扣减 key 的 in-flight 计数并广播 `request-done (error)`；(3) **Trace attemptCounter 即时清理**：`request_log.go` 新增 `clearAttemptCount`，`forwardWithRetry` 退出时通过 `defer h.clearAttemptCount(reqID)` 立即移除对应条目，切断全局 `attemptCounter` 膨胀；(4) **pprof 路由与内存释放**：`router.go` 挂载 `/debug/pprof/*`，且在清理点触发 `runtime/debug.FreeOSMemory()` 释放物理内存页。
> **最后核对（2026-08-09，docs/audit_fix.md Phase D/E 落地）：** (1) **响应预算（F-14）**：`internal/proxy/stream.go::passThroughResponse` 不再无上限读上游 body——`maxPassThroughBodyBytes=256MiB`（`Handler.maxPassThroughBody` 可被测试缩小），先完整读入预算内 body、读成功后才 `WriteHeader`，超预算 → 受控 502 + `recordUsage` 错误（`TestPassThrough_LargeBodyStreamsFully` 64MiB 通过）；流式 `streamResponse` 用 `sse.NewSSELineBuffer` 预算化行缓冲，`Feed` 失败 → 受控中断 `SSE stream exceeded line buffer budget`；usage 捕获（`maxStreamCaptureBytes` 512KiB）与客户端传输预算分离（§8.1/§8.6 更新）。(2) **日志脱敏（F-23）**：`internal/proxy/request_log.go` 集中 `sensitiveHeaderNames` + `isSecretHeader` + `maskHeaderMap`（`*Handler` 方法，含 `isCustomSecretHeader` 覆盖 per-provider CustomHeaders）+ `redactURL`（userinfo + `sensitiveQueryParams` 剥离）；`recorder.go` 响应头与 `forward_retry.go` 请求头改经掩码后记录、`upstreamURL` 入口 `redactURL`（§11/§17.1 更新）。(3) **管理探测出站（F-06）**：`apibase.Deps::ManagementClient(p)` 走 `internal/outbound.Policy`（`AllowPrivate: p.AllowPrivateNetwork`，DNS-rebinding 安全拨号 + 逐跳重定向重校验）——probe/combos/providers 管理调用统一受控（§4.4 更新）；`fetchProviderModels` 8MiB 有界读取（`maxModelsResponseBytes`，超限受控 502）。(4) **`/v1` 鉴权决策（F-12）**：`/v1/*` 继续位于 `AuthMiddleware` 之外、无应用层认证——显式兼容决策：它是本地代理入口，不代表管理认证，且无配置修改能力；文档同步于 §3。(5) **F-18/F-21 相关**：proxy 依赖的 `registry.Registry::Config()` 现为深拷贝、state key 为长度前缀编码（见 config-registry-state-architecture.md）。
...
> **最后核对（2026-08-10，本地日志可观测性）：** Recent Requests 与 Trace 对所有来源始终捕获完整请求/响应体、Header、上游 URL、重试决策与 provenance；`internal/logredact` 仅把实际凭证值替换为固定 `******`，普通 Header、Cookie、URL 参数和诊断字段不再被整体隐藏。Trace API 不再使用字段白名单，保留磁盘记录中的全部字段；保留流式分页/取消/资源边界，超限以 `truncated` 明确标记而不静默丢失。
...
- `SSELineBuffer`（stream.go:15-40，类型定义与预算实现在 `internal/sse`）按换行符切分 SSE 行，跨块缓冲剩余部分，`Remaining()` 返回未换行尾部。**2026-08-09（audit F-14）：** `NewSSELineBuffer(maxLine, maxTotal)` 预算化——单行超过默认 1MiB → `ErrLineTooLong`、总缓冲超过默认 8MiB → `ErrBufferOverflow`；`streamResponse` 用预算化 buffer，`Feed` 出错即受控中断（`SSE stream exceeded line buffer budget`），不再无界累积单行。
...
**2026-08-10 本地透明日志基线：** `recordUsage` 不再根据 debug/Trace 开关抛弃 Recent Requests 的 payload/header；请求体、响应体（含 base64）、请求/响应 Header、上游 URL、`Decision` 与 `Provenance` 均进入对应 ring/Trace。`internal/logredact` 只替换实际 Provider Key，替换格式固定为 `******`，不再显示末四位，也不再因 Header 名为 Cookie 或 CustomHeaders 而整体隐藏普通值。Trace Reader 使用动态 JSON 记录保留原始 schema 中的全部字段，读取旧文件时仅对 Header/URL 凭证做重掩码。
...
- `internal/proxy/entry_tracker.go`：EntryTracker（13-138，含 `SetTTFT` 74-82 / `UpdateTokens` 84-98，2026-07-21 新增；`SweepStale` 116-138，2026-07-23 新增）、MarshalEntryJSON/MarshalEntryJSONLight（115-135，2026-08-19 新增 Light 轻量序列化）。
...
- `internal/api/trace/register.go`：3 个 GET 路由（`/api/traces/dates`、`/api/traces/index`、`/api/traces/req/{reqID}`）+ `POST /api/traces/clear`，auth-gated，路径遍历防护（`sanitizePathParam` 拒绝 `/`、`\`、`..`、null 字节）。**2026-08-10：** 取消字段白名单，index/detail API 动态保留磁盘 Trace 的全部字段（含 `finalKey`/`finalKeyName`/`upstreamURL`/`upstreamURLBase`/未来字段）；Header/URL 读取时经 `internal/logredact` 仅替换凭证为 `******`。两遍流式分页、`newTraceScanner` 1MiB 行上限、`writeIndexEnvelope`/`writeReqEnvelope` 增量输出与 `truncated` 资源边界保留。
