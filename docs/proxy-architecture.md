...
| 修改日志/追踪脱敏（F-23） | `internal/proxy/request_log.go`（`sensitiveHeaderNames`/`sensitiveQueryParams`/`isSecretHeader`/`maskHeaderMap`/`isCustomSecretHeader`/`redactURL`）+ `recorder.go`（响应头掩码 + `upstreamURL` redact）+ `forward_retry.go`（请求头掩码）；改敏感头集合必须同步 `maskHeaderMap` 调用点与 `redactURL` 调用点 |
...
- `internal/api/trace/register.go`：3 个 GET 路由（`/api/traces/dates`、`/api/traces/index`、`/api/traces/req/{reqID}`）+ `POST /api/traces/clear`，auth-gated，路径遍历防护（`sanitizePathParam` 拒绝 `/`、`\`、`..`、null 字节）。**2026-08-09（audit F-22）：** 查询返回 **DTO 投影**（`traceIndexDTO`/`traceDetailDTO`，排除 `finalKey`/`finalKeyName`/`upstreamURL`/`upstreamURLBase`）+ 幂等重掩码（`maskHeaderMap`/`maskHeaderValue`/`maskToken`）+ 两遍流式分页（`countIndexMatches`/`collectIndexPage`/`collectDetailPage`，`newTraceScanner` 1MiB 行上限，`writeIndexEnvelope`/`writeReqEnvelope` 增量输出）——不再整文件加载内存；上限 `maxReqDetailLines` 1000 / `maxTraceResponseBytes` 16MiB / `maxQFilterLen` 256 / `maxReqIDLen` 256，超限 `"truncated":true`。
