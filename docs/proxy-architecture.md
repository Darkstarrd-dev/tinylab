...
>
> **最后核对：** 2026-07-28，仓库工作区（`main`）。**P0/P1/P4 修复**：`recorder.go` reqBody 截断后新增 `json.Valid` 校验防止存储破碎 JSON（对齐 respBody 逻辑）；`forward_retry.go` `processingEntry` 新增 `SessionKey` 字段使 in-flight / `request-start` 条目携带会话键（修复"按会话分组"在请求完成前不生效）；`handler.go` `RequestUpdates` Broadcaster 缓冲区由 64 扩至 256 防止高并发 SSE chunk 洪泛下丢弃 `request-done` 事件。**payload 捕获由 debugMode 门控**（2026-07-28 撤销 2026-07-26 的非 playground 解耦，详见 §11.1）：`captureDetails := isPlayground || h.debugMode()`——playground 始终捕获；非 playground 仅 debug 开时捕获 payload/headers，debug 关时 Recent Requests 仅保留基本字段（时间/服务商/模型/密钥/延迟/Tokens）。**两层 JSONL 追踪日志**（见 2026-07-27 更新条目）：`recordUsage` 新增 `decision string`、`provenance string` 参数；`writeRequestLog` hook 在 body 截断前调用（保证完整 body 被捕获），受 `h.logRequests()` 运行时原子开关（加载自持久化 `cfg.Trace.Enabled`，默认 `false`）控制；追踪日志采用两层 JSONL 格式（`traces/index-YYYYMMDD.jsonl` 每日轮转 + `traces/req/<reqID>.jsonl` 追加）；…
...
- **详情捕获（由 debugMode 门控，2026-07-28 恢复）：** `captureDetails := isPlayground || h.debugMode()`——playground 来源始终捕获 `ReqPayload`/`RespPayload`/`RespHeaders`/`RespStatus`/`ReqHeaders`/`UpstreamURL`；非 playground 仅在 debug 开时捕获，debug 关时 Recent Requests 仅保留基本字段（时间/服务商/模型/密钥/延迟/Tokens），不存 payload/headers。`reqBody` 截断 64KB 后经 `json.Valid` 校验，`respBody` 截断 512KB（含 `data[].b64_json` 占位符替换）后经 `json.Valid` 校验，二者非法 JSON 均包装为 `{"raw":...}`（recorder.go:49-59、78-85）。内存开销由 ring 容量（`config.UsageRingSize`，默认 500）× 单条 body 大小封顶，仅 debug 开（非 playground）时产生 payload 内存开销。`debugMode()` 同时门控存储捕获（非 playground）与 `stream.go` 的 `parseAndBroadcastChunk`（SSE debug 控制台实时推理面板广播，见 §8）。
