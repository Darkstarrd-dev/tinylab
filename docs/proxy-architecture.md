...
>
> **最后核对：** 2026-07-25，仓库工作区（`main`）。本轮新增：**(h) Ollama provider 特例**——`internal/proxy/upstream.go` 新增 `isOllamaBaseURL`（纯字符串匹配：host 为 `ollama.com`，或 host 为 `localhost`/`127.0.0.1` 且端口为 `11434`；无网络探测，本地服务可能未运行）与 `normalizeOllamaBaseURL`（清空整个 path/query/fragment，把 BaseURL 降为 host root）；`BuildUpstreamURL` 在 Raw 模式之后、归一化之前新增 Ollama 分支，命中时调 `normalizeOllamaBaseURL` 后落入 host-root 分支统一注入 `/v1`。设计动机：Ollama 同时提供原生 `/api/*`（非 OpenAI 格式，响应/SSE/工具调用结构均不同，需完整格式转换）与 OpenAI 兼容 `/v1/*`（标准 OpenAI 格式，零转换），本项目"不做格式转换、SSE 原样透传"原则决定走 `/v1/*`；无论用户填什么路径（`/api`、`/api/tags`、`/api/chat`、`/v1`、`/v1/chat/completions`）都归一到 `/v1/*`。覆盖全部 11 个 `BuildUpstreamURL` 调用点（proxy/admin/probe/combo-speedtest），无需逐点改造。详见 §7.2。本轮还含：(g) **Embedding 模型支持**——`internal/proxy/handler.go` 新增 `Embeddings` handler（`POST /v1/embeddings`，复用 `handleProxy`），`internal/api/router.go` 注册路由（router.go:232）；`internal/config/types.go` 新增 `ProtocolOpenAIEmbedding = "openai-embedding"` 协议常量；`internal/api/probe_common.go` 新增 `probeOpenAIEmbedding` 单协议探测函数（输入为有意义的完整句子 `probeEmbeddingTestInput`，非 chat 提示词）与 `extractEmbeddingDim` 响应验证器（解析 `data[0].embedding` 数组并提取维度），`ProbeResult` 新增 `EmbeddingDim int` 字段；`internal/api/probe_model.go` 的 `testProviderModelProto` handler 支持 `openai-embedding` 协议，`probeResultToMap` 输出 `embeddingDim`；前端 `web/static/providers.js` 的 `testModelProtosSerial` 按模型 `kind` 过滤测试协议（`kind=text` → O/R/A，`kind=embedding` → E），徽章 title 与 Info modal 显示 embedding 维度（如 `dim:1536`）；`web/static/i18n.js` 新增 `embeddingModel`/`protoOpenAIEmbedding` 翻译键。详见 §1、§3、§3.3、§17、§18。
>
...
- `normalizeBaseURL`（upstream.go:48-73）：按**最长优先**顺序剥除已知 endpoint 后缀（`/v1/chat/completions`、`/v1/images/generations`、`/chat/completions`、`/v1/responses`、`/v1/completions`、`/v1/messages`、`/v1/models`、`/images/generations`、`/completions`、`/responses`、`/messages`、`/models`），命中即 break。**不含**尾部版本段（如 `/v1`、`/v1beta`、`/v2`）的剥离——版本段检测由 `BuildUpstreamURL` 的启发式 A 处理。
