...
>
> **最后核对：** 2026-07-27，仓库工作区（`main`）。**P0/P1/P4 修复**：`recorder.go` reqBody 截断后新增 `json.Valid` 校验防止存储破碎 JSON（对齐 respBody 逻辑）；`forward_retry.go` `processingEntry` 新增 `SessionKey` 字段使 in-flight / `request-start` 条目携带会话键（修复"按会话分组"在请求完成前不生效）；`handler.go` `RequestUpdates` Broadcaster 缓冲区由 64 扩至 256 防止高并发 SSE chunk 洪泛下丢弃 `request-done` 事件。**payload 捕获与 debugMode 解耦**（见 2026-07-26 更新条目）：`captureDetails` 恒为 `true`，Recent Requests 始终捕获 payload/headers，在任意 debugMode 状态下可查看。**两层 JSONL 追踪日志**（见 2026-07-27 更新条目）：`recordUsage` 新增 `decision string`、`provenance string` 参数；`writeRequestLog` hook 在 body 截断前调用（保证完整 body 被捕获），受 `h.logRequests()` 运行时原子开关（加载自持久化 `cfg.Trace.Enabled`，默认 `true`）控制；追踪日志采用两层 JSONL 格式（`traces/index-YYYYMMDD.jsonl` 每日轮转 + `traces/req/<reqID>.jsonl` 追加）；`TraceMgmtCall` 捕获 ManagementClient 路径（probe/speedtest/providers-probe）；`SweepTraces` 后台保留清理（每小时，按 retainDays + maxDiskMB）；`TracesDir()` getter；`X-TinyRouter-Provenance` 头由内部调用者设置；覆盖范围包括所有 LLM 流量（直接 /v1/*、playground、textreview in-process、gallery in-process、ManagementClient 路径）。**会话连续性推理**（见 2026-07-26 新增条目）。
...
> **2026-07-27 更新（新增状态模型）：** `TraceConfig` 结构体（`internal/config/types.go`）定义持久化追踪配置：`Enabled bool`（默认 `true`）、`RetainDays int`（默认 `2`）、`MaxDiskMB int`（默认 `500`）；`Config.Trace` 字段（yaml/json `trace`）在 `DefaultConfig()` 中初始化。`traceLine` 结构体（`request_log.go:20-52`）定义两层 JSONL 行 schema：`Type`（`"index"`/`"request"`/`"attempt"`）、`ReqID`、`Timestamp`、`Provider`、`Model`、`KeyID`、`Status`、`LatencyMs`、`TTFTMs`、`InputTokens`、`OutputTokens`、`Error`、`Decision`（重试状态机决策）、`Provenance`（`X-TinyRouter-Provenance` 头值）、`Body`（截断后 body）。`TraceMgmtCall` 方法（`request_log.go`）捕获 ManagementClient 路径的调用，attempt n=1，decision="management probe"。`SweepTraces`/`sweepTracesOnce` 后台保留清理（每小时，按 retainDays + maxDiskMB 限制），在 `app.go:191` 以 `go a.proxyHandler.SweepTraces(a.shutdownCtx, cfg.Trace.RetainDays, cfg.Trace.MaxDiskMB)` 启动。`TracesDir() string` getter 返回 `h.requestLogDir`。
...
> **2026-07-27 更新（两层 JSONL 追踪日志，行为新增）：** `recordUsage` 新增 `decision string`、`provenance string` 参数（写入 `usage.Entry`）；`writeRequestLog` hook 在 body 截断前调用（保证完整 body 被捕获），受 `h.logRequests()` 运行时原子开关（加载自持久化 `cfg.Trace.Enabled`，默认 `true`）控制。追踪日志采用两层 JSONL 格式：`writeRequestLog` 写入 `traces/index-YYYYMMDD.jsonl`（每日轮转，同 reqID 末次写入覆盖，`traceLine` 结构体为 JSONL 行 schema）+ `traces/req/<reqID>.jsonl`（追加，仅首次调用写 request 行，后续每次调用写 attempt 行）。`TraceMgmtCall` 方法捕获 ManagementClient 路径（probe/combo-speedtest/providers-probe）的调用，attempt n=1，decision="management probe"。`SweepTraces(ctx, retainDays, maxDiskMB)` + `sweepTracesOnce` 后台保留清理（按 modifyTime/文件名日期删除过期 index+req 文件，按 MaxDiskMB 删除最旧 req 文件），在 `app.go:191` 以 `go a.proxyHandler.SweepTraces(a.shutdownCtx, cfg.Trace.RetainDays, cfg.Trace.MaxDiskMB)` 启动，立即执行一次后每小时运行，shutdown ctx 停止；`TracesDir() string` getter 返回 `h.requestLogDir`。敏感请求头（`Authorization`/`X-Api-Key`）遮蔽为 `***`+末 4 字符（保留 scheme 前缀），base64 图像数据（data URLs、`b64_json`、Anthropic `{type:base64,data:}`）剥离。覆盖范围：所有 LLM 流量——`/v1/*` 直接（recordUsage）、playground（同路径 + `X-TinyRouter-Source: playground`）、textreview in-process（`proxy.Handler`）、gallery review + promptgen（in-process）、ManagementClient 路径（probe/speedtest/providers-probe 通过 `TraceMgmtCall`）。`X-TinyRouter-Provenance` 头由内部调用者设置（textreview: `textreview:clean:node=<id>` / `textreview:cleanbatch:node=<id>:chapters=<N>`；gallery: `gallery:review:session=<sid>:image=<idx>` / `gallery:promptgen`；probe/combos/providers 通过 `TraceMgmtCall` provenance 参数如 `probe:singlekey:...` / `probe:combo:...` / `probe:upstream:...` / `probe:models:...`；直接客户端 absent → source "client"）。
...
- **详情捕获（始终，与 debugMode 解耦，2026-07-26）：** `captureDetails` 现恒为 `true`，对所有来源（含非 playground）始终捕获 `ReqPayload`/`RespPayload`/`RespHeaders`/`RespStatus`/`ReqHeaders`/`UpstreamURL`，使 Recent Requests 在任意 debugMode 状态下都可查看。`reqBody` 截断 64KB 后经 `json.Valid` 校验，`respBody` 截断 512KB（含 `data[].b64_json` 占位符替换）后经 `json.Valid` 校验，二者非法 JSON 均包装为 `{"raw":...}`（recorder.go:49-59、78-85）。内存开销由 ring 容量（`config.UsageRingSize`，默认 500）× 单条 body 大小封顶。`debugMode()` 不再用于门控存储捕获，但仍门控 `stream.go` 的 `parseAndBroadcastChunk`（SSE debug 控制台实时推理面板广播，见 §8）。
...
- `internal/config/defaults.go`：`DefaultConfig()` 中 `Trace` 默认值（Enabled=true, RetainDays=2, MaxDiskMB=500）；
