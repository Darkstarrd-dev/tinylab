...
| Demo 页集成 | `web/static/app-demo.js`（`DEMO_TOOLS=[ademo,tilemap,design]` 下拉：`demoActiveTool/demoMenuOpen` 状态机、`renderDemoWithMenu` 按需渲染（ademo=Assistant Demo、tilemap=TileMap Editor、design=Game Designer）、`F6→toggleDemoMenu`）、`web/static/app-router.js`（`case ademo/tilemap/design→renderDemoWithMenu`、离开清理 `cleanupGameDesigner`）、`web/static/auth.js`（`#demo-menu` 三件套：click/mousemove/keydown）、`web/static/style-editor.css`（`.dgn-root` 作用域、`.dgn-stage/.dgn-status/.dgn-preview-toolbar/.dgn-tab/.dgn-console`）、`web/static/i18n.js`（`design/designer*` en+cn + `designerExampleUnits/designerExampleSeeding/designerExampleSeeded/designerExampleExists/designerPreview/designerConsole/designerConsoleClear/designerConsoleEmpty`）、两个 index 变体 `.demo-nav-wrap#demo-menu`+`demo-designer.js` 脚本 | Demo 导航与 Utility/Gallery 同构：点击 toggle + 外部点击关闭 + Esc 关闭 + 页内再次触发 toggle（hover 已移除）；`localStorage.demoActi…
| Game Designer | `web/static/demo-designer.js`（独立 Demo 工具 `GameDesigner.render/cleanup`：复用 `EditorLayout` 工厂（tree/input/gutter/status）+ `?root=games` 的 editor 后端（tree/open/save/delete 均 scope 到 games 目录）+ Phaser 预览（Blob 注入 `TRGames.register` 的 entry → `__dgames.loadPhaser/injectScript/makeHost` → `launch`）+ 右侧 Preview/Console 切换（`.dgn-preview-toolbar`+`.dgn-console[role=log]`+`.dgn-stage.is-hidden/.dgn-console.is-visible`，`localStorage.designerConsoleVisible`，`console.*`/`onerror`/`unhandledrejection`/`launch` 抛错含 stack，`DGN_CONSOLE_MAX=500/TRUNCATE=4000`）；经 Demo 下拉 `design` 项渲染；**与 collapse explorer 同列**的 `Example Unit` 按钮（`.dgn-example-unit`，svg+tooltip `designerExampleUnits`，按需 `POST /api/games/seed` 后 `dgnLoadTree`）） | games 作用域的文件编辑（新建项目=`game.json{ id=目录名, entry: main.js}`+`main.js` 模板→`TRGames.register/Phaser.Game`、`Ctrl+S`、新建文件、删除项目/文件）+ 右侧 Phaser 实时预览（`Run/Reload` Blob 热注入，`Stop` 销毁）+ Console（`console.*`/`window.onerror`/`unhandledrejection`/`launch` 抛错/`injectScript` 失败含 stack，`DGN_CONSOLE_MAX=500/TRUNCATE=4000`，`localStorage.designerConsoleVisible` 持久，`Clear` 清空）；藏 markdown 控件（`.dgn-root`）、显 `JS`/`Saved/Preview error` 状态；工具内增删改经 `GET /api/games` 立即出现在 `Demo→Games` 下拉。示例：Designer 内点 `Example Unit`（与 collapse explorer 同列）按需创建 Unit01~06，已存在的跳过；用户可在 Explorer 删除已建示例以腾出 Demo Games 下拉 |
...
- **Designer 缝（2026-09-01 Console）**：`#ed-main-preview` 内新增 `.dgn-preview-toolbar`（两枚 `role=tab .dgn-tab[data-tab=preview|console]` + `Clear`）与 `.dgn-console[role=log]`；可见性由 `.dgn-stage.is-hidden` / `.dgn-console.is-visible` 控制，`dgnSetConsoleVisible` 读写 `localStorage.designerConsoleVisible` 并切 `is-active/has-error`。管线 `dgn.consoleLines[]` + `dgnConsoleFormat/Push/Render/SetVisible/Clear` + `dgnInstall/UninstallConsoleCapture`（覆写 `console.log/info/warn/error/debug` + 监听 `error`/`unhandledrejection`，`cleanup` 时还原；`dgnPreviewRun` 全链路 `dgnConsolePush('error',[stack||message])`，`▶ Run` 入 `info`）。webview 默认 `Debug:false` 无 DevTools 时为唯一排障通道（`host_webview_windows.go: webview2.NewWithOptions{Debug:false}`）。
...
- **webview DevTools 边界（2026-09-01）**：独立窗口 `webview2.NewWithOptions{Debug:false}` 默认关闭 DevTools 且不开放 F11（见 `host_webview_windows.go`）；Design 右侧 Console 为唯一排障通道，不改 host DevTools 开关。
...
- 2026-09-01 Design Console（`web/static/demo-designer.js` console 管线 + `style-editor.css` toolbar/console + `i18n.js` 新键）：`dgnConsoleFormat/Push` 多参/对象/Error/missing 歧义消解与 `DGN_CONSOLE_TRUNCATE=4000`，`error/warn` 时 `Console` tab 打 `has-error`，`render` 时 `localStorage.designerConsoleVisible` 恢复与空列表占位 `designerConsoleEmpty`，`Run/Reload` 注入失败与 `launch` 抛错含 stack 落 Console 且保留 `dgnStatus`，`Clear` 清空并 `render`，`cleanup` 卸载 capture；`web/demo-designer.test.js` 预留扩展、`web/assistant-demo.test.js`/`web/demo-games.test.js` 回归绿。
...
| Game Designer Console（Preview/Console 切换） | §1 Demo 页集成/§5 Designer 缝/§7 边界/§8 验证、本表 | `web/static/demo-designer.js`（`dgn.consoleLines/consoleVisible/origConsole/consoleEl` + `DGN_CONSOLE_MAX/TRUNCATE` + `dgnConsoleFormat/Push/Render/SetVisible/Clear/Install/UninstallCapture` + `render()` 的 `#ed-main-preview` 工具栏+console DOM+`localStorage.designerConsoleVisible`+`dgnPreviewRun` 全链路 `dgnConsolePush` + `cleanup` 卸载）、`web/static/style-editor.css`（`.dgn-preview-toolbar/.dgn-tab/.dgn-console*` 与全屏规则）、`web/static/i18n.js`（`designerPreview/Console/ConsoleClear/ConsoleEmpty` en/zh） |
