...
> **最后核对（2026-09-09，Gallery Tree Batch Convert 移除 Compress 与增加 Fit to 限制长边）：** (1) **移除 Compress 选项**：`gallery-edit-batch.js`（`openTreeBatchConvert`）移除 Compress 开关行与相关变量绑定，Tree Batch Convert 统一采用原地处理模式（`overwrite: true`，同格式原地覆盖，跨格式同目录下新增并保留源文件）；格式选择器前新增清晰的 Format 标签并与 Quality 滑块两端对称排布。(2) **新增 Fit to（限制长边）Toggle 开关与 Stepper 步进输入框**：Scale 右侧新增 `.toggle-switch` 形式的 Fit to 开关（默认关闭）与带加减号的 `.number-stepper` 自定义步进输入框（`_treeBatchFitStepperHtml`，默认 1920，步长 64）；开关开启时 Scale 区域半透明置灰且禁止交互（`scaleInput.disabled = true`），Scale 设置无效，并将 Fit to 数值传递至后端；关闭时 Stepper 置灰且输入禁用，恢复 Scale 滑块控制。(3) **后端保持比例限制长边转码**：`internal/mediaedit/types.go` 在 `ImageTranscodeParams` 中新增 `FitTo int` 字段；`internal/mediaedit/args.go` 在 `FitTo > 0` 时优先生成 `scale='if(gte(iw,ih),target,-2)':'if(gte(iw,ih),-2,target)'` 滤镜表达式并自动规整目标尺寸为偶数，忽略 `ScalePercent`，确保横图、竖图或方图均等比将长边缩到目标像素且短边维持偶数比例；`args_test.go` 补全单测覆盖。(4) **国际化与验证**：`pg-i18n.js` 新增 `geFitTo`/`geFitToTip` 中英文翻译；全量 Go 测试 64 包、Node/Bun 契约测试 `gallery-edit-contract.test.js` 与前端 JS 语法全绿。
> **最后核对（2026-09-09，Gallery Batch Convert 自定义控件、Modal 高度增大与跨格式仅保存不删源文件）：** (1) **自定义 UI 与 Modal 窗高度增大防剪裁**：`gallery-edit-batch.js`（`_treeBatchFormatSelectHtml`）将原生 `<select>` 替换为全局 `renderCustomSelectHtml`，Compress 复选框改为 `.toggle-switch` + `.toggle-slider` 自定义滑动开关；弹窗 body 设置 `min-height: 300px`，模态窗应用 `width: 520px; min-width: 520px; min-height: 460px`，`playground.css` 新增 `.gallery-edit-row` 下拉专用样式并将菜单 `max-height` 提至 220px，彻底根除初始状态下下拉菜单被 modal 窗底部剪裁问题。(2) **跨格式仅保存不删源文件，仅同格式才覆盖替换**：`internal/mediaedit/manager.go` 彻底移除 `removeOnSuccess` 逻辑，跨格式转换（如 `.png` → `.webp`）在源文件同目录下保存新文件且严格保留源文件不删除；仅在目标格式与源格式一致（如 `png` → `png` 压缩/缩放）时通过 temp+rename 原位覆盖原文件；`edit_handlers.go` 覆盖模式直接返回 `overwritten: true`。(3) **画廊列表与完成态定位联动**：`_refreshTreeBatchOutputs` 在同格式覆盖时刷新当前项缩略图，在跨格式转换时保留源文件条目并将新格式文件作为新 item 追加到 `galleryState.items` 中，同步刷新缩略图与目录树；`_onBatchComplete` 的 `openRel` 绑定新生成文件相对路径供 Open Folder 精确定位。(4) **测试与验证**：`manager_test.go` 与 `edit_handlers_test.go` 单测断言跨格式转码后源文件（`source.png`）与目标文件（`source.webp`）均存在，新增同格式覆盖测试全绿，`node --check` 语法全绿。
> **最后核对（2026-09-07，删除后 Playground 残留模型清理）：** `pg-state.js::pgLoadModels` 在 `/api/models` 拉取后新增 `pgPruneStaleModels()`——用 live 模型 id 集合校验全部窗口（含 `modeWindows` 停车窗口）`config.model`/`config.imgPromptModel`、director `directorModel`/`narratorModel`、`pgSetupState.model`，失效清 `''`（`__comfyui__` 本地占位保留），变更后 `pgSave()`+`pgSaveAutoChat()` 回写并重渲染 sidebar/panes/inputbar + warning toast（`pg-i18n.js` 新增 `pgStaleModelCleared` en+cn）。此前 provider/model/combo 删除后 playground 侧边栏与 pane 仍显示已删名，发送才失败。
> **最后核对（2026-09-07，Gallery Tree 顶部 Batch Convert 原地批量）：** image 左侧 Tree 标准 Header 新增左对齐 `Batch Convert` 按钮（`gallery-tree.js` + `openTreeBatchConvert` 入口）；`gallery-edit-batch.js` 新增瘦身弹窗（仅 Compress/格式/画质/缩放）+ `_treeBatchTargets`（仅 backend+grantId 可写，余者跳过计数）+ `_refreshTreeBatchOutputs`（跨格式同步 rel/path/name 并刷新视图）；`_startBatch` 透传 `dest.overwrite`（覆盖时不送 outputName）；`_pollBatchJob` 记录 `overwritten`；`_onBatchComplete` Open Folder 闭包捕获 grant 回退；`_checkFfmpegStatus`/`_show/_hideProgressSection` 兼容 tree Start 按钮禁用；i18n 新增 `geTreeBatch*` 4 键（en+zh）。后端零改动（复用 overwrite 原地通道）。
> **最后核对（2026-09-01，Playground Search 思考流式解析 + 加载状态修复 + 跨模式存储隔离）：** (1) **思考与流式解析修复**：`pg-search.js`（`pgSearchApplyChunk`/`pgSearchFlushRender`/`pgSearchFinish`）新增对 `delta.reasoning_content`/`thinking`/`thought`、`<think>` 标签提取与 citations/sources 的完整收集与时序追踪，同步至 `windows[1]`；`pg-render.js` 右侧 Synthesized 窗口在 `(loading || streaming) && !content && !reasoning` 时展示加载动画，解决流式期间窗口显示空白标题无反馈的问题；(2) **跨模式存储与窗口隔离**：`pg-state.js` `pgLoad` 独立解析 `localStorage[PG_MSG_KEY]` 到 `modeWindows.normal` 并在 `pgSave`/`pgSaveSync` 严格限定 `curMode === 'normal'` 才写入 `PG_MSG_KEY`；`pg-ui.js` `pgSetMode` 切换时即时清除 `pgSaveTimer` 并在退出前同步持久化，彻底杜绝 Search 结果写入 Normal 模式的问题。
> **最后核对（2026-08-31，QuickSlot 切模型残留修复）：** (1) `pg-ui-events.js::pgApplyActiveQuickSlot` **放开 mode 守卫**——原 `if (pgState.mode !== 'normal' && pgState.mode !== 'search') return;` 删除，normal/search/**autochat**/image 全模式应用（AutoChat 群聊下先前是零窗更新而非单窗更新）；新增 `pgApplyActiveQuickSlotToAll(model)` 遍历 `pgState.windows` 全量 `w.config.model = model` + `pgSave()`，并在切换时 **abort 在途流**（`w.streaming && w.abortCtrl` → `abort + w.streaming=false`），避免 Console 长尾旧条目。(2) `pg-lifecycle.js::renderPlayground` 首屏应用同样放开守卫（AutoChat 刷新后槽模型生效，ToAll 语义但不 abort——首屏无在途）。(3) `web/static/quickslots.js::_qsModalSelectFocused` 由 `apiGet→改→apiPut` 全量 PUT 改为 **`PATCH /api/quickslots/{id}/selectedIndex` 原子接口**（`registry/quickslots.go::SetQuickSlotSelectedIndex` 锁内设置+越界钳制），以 PUT 成功返回体回填 `pgApplyAct…
>
> **最后核对（2026-08-29，Round-2 P1-01 searchHistory与Google非流式）：** `pg-state` `pgSyncSearchMessages` 浅拷贝隔离 `searchHistory.messages`；`pg-stream` `pgSendNonStream` 按 `pgGetTextProtocol` 选择 `/v1beta/models/:generateContent`；`pg-image-tasks` `effectiveConcurrency(winIndex)` 按任务窗口计并发。

# TinyLab Playground 架构
> **最后核对（2026-08-28，Normal 模式新增 Reasoning Effort 下拉）：** Playground Normal（OpenAI 兼容）模式 Parameters 面板新增 **Reasoning Effort** 下拉（`pg-ui.js` `pgRenderSidebar` else 分支），选项为 omp 同款全档位 `Off / Minimal / Low / Medium / High / XHigh / Max`，默认值 `medium`、开关默认关（`pg-core.js` `PG_DEFAULT_CFG.reasoningEffort` + `PG_DEFAULT_PARAMS.reasoningEffort`）。开关开启后 `pgBuildBodyForWin`（`pg-request.js`）在请求体写入 `reasoning_effort: "<wire>"`，经 `PG_REASONING_EFFORT_WIRE` 映射（`pg-core.js`）：`Off→"none"`（wire 禁用值，同 omp `none-effort` 语义）、`minimal→"minimal"`、`low→"low"`、`medium→"medium"`、`high→"high"`、`xhigh→"xhigh"`、`max→"max"`；开关关闭时省略字段。仅 OpenAI 兼容分支渲染，Google 分支仍用 `thinkingLevel`。i18n 新增 `pgReasoningEffort`/`pgReasoningEffortHint`/`pgReasoningOff`/`pgReasoningXHigh`/`pgReasoningMax`（en+cn）。headless Chrome 隔离实例（port 20199）驱动验证：7 档 wire 值逐一断言通过、toggle off 省略字段、Preview Request 面板同步显示。
> **最后核对（2026-08-28，Text Review Step3 预设提示词替换 + 节点池批处理移除）：** (1) `internal/api/textreview/register.go` 内置默认清理 system prompt（`defaultCleanSystemPrompt`）整体替换为「誊抄+删字」新版（新增「核心操作方式（最重要）」/「删字要外科手术式」/「最后再强调一遍」小节；删除旧「输出格式（多章分段）」小节——批处理已移除，每个请求只含一章，输出无需 `===CHAPTER_ID:`/`<<<|||CHAPTER_SEP|||>>>` 标记）。(2) **移除多章批处理**：`config.TextReviewNode.BatchChars` 字段删除（旧 config.yaml 的 `batchChars` 由 yaml.v3 静默忽略）；`internal/textreview/scheduler.go` `runBatch`→`runChapter`（每次请求恰一章）、`dequeueBatch`/`nextBatchLocked`→纯函数 `nextPendingIdx` + `nextChapterLocked`（认领范围内第一个 pending 章）；`cleaner.go` 删除 `BatchCleaner`/`RawBatchCleaner`/`ProgressiveBatchCleaner` 接口与 `ChapterSep`/`chapterIDHeader`/`BatchChapter`；`proxy_call.go` 删除 `CleanBatch*`/`callBatch`/`buildBatchContent`/`buildBatchRequestBod…
> **最后核对（2026-08-27，Gallery 分屏 Clear 按钮按所属 pane 定向清空）：** 修复 split 模式下点击未获焦 pane 底部垃圾桶「点击无效/清错侧」：`gallery-layout.js::bindEventsForCurrentLayout` 的 clear handler `stopPropagation()` 阻断了 pane click 监听器同步 `galleryState.focus`，而 `clearActiveSideTree()` 按逻辑聚焦侧清空 → 未聚焦侧的按钮实际清了另一侧（该侧为空时表现为点击无效）。(1) `gallery-tree.js::clearActiveSideTree(forceVid)` 新增可选参：boolean 显式指定 pane，undefined 保持原语义（键盘 `gallery.clear-tree` 快捷键与目录树面板按钮不受影响）；(2) 两个垃圾桶 handler 经 `btn.closest('.gallery-pane').id === 'gallery-pane-video'` 判定按钮所属 pane 并显式传参。
> **最后核对（2026-08-26，Text Review 默认清理提示词补回分段输出格式 + i18n 键恢复 + Reasoning 开关 wire 修复）：** (1) `internal/api/textreview/register.go` 的内置默认清理 system prompt（`defaultCleanSystemPrompt`）在保留原 v2 novelhelper 内容的基础上，追加「输出格式（多章分段）」小节：多章输入时按输入顺序逐章分段输出、每段以 `===CHAPTER_ID:章节序号===` 开头、段间用 `<<<|||CHAPTER_SEP|||>>>` 分隔、段数与输入章数完全一致不得合并/遗漏/调换；单章时直接输出不加标记。对齐 novelhelper 批处理协议（`docs/M1_text_cleaning.md §3.3`：系统提示词附严格指令）与 `proxy_call.go` 既有 `batchInstruction`，解决模型未按章节标记分段输出的问题。(2) `web/static/i18n.js` 补回 `trSystemPrompt`/`trSystemPromptPlaceholder`（en+cn，b364f22 加 `trNodeReasoning` 时从该文件移除、仅存于 pg-i18n.js，导致 Utility Text Review 页 `trT('trSystemPrompt')` 走 `web/static/i18n.js` 的 `t()` 查不到 key 而回退显示原始键名）——en「System Prompt」/cn「系统提示词」恢复。(3) `internal/textreview/proxy_call.go` Node Pool Reasoning 开关 wire 修复：原实现 Reasoning=false 时不发送任何思考参数，llama.cpp 等 OpenAI-compat 服务按模型模板默认开启思考（实测 Ornith-1.5-9B 无参请求返回 226 字 reasoning_content）——现改为显式双分支 `reasoning_effort: "medium"`（开）/ `reasoning_effort: "none"`（关，对应 OMP reasoningDisableMode=none-effort，llama.cpp 实测 `enable_thinking`/`chat_template_kwargs` 均被忽略），并移除无效的顶层 `enable_thinking`；`buildRequestBody` 与 `buildBatchRequestBody` 同步，新增 `internal/textreview/proxy_call_test.go` 锁定 wire 契约；端到端验证（临时 playground 构建 + 临时配置指向 127.0.0.1:8080 llama.cpp）两节点会话 completed，DebugRequest 分别携带 "none"/"medium"。
> **最后核对（2026-08-26，Inspire 弹窗与任务队列六项修复）：** (1) `pg-ui.js` `pgSetMode` 新建模式窗口时以切出前 `windows[0].config` 深拷贝播种（此前 image 模式窗口每次进入均为全新 `makeWin()`，`imgInspirePresets`/`imgPromptModel`/`imgSubmitCount` 等 image 配置重启即丢，为 Inspire 预设空白根因）；(2) `pgOnImageSubmitCount` 同步所有 `input.pg-image-submit-count` 显示（修复 Inspire 弹窗批量数量步进 +/- 点击无反馈）；(3) `pg-image-inspire.js` `render()` 新增 `batchDone` 参数：批量生成完成后 footer 提供 Apply 按钮（复用 `pgImageInspireApply` 回填 `#pg-input` + `config.prompt`）；(4) `playground.css`：`.pg-input-actions .pg-btn-row` 加 `flex-wrap:nowrap`（英文 "Inspire" min-content 超出 flex 份额换入隐藏第二行 → webview 端按钮消失）、新增 `.pg-btn-attach` 固定 36px、`.pg-btn-inspire` `min-width:0`+ellipsis、`.pg-inspire-input-wrap textarea` `max-height:none` 解除 `.pg-input` 160px 上限使弹窗随内容增高；(5) 任务队列移除/清空按钮追加 `btn-icon` 类，令 `style.css` `.btn-icon.bin-button` 纵向排列 + hover 旋转动画在 playground 页生效；(6) 新增 `web/pg-image-inspire.test.js`（VM 沙箱行为测试）；(7) **预设=提交给辅助模型的指令模板**：`BUILTIN_PRESETS` 内置 4 条指令（默认润色/多条生成/细节扩写/创意发挥），占位符 `{n}`/`{format}`/`{delimiter}`，用户列表空时回退内置、Edit 预填可改；点击=选中（`.pg-inspire-preset-item.active` 高亮，存 `pg-core.js` `imgInspirePreset`），Generate 经 `buildInstruction()` 替换占位符为 system 消息，选中模板无 `{delimiter}` 且批量>1 时自动追加多条/分隔符要求；i18n 标签 Preset Instructions/预设指令，空态分支与 `.pg-inspire-preset-empty` 样式已删除。
> **最后核对（2026-08-26，Task Queue 清除按钮与 Prompt Inspire 预设/批量提示词）：** (1) `pg-image-tasks.js` 新增 `pgTaskRemove`（单条移除，运行中即中止，已保存图片与窗口历史 generation 不受影响）与 `pgTaskClearAll`（清空全部条目并中止在途任务），队列行与 Task Queue 头部分别挂载行内/全局清除按钮，复用 download 页 Clear Completed 的动画垃圾桶 SVG（`binSvg()` 每实例唯一 mask id，`.pg-task-remove-btn`/`.pg-task-clear-all-btn`）；(2) `pgTaskEnqueue` 支持分隔符 `<<<PROMPT>>>`（`pgImageSplitPrompts`）多提示词输入：按提示词拆分为逐请求单元（每单元携带独立 `prompt`/`req`），`startUnit` 优先用单元级 prompt/req，totalExpected=提示词数；(3) `pg-image-inspire.js` 重构：新增预设提示词区（`w.config.imgInspirePresets`，点击回填、编辑模式每行一条保存）、批量数量步进器（联动右侧 `imgSubmitCount`，`pgGetImageSubmitCount`/`pgOnImageSubmitCount`/`pgStepImageSubmitCount`）、批量生成（system 指示返回 N 条以 `<<<PROMPT>>>` 分隔的结构化提示词，结果回填输入框，Apply 整体写回主输入框）、helper model 选择即写回 `imgPromptModel`、输入框随内容自动增高至窗口 80% 高度后滚动（`pgInspireAutosize`）；(4) `pg-core.js` 新增 `imgInspirePresets: []`；i18n en/cn 同步；测试 `web/pg-image-tasks.test.js` 新增分隔符拆分/移除/清空三用例。
> **最后核对（2026-08-26，Playground Image 模式：任务派发 + 异步队列 + 并发控制）：** (1) 新增 `pg-image-tasks.js` 任务管理模块（任务队列 `tasks[]`、FIFO 调度器 `pump()`、per-provider 在途计数 `providerRunning`、500ms 刷新计时器、`pgTaskEnqueue`/`pgTaskCancel`/`pgTaskSelect`/`pgTaskSelectedView`/`pgTasksSnapshot`/`pgTaskWindowBusy`/`pgRenderTaskQueue`）；(2) 重构 `pg-image-model.js`：请求构建 `pgImageBuildRequest`、单元规划 `pgImagePlanUnits`、单元执行 `pgImageExecUnit` 与统一资产归档 `finalizeImageAssets`，移除旧同步 `pgImageGenerate`/`pgImageStop`/`pgImageRunSequence`；(3) `pg-render.js` 引入 `pgImageViewSource` 优先渲染任务队列选中的任务结果，`pgImageSelectAsset` 经视图源读写；(4) `pg-core.js` 默认配置增加 `imgConcurrency: 1`，`pg-ui.js` 增加并发 Stepper 调节控件（1..8）并改造发送逻辑（generate 入队立即解锁输入框）、侧栏对半拆分渲染 Recent Requests 与 Task Queue、移除全局 stop 锁改为队列行内 stop；(5) `internal/feature/feature.go` 和 `feature_test.go` 注册 `pg-image-tasks.js`；(6) 样式 `playground.css` 添加 Task Queue 状态点、running 动画与行内样式；(7) 新增前端单测 `web/pg-image-tasks.test.js` 并适配全量契约测试。
...
> **最后核对（2026-08-26，Playground Image 渐进式多图批量生成与逐张即时查看）：** (1) `pg-image-model.js` 改造 `pgImageRunSequence` 增加 `onProgress` 回调，多图（`imgSubmitCount > 1`）每跑完一张立即将生成的 asset 推入 `generation.assets`、附加元数据并执行 `finalizeImageAssets` 自动保存，同时刷新画布，首张出图时自动选中；(2) `pg-image-model.js` 在 generation 记录 `totalExpected: count` 并在全部结束或取消/异常时清零；(3) `pg-render.js` 改造 `pgImageFlatAssets`，当 `generation.status === 'generating'` 且还有未完成张数时在末尾注入一个正在生成的虚拟占位项（`isPlaceholder: true`）；(4) `pgImageRenderCanvas` 在查看已完成图片时移除遮罩层 loading overlay 并开放翻页与操作按键，查看占位项时渲染仓鼠跑滚轮动画且禁用 next 按键，导航指示器支持 `(index + 1) / total` 动态反映进度；(5) 编写契约测试 `web/pg-image-progressive.test.js` 并通过全量回归。
> **最后核对（2026-08-26，SenseNova Image 协议支持与模型专属分辨率隔离）：** (1) `pg-core.js` 增加 `snWatermark`/`snPromptExtend` 默认字段；(2) `pg-i18n.js` 增加中英文翻译；(3) `pg-ui.js` 在 `pgImageProtocols()`、`protoLabels` 与 `pgRenderImageParams` 增加 `sensenova` 分支，且**按模型独立隔离分辨率下拉列表**（`sensenova-u1.5-lite` 支持 auto 与 2048x2048/2720x1536 等 2K/4K 分辨率；`sensenova-u1-fast` 仅支持专用的 11 种比例 2K 分辨率白名单，首选 2752x1536）；(4) `pg-modal.js`（`pgImgBuiltinSizesFor`）同步适配模型级默认尺寸；(5) `pg-request.js` 在 `pgBuildImageBody` 构建 SenseNova 字段并将参考图适配为 `images: [{image_url: "..."}]` 数组格式；(6) `pg-image-model.js` 在 `cfgParams`/`imageBody` 收集参数并在多图（`count > 1`）时按单张无 `n` 字段串行调度请求；(7) 测试通过 `web/pg-sensenova-protocol.test.js` 覆盖。
> **最后核对（2026-08-23，Gallery 渐进加载回归修复——runWithConcurrency 首波闭包丢失）：** 修复 94f4e31 渐进加载优化引入的回归：粘贴/拖动 24 包仅加载 13（后端路径，并发 12）或 19（FSAA blob 路径，并发 6）个包，且有一个包被重复处理 N 次。根因：`runWithConcurrency`（gallery-io.js）首波用 `var idx` 在 while 循环里捕获索引，`cap` 个微任务共享同一函数级绑定，全部读到循环结束后的 `idx=cap-1`——前 `cap-1` 个包静默丢失、第 `cap` 个包被并发处理 `cap` 次。优化前主路径为逐包串行 `await`，故为纯回归。修复：`var idx` → `let idx`（块级绑定逐迭代捕获）。`autoresearch/bench.mjs` 原有 `items_total` 断言无法拦截此 bug（重复任务数恰好抵消丢失任务数，总数不变）；已新增按输入派生路径前缀分组的 pack 完整性断言（distinct pack 数 === N_ZIPS 且每包条目数 === ENTRIES_PER_ZIP；不可按 sessionId 分组——mock 后端每次请求铸造新会话，重复包会伪装成新包）。A/B 验证：未修复代码 49/60 包触发断言失败；修复后 60/60 通过，ttfi/total 指标与基线持平。涉及文件：`web/playground/static-pg/gallery/gallery-io.js`（`runWithConcurrency`）、`autoresearch/bench.mjs`（`runOnce` pack 断言）。
> **最后核对（2026-08-22，Gallery 渐进加载：tree 先显 + 首图即显 + 压缩包流式展开；修复 zip 内动画条目主图空白）：** 修复 Gallery 粘贴/拖入大量内容（尤其多个 zip 包）时全部处理完才一次性显示、体感等待可达 10s+ 的问题。`gallery-io.js` 加载管线改为三阶段渐进：(1) `walkDir` 保持纯遍历——zip 不再 inline `addArchive` 上传，而是收集 `{kind:'zipfile',file,zipFileHandle}` 交调用方处理（顺带修复 `processHandles`→`processCollectedEntries` 路径上 fs/zipfile 条目被静默丢弃的隐患）；(2) 松散文件在 list-dir/分类完成后立即 `appendItems`/`appendVideoItems`——tree 与第一张主图即刻渲染；(3) 压缩包流式展开：`processZipFilesProgressive`（FSAA blob 包，并发 6 保持 LRU 上传保护）与 `processBackendZipsProgressive`（磁盘包 `zip-from-path`，**并发 12**——会话引用磁盘文件、被 LRU 驱逐后重读磁盘即可重建）逐包取 manifest；增量上屏经 **`queueProgressiveItems` 定时合并**（120ms 窗口，首包立即 flush、导入结束前强制 flush）避免逐包全量 strip/tree 重建。**主图空白修复（92cd29b 回归）**：`getVideoStreamURL` 的 grantId 直链分支补 `kind==='backend'` 守卫——zip 条目携带粘贴 grant 但无 `rel`，其 `.webp/.gif` 条目名命中 `isVideoExt` 后误走视频直链路径，`mainURL` 指向 grant 根（错误字节）→ 主图空白而缩略图正常；修复后回落 `getItemBlob` blob 渲染，真实 backend 视频直链不受影响。另修 `style.css` `.gallery-vol-slider-vert` 的 `appearance:slider-vertical` 弃用告警（改 `writing-mode:vertical-lr;direction:rtl`）。基准：60 包×30 图+20 散图 ttfi 11272ms→71ms、总耗时 11227ms→956ms、strip/tree 重建 61→6。涉及文件：`web/playground/static-pg/gallery/gallery-io.js`、`web/static/style.css`、`autoresearch.sh`、`autoresearch/bench.mjs`。
> **最后核对（2026-08-20，Playground ComfyUI 增加 Preset 预设与动态参数排序/移除）：** 在 Playground Image 模式下的 ComfyUI 协议侧边栏中引入 Preset 系统：(1) 将 Tab Select 标题行升级为 Tab/Preset 双视图切换按钮（`.pg-comfy-view-toggle` / `.pg-comfy-view-btn`，复用模式切换视觉风格）；(2) Preset 视图包含下拉选择器与 Add/Edit/Remove 三个操作按钮；(3) Add 弹窗输入预设名称，将当前 Tab 工作流与可见节点列表保存为独立 Preset（`localStorage` 键 `tinylab.playground.comfy.presets.v1`）；(4) Edit 弹窗支持 HTML5 拖拽排序与点击 ✕ 移除冗余参数节点，自定义 `visibleNodes` 有序过滤渲染；(5) Remove 删除当前选中 Preset 并切换回活动预设；(6) 动态参数表单在 Preset 模式下保持参数值实时可编辑，回退协议选择时同步重置视图状态。涉及文件：`web/playground/static-pg/playground/pg-comfyui.js`、`web/playground/static-pg/playground.css`、`web/playground/static-pg/playground/pg-i18n.js`。
...
> 3. **Prompt Helper 弹窗式模型选择器（Model Picker Modal）**：Playground Image 侧边栏的 Prompt Helper 模型选择器由原下拉菜单升级为与 Normal 模式一致的 `.pg-model-btn` 弹窗式选择器（`pgOpenModelPicker`），支持搜索过滤与非文本/非嵌入模型自动筛选。
> 涉及文件：`web/static/app.js`、`web/static/style.css`、`web/playground/static-pg/playground/pg-ui.js`、`pg-comfyui.js`、`pg-modal.js`、`pg-render.js`、`playground.css`。
> **最后核对（2026-08-18，Playground 多模态附件渲染、矢量 SVG 图标与横向并排布局整改）：** (1) **SVG 矢量图标体系**：在 `pg-core.js` 新增 `pgGetMediaType` 与 `pgGetMediaSvg`，为 PDF 文档、视频 (Video)、音频 (Audio)、图片 (Image) 提供专属高质量现代矢量 SVG 图标与主题色彩。(2) **待发送附件预览区整改**：在 `pg-ui.js` 的 `pgRenderInputThumbs` 增加媒体类型检测，为 PDF/视频/音频渲染专用徽标卡片（`.pg-input-thumb-file`, `.pg-input-thumb-pdf`, `.pg-input-thumb-video`, `.pg-input-thumb-audio`），彻底解决视频落入 `<img>` 标签导致显示为破损图片（`alt="media"`）及音频/PDF 简陋文本样式问题；为缩略图外层预留安全边距，解决 hover 移除按钮引起的卡片高度跳动。(3) **消息历史气泡渲染修复与横向并排**：在 `pg-render.js` 新增 `pgRenderMediaPart`，彻底解决发送后在历史气泡中将 PDF/视频/音频当成 `<img>` 渲染导致的破图问题；为非图片多模态附件生成结构考究、语义清晰的 `.pg-media-card` 交互卡片，支持悬停动效与点击预览；引入 `.pg-msg-media-layout` 将媒体附件水平放置在文字气泡左侧并保留 12px 间隔，消除顶部重叠。(4) **模态框全媒体预览支持**：在 `pg-modal.js` 将 `pgShowImageModal` 升级为 `pgShowMediaModal`，视频弹出带有全功能播放控制的 `<video controls autoplay>` 播放器，音频弹出带有声波/音符主题的 `<audio controls autoplay>` 播放器，PDF 弹出内嵌预览与“在新窗口打开”/“下载”工具。涉及文件：`web/playground/static-pg/playground/pg-core.js`、`pg-i18n.js`、`pg-ui.js`、`pg-render.js`、`pg-modal.js`、`playground.css`、`web/pg-media-render.test.js`。
> **最后核对（2026-08-17，Google 原生 generateContent 协议支持与多模态预处理）：** (1) Playground 支持 Google 原生 `generateContent` 协议请求构造与流式 SSE / 非流式响应解析（`pg-request.js` + `pg-stream.js`）。(2) 侧边栏动态参数切换：当当前模型为 Google 协议时，呈现专属参数（`topK`、`thinkingLevel`、`maxOutputTokens`、`stopSequences`、`candidateCount`、`responseMimeType`、`safetySettings` 等）。(3) 内联多模态交互支持：文件选择器 / 剪贴板粘贴 / 拖拽上传支持图片、音频、PDF、视频，音频与视频经 `/api/playground/media-prep` 调用 ffmpeg 提取并转换为 44.1kHz mono 128k mp3 后内联 `inlineData` 发送。
> **最后核对（2026-08-15，Gallery & Text Review 内存治理与清理重构）：** (1) **Gallery Footer 垃圾箱与内存释放**：将原 Directory Tree Header 内的标准 Clear 按钮迁移至 Footer 底部控制栏（位于 Directory Tree 展开按钮右侧），并升级为带开盖动画的 SVG 垃圾箱图标（`.bin-button`，复用 `playground.css`）；点击时清空前端 Blob URL、重置视图，并调用 `POST /api/gallery/clear` 释放后端 owner 相关的内存 session 与临时文件，触发 `debug.FreeOSMemory()`；(2) **Text Review 导航栏 Clear**：在 `1 import -> 2 split -> 3 clean -> 4 review` Stepbar 右侧新增 `Clear` 操作按钮，点击重置前端 `trState` 并调用 `POST /api/text-review/clear` 清空全局 session 并释放物理内存。
...
> **最后核对（2026-08-11，GIF 修复轮）：** GIF 编辑器导出/交接契约与事务化导入收尾（详见 `docs/gif_implented.md` 顶部最后核对）：(1) **导出弹窗（export modal）取代旧结果 overlay**——`gif-editor-export.js` 的 `previewCache`（按格式/config key 隔离）、`setHandoff`/`openResultInGallery` **惰性** `MediaBridge.register`（首次点击才登记）、`closeExportModal`/`cleanup` 中止在途 encoder、清缓存与 handoff（`tinylab:modal-close`）；旧 `showResultModal`/`lastResultAsset` 已移除。(2) **ZIP 帧资产纪律**：`releaseRegistered` 在上传/打包失败与成功均释放帧 asset，打包结果归 MediaBridge token；legacy `upload-temp` 保留 `frame_NNN.png` basename（`galleryEditUploadTemp` 以请求名注册），`zip-outputs` `cleanUp:true` 在成功与所有失败路径释放输入资产（`galleryEditZipOutputs` defer）。(3) **MediaBridge 逐资产投递**：`deliverPendingImports` 逐资产串行——`importedCount>0` 才 `consume`，拒绝/跳过留队下次重试（`web/media-bridge.test.js` 增用例）。(4) **导入事务**：`draftGeneration` 代数守卫异步元数据读取、GIF 切边帧 delay 裁剪、gap/margin 输出像素、commit 清 source 备份。(5) 时间线 zoom 20%–300% 与舞台 zoom 解耦；窄视口响应式布局（style.css GIF 段末尾权威规则）。(6) **i18n 参数化确认文案（post-smoke 修复）**：模块内 `t(key,args,fallback)` 透传 `{N}` 占位符参数给全局 `window.t`，四个确认弹窗（删帧/仅保留/全局缩放/间隔删除）参数化，配套 `web/gif-editor-i18n-args.test.js` 零依赖 Node 合同测试；**二次 smoke-gap 补键**：i18n.js 新增 15 键（`gifEditorSetLatency`/`gifEditorGridSplitTitle`/`gifEditorEven`/`gifEditorUneven`/`gifEditorSplitX/Y`/`gifEditorCellWidth/Height`/`gifEditorCenterGap`/`gifEditorOuterMargin`/`gifEditorEnableOuterGap`/`gifEditorSplitBtn`/`gifEditorTextColor`/`gifEditorStrokeColor`/`gifEditorTimelineAria`），CN 修正 9 处（设置延迟/删除范围/保留范围/间隔删帧×2/应用×2/导出/批量删除帧），CN 补齐 5 缺失键（MagicWandTitle/ApplyTrans/CancelTrans/ApplyDelay/CancelDelay）；Split Sheet/Overlay/时间线 aria 标签接线 + Import Modal Even/Uneven、X/Y、单元格宽高标签接线（5 处残留 GridSplitTitle/Scale/CenterGap/OuterMargin/EnableOuterGap 已随 `gif-editor-import.js` 模板构建 `t(key,fallback)` 接线，Import Modal i18n 全量完成）；测试扩展 smoke-flagged 断言。**硬编码标签清零（final five）**：最后 5 处硬编码英文标签全部 `t()` 接线（en/cn 双语）——`gifEditorExportSettingsTitle`（'Export Settings'/'导出设置'，`#gif-open-export-btn` title）、`gifEditorResetWorkspace`（'Reset Workspace'/'重置工作区'，`#gif-reload-btn` title）、`gifTimelineZoom`（'Zoom:'/'倍率:'，`#gif-timeline-zoom-range` title）、`gifEditorExportPreviewAlt`（'Export preview'/'导出预览'，`#gif-modal-preview-img` alt）、`gifEditorPreviewBtn`（'Preview'/'预览'，`#gif-modal-preview-btn` 含 `data-i18n`）；3 个新键、2 个复用既有键。涉及：`web/static/gif-editor/*`、`web/static/media-bridge.js`、`web/static/style.css`、`web/static/i18n.js`、`internal/api/gallery/edit_handlers.go`。
>
> **最后核对（2026-08-11，Batch 显式进出与手动生成计数）：** (1) Image 侧栏 Batch Project 按钮在 Batch UI 激活期间变为可点击 `Return`（`pgT('pgBatchReturn')`），点击走 `pgImageBatchCloseUI()`（cleanup 关 SSE/reconnect → ExitUI 恢复 Image 布局），保留 draft/plan/transform/active-project/后端执行；Batch 只在显式点击 Batch Project 时进入——`pgImageBatchRestore()`（`pg-image-batch.js`，替代已删除 `pgImageBatchOnEnter`）按 内存 projectId → 内存规划态（plan/transform/stage/draft 内容）→ `imageBatchActiveProject.v1` → `imageBatchDraft.v1` 顺序恢复，否则新建项目；`renderPlayground`/`pgSetMode` 不再自动重入（刷新/切回 Image 模式均落回普通 Image 布局）。(2) Batch 激活期间侧栏 Clear Chat 隐藏，普通 Image 模式保留。(3) Batch Project/Return 按钮右侧新增手动生成计数输入（默认 1）：`w.config.imgSubmitCount`（`pg-core.js` 默认 1）+ `pgGetImageSubmitCount()`/`pgOnImageSubmitCount()`（clamp 1..99，pgSave 持久化，不进 API body，与 Batch Planning quantity=4 无关）；多图循环消费 `pgGetImageSubmitCount()`。
> **最后核对（2026-08-11，Batch 生命周期审计修正）：** 依据 `image_batch_project_redesign_plan.md` 审核结论修正 Batch 生命周期与 trace 契约：(1) 执行项目引用持久化 `tinylab.playground.imageBatchActiveProject.v1`（`{schemaVersion,projectId,savedAt}`，create 后写、Close 不清除、刷新 snapshot-first 重入）；(2) 统一 Close 入口 cleanup+exitUI（关 SSE 再退出 UI）；模式切换先退出 Batch 再切 mode/windows；(3) Stop 显式 immediate/after-current；(4) viewer Prompt×Variant 双层导航；(5) 新建项目清理旧引用；(6) `responseRawBody` 脱敏后 256 KiB 截断。后端协议/schema 未变；详见 §6.1/§7.2/§15 与 flow review §15.4。
> **最后核对（2026-08-11，Batch Project 工作区 inline 双 Pane 与 Request Trace 改造）：** 重构 Playground Image Batch Project 交互范式：(1) 取消 Modal 创建向导，Stage 1–3 改为在 Image workspace 的 inline 双 Pane 视图呈现（Stage 1 左 Pane 为 Planning 表单，Stage 2/3 为 Format Conversion / Review & Start）；Stage 4 执行重置为单 Pane，`#pg-side` 承载项目状态与章节树，`#pg-main-inner` 渲染独立 Batch viewer，不污染 Manual Image generations。(2) 请求透明度 Request Trace：实现 `apiPostTrace()` 支持对 `/plan`、`/transform` 和 `/create` 的原始 Headers/Body/Status/Duration 进行精准捕捉与展示；自动脱敏 sensitive credentials，且 body 限制 256 KiB 内存级展示不落地。(3) State Contract & Draft Persistence：`pgState.imageBatch` 作为唯一状态源；Stage 1–3 草稿持久化至 `localStorage`（`tinylab.playground.imageBatchDraft.v1`）；Stage 4 依据 `projectId + GET snapshot + SSE` 恢复；退出/模式切换完整保存与恢复原始布局（splitCount, activeWin, modeWindows）。涉及文件：`web/playground/static-pg/playground/pg-image-batch.js`、`pg-ui.js`、`pg-render.js`、`pg-lifecycle.js`、`playground.css`、`web/static/api.js`。
> **最后核对（2026-08-11，Set Latency 批量延迟响应修复）：** 修复 `gif-editor.js` 中 `Set Latency` 面板 Apply 按钮点击无响应 Bug：(1) 在 `cacheDom()` 中补全 `dom.batchDelayInput` 与 `dom.batchDelayBtn` DOM 缓存绑定；(2) 在 `bindEvents()` 中加入双重降级获取 `applyDelayBtn` 与 `delayInput` 监听器注册；(3) 点击 Apply 后批量更新 `slices[i].delay`，并同步触发 `timeline.render()` 与 `playback.updateButtons()`。涉及文件：`web/static/gif-editor/gif-editor.js`。
...
> **最后核对（2026-08-10，Playground Image 模式布局重构与沉浸呈现）：** 重构 Playground Image 模式下图片生成后的布局：左右切换按钮（`‹`/`›`）及底部操作按钮组（`Prompt/Parameters`/`Copy`/`Save`/`Regenerate`/`Delete`）重构为悬浮 overlay 层并居于图片容器两侧/底部，默认半透明（`opacity: 0.35`）、鼠标 hover 时变为实体（`opacity: 1`）高亮显现；页码索引信息（`1 / 1`）与图片元数据（`1024 × 1024 · image · imgs\...`）移动至窗口 Title 栏（`.pg-pane-head`）显示；图片本身通过 `object-fit: contain` 100% 比例自适应充盈消息窗口，彻底消除原本控件与路径信息压榨图片空间的问题。涉及文件：`web/playground/static-pg/playground/pg-ui.js`、`pg-render.js`、`web/playground/static-pg/playground.css`。
...
> **最后核对（2026-08-10，本地日志可观测性）：** Playground 与管理端 Recent Requests 均始终捕获完整请求/响应 payload、Header、上游 URL、Key ID、决策和 provenance；实际 Provider Key 通过 `internal/logredact` 固定替换为 `******`，普通 Header、Cookie、URL 参数和 Body 内容保持可见。
> **最后核对（2026-08-10，PNG tEXt 元数据注入与 Gallery 元数据浮层）：** 非 ComfyUI 图片保存链路注入 ComfyUI 同款 `prompt` tEXt——`pg-image-model.js` 生成资产附 `asset.meta`（prompt/model/protocol/params/revised_prompt/created_at/duration_ms/provider/generator），`pg-stream.js::pgAutoSaveImageArtifact`/`pg-modal.js::pgSaveImage` 自动保存时经 `POST /api/save-image` 转发 `metadata`，后端 `saveImage` 在 `ext==".png"` 时经新 leaf 包 `internal/image`（`AsciiJSON` 全 ASCII 转义、`InjectPNGText` 在 IHDR 后写 tEXt 并去重，纯 stdlib）写入，出错回退原字节（保存永不失败）。Gallery 新增 `gallery-meta.js` 元数据浮层（§16）：`gallery-layout.js` 建 `#gallery-meta-btn`/`#gallery-meta-overlay`（200ms hover 计时，中心 400px 方块）、`gallery-state.js` 增 metaOverlay 状态、`gallery-fullscreen.js` 全屏 ESC 分支；客户端解析 PNG tEXt / MP4 `moov→udta→meta`，TinyLab 记录 / ComfyUI 图 / `<pre>` 三种渲染且值全部 `escapeHtml`。ESC 走 capture 阶段 `stopImmediatePropagation`——只关浮层并阻断 app.js 关机与退出全屏处理，浮层隐藏/有 modal 时放行。
>
>
...
> **最后核对（2026-08-07 Playground CSS 第二批高置信度 Token 清理）：** 第二批 CSS 清理在 `web/playground/static-pg/playground.css` 完成 **3 处** `--text-contrast` 替换：`.pg-send`、`.pg-send.stop`、`.pg-image-rem:hover` 三处按钮表面（accent 渐变与 danger 悬停态）的硬编码 `color:#fff` 改为 `var(--text-contrast)`，均属高置信度匹配；本轮同时推进了核心 `web/static/style.css` 的 Token 清理，其详细事实以 `docs/css_upgrade_plan.md` 为准。本轮**未**改变任何 DOM/JS/加载顺序，也**未**进行 CSS 拆分、`!important` 清理或浏览器/HTTP 验证（留待后续轮次）。
> **最后核对（2026-08-07 Playground CSS 第三批 Token 清理）：** 第三批 CSS 清理涉及 Playground 的 `.pg-code-warning` 与 `.pg-code-expand-btn` 两处 `border-radius:6px` → `var(--radius-sm)`；同时核心 `web/static/style.css` 也进行了第三批 Token 清理，详细范围见 `docs/css_upgrade_plan.md` §1.8。本轮不改 DOM/JS/加载顺序，不拆 CSS，不增删 `!important`；构建级门禁已通过，HTTP/浏览器验证仍未执行。
> **最后核对（2026-08-07 Playground CSS 第四批 Token 清理与 Phase 2 第一批）：** 第四批完成 Playground 5 处 `color:#fff` → `var(--text-contrast)`（`.pg-input-thumb-del`、`.pg-gc-new-msgs`、`.pg-model-picker-item.selected`、`.pg-search-history-del`、`.log-mode-btn.active`，均为 accent/danger 表面上文字）；Phase 2 第一批按「同 selector 后规则完全覆盖」证明删除 Playground 3 条规则（`.pg-winbar`、`.pg-param-row label`、`.pg-param-row input:focus`）并收窄 `.pg-param-row` 3 条重复声明（保留 `font-size`/`color`）；不改 DOM/JS/加载顺序、不拆 CSS、不增删 `!important`；构建级门禁已通过（feature test / 双构建 / media-bridge / 57 JS check / CSS 平衡），HTTP/浏览器验证仍未执行。详细记录见 docs/css_upgrade_plan.md §1.11/§1.12。
> **最后核对（2026-08-07 Playground CSS 第五批：DESIGN 一致性核验 + 圆角 Token 化）：** 第五批完成 `DESIGN.md` 与全部 token 层（root/light/9+9 variant/4 style/3 字号）一致性核验（结论完全一致）；Playground 侧圆角 Token 化 3 处（`.pg-bubble kbd`/`details`/`code` 的 4px/6px → `--radius-xs`/`--radius-sm`，低风险选点，避开 GIF/Text Review/全屏区域）；不改 DOM/JS/加载顺序、不拆 CSS、不增删 `!important`；构建级门禁已通过（feature test / 双构建 / media-bridge / 57 JS check / CSS 平衡），HTTP/浏览器验证仍未执行。详细记录见 docs/css_upgrade_plan.md §1.14。
> **最后核对（2026-08-07 Playground 真实 HTTP/浏览器 smoke 首次执行）：** 以 `-tags playground` 构建在 localhost:21337 验证：Playground shell 加载 4 个 CSS（/style.css、/playground.css、/vendor/pg-highlight-theme.css、/vendor/katex.min.css）全 200、默认 shell 仅 /style.css 且 pg 元素缺席；0 console/page error（唯一 request failure 为 `/api/monitor/events` SSE 导航中止，属应用行为）；主题矩阵 9 组合（dark/light × default/tokyo-night/cool × sharp/soft/compact × font s/m/l）computed 值与 token 层一致；desktop 1440 / tablet 768 / mobile 390 无横向溢出；modal 无裁切、Download 页 dropdown 在视口内且 bg=--surface-overlay；reduce 媒体匹配下动效有意不变。证据存 tmp/css-verify/evidence/（gitignored）。详细记录见 docs/css_upgrade_plan.md §1.19。
> **文档定位：** Playground 前后端实现的 canonical 架构事实基线。
> **最后核对（2026-08-15 WebView2 外链默认浏览器打开、Gallery 视频音量控件与全屏退出播放重置）：** 完成 3 项调整：(1) **WebView2 外部 URL 默认浏览器打开**：`host_webview_windows.go` 中通过 `w.Bind` 注入 `openExternalURL`（调用 `fsutil.OpenInBrowser`），并在 `w.Init` 脚本中捕获阶段拦截所有外部链接点击与 `window.open`，阻止 WebView2 自身窗口导航，转而调用系统默认浏览器打开；(2) **Gallery 视频音量控制条**：在视频控制栏最左侧（Prev 按钮左侧）新增音量控制组件，支持 4 种状态的 Feather 规范 SVG 图标（`volumeMute`、`volumeLow` 0~33%、`volumeMedium` 33~66%、`volumeHigh` 66~100%），鼠标悬浮（hover）向上浮现垂直滑动条与百分比提示，支持快速静音/恢复以及快捷键（1-9/↑↓）双向同步更新；(3) **Gallery 退出全屏播放状态重置**：在 `gallery-fullscreen.js` 中封装 `onExitFullscreenPlaybackHook()`，在用户主动退出全屏（F/Esc/右键）或浏览器触发 `fullscreenchange` 退出全屏时，将处于播放状态的图片自动播放（autoplay）停止、视频播放（video play）暂停、动图动画停止。涉及文件：`host_webview_windows.go`、`web/playground/static-pg/gallery/gallery-state.js`、`gallery-layout.js`、`gallery-video.js`、`gallery-fullscreen.js`、`web/static/style.css`。
...
- **执行项目引用（P0-2，2026-08-11 改为显式恢复）**：create/open-project 成功后写 `tinylab.playground.imageBatchActiveProject.v1`（`{schemaVersion:1, projectId, savedAt}`）；Stage 4 Close 与模式切换不清除；仅新建项目（`pgOpenImageBatch`）与恢复引用 snapshot 404 时清除。**不再自动重入**：`renderPlayground`/`pgSetMode` 均不触发 Batch 进入；`pgImageBatchRestore()` 仅在用户显式点击侧栏 Batch Project 时调用，按 内存 projectId → 内存规划态 → active-project 引用 → draft 顺序恢复（恢复 executing 时 GET snapshot → `uiMode='executing'`/`stage=4`、切单 Pane、开 SSE）；EventSource 只订阅，不取消后端任务。
...
- **Viewer 双层导航（P1-2）**：Stage 4 viewer 新增 Prompt 上一层/下一层按钮（`pgImageBatchViewPrompt(index)`，越界禁用）+ Variant ←/→（`pgImageBatchViewVariant(±1)`，越界禁用）；侧栏树 variant 行点击（`pgImageBatchSelectViewer(pi, vi)`）直接定位。
...
| 新增/修改 GIF 编辑器页面（`web/static` 全局 SPA 页） | `docs/gif_implented.md`（实施入口，§4.3/§4.5/§4.6/§7/§8/§10 与 ADR 10-11）、`web/static/gif-editor/gif-editor-state.js`（`window.GifEditorCore`：constants/state/dom/modules/commands/cleanupFns + `registerModule`/`cleanupModules`/`resetSlices`/`releaseSource` + `bindStateEvents`；cleanup 注册表持久化，2026-08-11 起 state 模块经 `registerModule('state', {cleanup})` 每次 teardown 移除 window 键盘块/清 spinner）、`gif-editor-import.js`（Import Modal：`openFromFile`/`bindEvents`/`cancel`/`cleanup` + `draftGeneration` 事务代数 + Import Modal 全量标签 `t(key,fallback)` 接线：GridSplitTitle/Scale/CenterGap/OuterMargin/EnableOuterGap）、`gif-editor-timeline.js`（虚拟化窗口 `timelineWindow`/`thumbCache`/`THUMB_CACHE_MAX`，容器委托交互 + `render`/`updateWindow`/`ensureVisible`/`setZoom`（**20%–300%**，2026-08-11 起与舞台 zoom 解耦））、`gif-editor-playback.js`（播放状态机 + `core.commands.focusFrame` 每 render 重注册 + `first`/`previous`/`play`/`pause`/`toggle`/`next`/`last`/`updateButtons`）、`gif-editor-export.js`（GIF/ZIP/精灵图三导出 + **export modal** `previewCache`/`setHandoff`/`openResultInGallery`（惰性 MediaBridge 登记）/`closeExportModal`；ZIP 走 Archive `assets→pack` + legacy 回退，帧资产失败/成功均释放；旧 `showResultModal`/`lastResultAsset` 已移除）、`gif-editor.js`（入口 `renderGifEditor`/`cleanupGifEditor` + 模板/cacheDom/`applyPageI18n`/`registerCoreCommands`/画布绘制；常量 `MAX_FILE_BYTES`/`EXPORT_MEM_LIMIT` 与 `checkExportMemory`；图片源 Edge Crop/Grid Slice 折叠面板 + Split Sheet Even/Uneven `state.splitMode` 持久化；模块内 `t(key,args,fallback)` 透传 `{N}` 占位符，四确认弹窗参数化；Split Sheet/Overlay/时间线 aria 标签走 t()）、`web/static/i18n.js`（`gifEditor*` 键，导出弹窗 `gifEditorSaveAsZip`/`gifEditorSaveAsGif`/`gifEditorOpenGallery`/`gifEditorClose`；确认键 `gifEditorConfirmDeleteRange`/`gifEditorConfirmKeepRange`/`gifEditorResizeConfirm`/`gifEditorIntervalDeleteConfirm` 用 `{0}..{N}` 占位符；二次修复新增 15 键（`gifEditorSetLatency`/`gifEditorGridSplitTitle`/`gifEditorEven`/`gifEditorUneven`/`gifEditorSplitX/Y`/`gifEditorCellWidth/Height`/`gifEditorCenterGap`/`gifEditorOuterMargin`/`gifEditorEnableOuterGap`/`gifEditorSplitBtn`/`gifEditorTextColor`/`gifEditorStrokeColor`/`gifEditorTimelineAria`）+ CN 修正 9 处（`gifEditorGlobalDelay`='设置延迟'、`gifEditorDelRange`='删除范围'、`gifEditorKeepRange`='保留范围'、ReduceFrame/IntervalDeleteTitle='间隔删帧'、Apply×2='应用'（BatchDelayBtn/IntervalDeleteBtn）、ExportTitle='导出'、BatchDelete='批量删除帧'）+ CN 补齐 5 缺失键（MagicWandTitle/ApplyTrans/CancelTrans/ApplyDelay/CancelDelay））、`web/static/style.css`（`gif-*` 样式，`.gif-timeline-item` 绝对定位 + GIF 段末尾窄视口权威规则；旧 `.gif-result-overlay` 已移除）、`web/static/{index.html,index-nopg.html}`（第 6 按钮 + gif.js→gifuct-js→state→import→timeline→playback→export→editor 脚本序）、`web/static/app.js`（`case 'gif'` + cleanup）、`internal/feature/feature.go`（GIF `StaticFiles` 清单：6 模块 + 3 vendor）、`web/static/media-bridge.js`（导出结果登记生产者；`deliverPendingImports` 逐资产投递 + 拒绝/跳过重试）、`internal/api/gallery/edit_handlers.go`（`galleryEditUploadTemp` 保留请求 basename、`galleryEditZipOutputs` cleanUp 全路径释放）、`web/gif-editor-i18n-args.test.js`（零依赖 Node 合同测试：`{N}` 占位符替换 + 字符串 fallback 兼容 + smoke-flagged 键覆盖（14 个 `check()`——`gifEditorSetLatency` en/cn 与五 CN 标签非英文断言）、PROJECT_MAP §18.2/§24。`go test ./...` 全量（37 包）已通过；**未完成项（勿标完成）见 `docs/gif_implented.md` §10.2**（P4 浏览器冒烟未跑） |
...
- **投递触发**（`gallery.js::renderGallery`）：渲染完成后调 `MediaBridge.deliverPendingImports()`——**2026-08-11 起逐资产串行投递**：每个资产单独调 `galleryImportAssets([asset])`，解析 `importedCount>0` 才 `consume`（明确接受）；被拒（rollback）或跳过（bytes 不可解析）的资产留在队列、下次投递重试；pendingImports 队列不被 `cleanupGallery` 清空，切走再切回仍补投递未消费 handoff。
...
- `web/playground/static-pg/gallery/gallery-meta.js`（元数据侧边栏：toggle 开关右侧 1/3 侧边栏、媒体左移等比缩放、Prompt 全显 + 其余折叠、客户端解析 PNG tEXt / MP4 元数据、capture 阶段 ESC 关闭，2026-08-10）
...
| `/edit/upload-temp` | POST | raw body + `?name=` query | `{assetId}`（audit F-28：原 `tempPath` 已 410；**2026-08-11** 以请求 `name` 注册 asset——`galleryEditUploadTemp` 保留 `frame_001.png` 类 basename 供后续 pack/zip 条目名，不再折叠成 upload.png） |
| `/edit/zip-outputs` | POST | `{assetIds:[...], zipName?, cleanUp?:bool}` | `{assetId, name, size}`（**2026-08-11** `cleanUp:true` 在成功与所有失败路径都释放输入帧资产——defer 全路径；前端经受控 `/api/gallery/file?assetId=` 下载，不再持有 `outputURL`/`zipPath`） |
...
- **Tree Batch Convert（image 左侧 Tree 顶部按钮）**：`gallery-tree.js` 标准 Header 新增 `Batch Convert`（左对齐）+ `AI Review` 按钮，经 `window.openTreeBatchConvert`（`gallery-edit-batch.js`）打开瘦身版 Image Convert 弹窗——包含 Format/Quality/Scale 以及 **Fit to（限制长边）** 选项（移除 Compress 开关，默认统一原地保存）。目标 = `_treeBatchTargets()` 过滤 tree 全部 `galleryState.items` 中 `kind:'backend'`+`grantId` 可写项（fs/plain/zip 跳过并计数 `geTreeBatchSkipped`）；无可写项弹 `geTreeBatchNoWritable`。**Fit to 交互与处理**：Scale 右侧新增 Fit to 开关（`.toggle-switch`）与项目自定义数字步进输入框（`renderStepperHtml`，默认 1920，步长 64），开启时 Scale 滑块禁用置灰，每张图片保持比例将自身长边缩到输入的值（后端 `internal/mediaedit/args.go` 针对 `FitTo > 0` 优先生成 `scale='if(gte(iw,ih),target,-2)':'if(gte(iw,ih),-2,target)'` 且规整偶数尺寸，忽略 `ScalePercent`）；关闭时 Stepper 禁用置灰并恢复 Scale 滑块控制。完成后 `_refreshTreeBatchOutputs()` 同步 rel/path/name（跨格式）+ 吊销 blob URL + 重渲染 thumbs/tree/main。i18n：`geTreeBatchConvert/geTreeBatchSummary/geTreeBatchSkipped/geTreeBatchNoWritable/geFitTo/geFitToTip`（en+zh）。
...
>
>
> **最后核对（2026-09-05，Step2 Split 三项）：** Pattern 下拉统一样式（editor_textreview_step2.js 原生 select→renderCustomSelectHtml 自定义下拉，隐藏 select#tr-s2-pattern 保 change 契约；#tr-s2-pattern-col 撑满左列，style-textreview.css+playground.css 同步三条规则）；Pattern 增删即时刷新（trStep2RenderPatternSelect()：add 选中新 key+重建下拉+重绘 editor+DoSplit，delete 重建下拉+重绘 editor+ReSplit，auto-detect 命中重建下拉）；AI Split 弹窗化（trStep2AISplit 改弹模型选择 Modal：Step3 同款 openModelPickerModal+.tr-model-select-btn+确定/取消，trS2AIModel 内存态，确认后走候选行+LLM 行号真实切分；i18n trAISplitHint 两套 en/cn；删 TR_AI_SPLIT_PROMPT/trExtractCh…
...
> **最后核对（2026-09-05，完整版 10.6MB 回归 + 幽灵章节修复）：** `…聪明？ - 副本 (2).txt`（18 万行/3.8M 字）：去重 236 块 + 74 广告 + 374K 字符（-15.1%），复扫残留 0；切分 1059 章（1–1060 缺 91，锚点 2→1）。修复两处完整版暴露的误切：(1) `findTitleInLine` 纯数字行内命中只认行首/`CJK_SENTENCE_END`（英文 `.` 剔除，`$100,043.28` 尾部 `28`/`83` 不再成章）；(2) 卷旁路只认 ≤30 字短标题（`第20卷内的描写的是…` 长段落不再成章）。短版重验 97 章零回归；契约 14 项（含金额/时间回归）全绿。
