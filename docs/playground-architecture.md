...
- Gallery/Editor/Archive/FileTransfer 资源按 **owner cookie**（`tinyrouter_owner`，HttpOnly，`internal/owner`）绑定会话：跨会话资源访问 403/404，浏览器只提交 `grantId`/`assetId`/`sourceId`（F-29）。
...
- **Stage 1–3 草稿持久化**：`tinyrouter.playground.imageBatchDraft.v1`（`{schemaVersion, savedAt, stage, draft, plan, transform}`）；不保存 trace/API Key/Authorization/凭证；schemaVersion 不匹配则丢弃；create 成功后清除，plan/transform/create 失败保留。
- **执行项目引用（P0-2，2026-08-11 改为显式恢复）**：create/open-project 成功后写 `tinyrouter.playground.imageBatchActiveProject.v1`（`{schemaVersion:1, projectId, savedAt}`）；Stage 4 Close 与模式切换不清除；仅新建项目（`pgOpenImageBatch`）与恢复引用 snapshot 404 时清除。**不再自动重入**：`renderPlayground`/`pgSetMode` 均不触发 Batch 进入；`pgImageBatchRestore()` 仅在用户显式点击侧栏 Batch Project 时调用，按 内存 projectId → 内存规划态 → active-project 引用 → draft 顺序恢复（恢复 executing 时 GET snapshot → `uiMode='executing'`/`stage=4`、切单 Pane、开 SSE）；EventSource 只订阅，不取消后端任务。
...
| `tinyrouter.playground.imageBatchDraft.v1` | Stage 1–3 草稿（stage/draft/plan/transform） | 是（schemaVersion 校验；create 成功清除） |
| `tinyrouter.playground.imageBatchActiveProject.v1` | 执行中项目引用（仅 `{schemaVersion,projectId,savedAt}`） | 是（Stage 4 snapshot-first 重入入口；Close/模式切换不清除） |
