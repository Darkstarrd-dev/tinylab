...
> **最后核对（2026-08-14，Playground Image 侧栏/ComfyUI Tab Select/Window Setup 控件样式整改）：** 按照用户反馈完成 3 项微调修复：(1) **Image 侧栏操作区整改**：移除普通 Image 模式下的 Clear Chat 按钮；`Batch Project` 按钮样式保持单行（`white-space: nowrap`）并不换行；每次提交张数数字输入控件升级为项目通用带有加减号与居中数值的 `.number-stepper` 步进组件（支持 `pgStepImageSubmitCount`），并与 Batch Project 按钮统一为 `36px` 等高平齐；(2) **ComfyUI 协议 Tab Select 下拉动效统一**：`pg-comfyui.js` 中 Tab Select 及节点参数下拉框由原生 `<select>` 统一替换为项目通用的自定义动效下拉组件（`pgRenderCustomSelect` / `renderCustomSelectHtml`），支持 hover、展开动效与选中项高亮；(3) **Window Setup 下拉控件高度对齐**：为 `#pg-split-wrap` 及 `.pg-winbar` 下的 `.custom-select-wrapper` / `.custom-select-trigger` 显式设置 `height: 32px`、`min-height: 32px` 与 `max-height: 32px`，与左侧 `1 2 3 4 ↻` 按钮（32px）实现 100% 绝对等高对齐。涉及文件：`web/playground/static-pg/playground/pg-ui.js`、`pg-comfyui.js`、`web/playground/static-pg/playground.css`。
>
