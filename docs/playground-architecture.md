> **最后核对（2026-09-01，Playground Append to Note + 去重 Retry）：** 删除 `PG_ICON_RETRY` 复制键，`pgMsgMetaInnerHTML` `error` 态仅留 `Regenerate` + `Edit Prompt`（`pg-core.js`/`pg-render.js`/`pg-ui.js:pgRetryError` 删除）；新增 `PG_ICON_APPEND` + 日记落盘：`internal/api/notes POST /api/notes/append`→`ResolveDocDir` 的 `YYYYMMDD-NOTE.md` 换行追加（`StrictRel`+`realPathWithin`+`sync.Mutex`）；前端 `pg-thinking-head` 右侧 `pgAppendSingle`（`user/assistant/reasoning` 带前缀 `\n\n` 拼）、`pg-pane-head` `pgAppendWindow`（遍历 `w.messages` `role:` 前缀 `\n\n` 拼，排除 reasoning/loading）、`pg-search-raw/synth` 头 `pgAppendSearch`（`raw=searchRaw`/`synth=content`），`pgAppendNote` 统一 `apiPost('/notes/append')` + `pgAppended/pgNothingToAppend/pgSearchSynthTitle` 双语 + `playground.css` 追加样式。
