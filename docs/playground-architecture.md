...
> **最后核对（2026-07-28 增补#9）：** Gallery 媒体编辑批量转换 UX 功能补全与 replace-original 语义修复——在 #8 修复（兄弟匹配/输出命名/压缩包名）基础上：(1) **重命名开关（rename）**：`ge-img-rename` + `ge-img-rename-name`，仅在「Convert all + 压缩为 ZIP + 非 overwrite」下显示；`_batchOriginZipName` 优先用 rename 输入，回退到原文件夹/压缩包 stem + `_converted.zip`。(2) **顺序规格化开关（normalise）**：`ge-img-renorm` + `ge-img-renorm-prefix`（默认 `img`）+ `ge-img-renorm-digits`（默认 2）；`_padNum` 左补零至指定位数、**超出位自动扩位**（n=1000,d=3 → `"1000"`，不截断）；`_startBatch` 在开启时按 `_getSiblingImages` 顺序将每条 `OutputName` 设为 `prefix+(pad idx+1)`，关闭时用原图名 stem，服务端 `buildArgs` 始终追加正确扩展名（避免 jpeg→`.jpg` 分歧）。(3) **i18n 占位符修复**：`pgT` 用 `{0}/{1}` 而非 `%s`；已把 `geBatchProgress`/`geBatchDone`（en+zh 共 4 条）从 `%s` 改为 `{0}/{1}`，此前"✔ %s completed, %s failed"字面显示现已正确替代为数值。(4) **show-in-gallery 修复**：`_addOutputToGallery` 原把 `path` 设为裸 `outputName` → `getDirPath` 取空 → 项落入 Root 桶，而 `renderThumbnails` 按 `currentFolderIndices` 过滤当前目录桶，新增项永不可见（"show in gallery 无效"根因）；改为 `path = <输出目录名>/<outputName>` 让项进入自有桶，并显式调用 `updateCurrentFolderItems(idx)` + `renderTreePanel()` 重新派生缩略图带+树。复用既有 `appendItems` 基础设施。(5) **download→打开目录**：新增 `POST /api/gallery/open-folder {path}`，复用既有 `fsutil.OpenInFileManager`（跨平台 explorer/xdg-open/reveal）；所有完成结果区（compress/non-compress/视频/单图/zip-writeback）将"Download"按钮替换为"打开目录"（`geBatchOpenFolder`），指向保存目录或 zip 所在文件夹。(6) **replace-original 真正原位**：`_startJob` 前置守卫——`overwrite && !(_editCurrentItem.kind==='backend' || (kind==='zip' && zipAbsPath && zipPath))`时弹 `geNoDiskPath` 拒绝（FSAA/drag-drop/plain 无可写回原文件，覆盖临时文件会让用户误以为成功）；后端 zip 单图/convert-all 走**新端点** `POST /api/gallery/edit/zip-writeback {archivePath, entries:[{zipPath, filePath}]}`：新增 `internal/gallery/zip_replace.go` `ReplaceZipEntries(data, map[string][]byte) ([]byte, Manifest, error)`，未命中条目字节级保留（含 Method/Modified/Extra/comment），命中条目替换原 FileHeader 写入新内容；`internal/api/gallery/register.go` `galleryEditZipWriteback` 读取磁盘归档→调用 `ReplaceZipEntries`→`fsutil.AtomicWrite` 原子回写并 best-effort 删除临时输入；客户端 `_onCompleted` 单图 zip 路径与 `_zipWritebackBatch` convert-all 路径均走此端点。后端单文件/单图（`kind:'backend'` 有真 `absPath`）依赖既有 ffmpeg overwrite 实现真写回原文件路径。**新增端点**：`POST /api/gallery/open-folder`、`POST /api/gallery/edit/zip-writeback`。**新增键**（`pg-i18n.js` en+zh）：`geBatchOpenFolder`/`geBatchOpenError`/`geRenameZip`/`geRenameZipHint`/`geRenamePlaceholder`/`geRenorm`/`geRenormPrefix`/`geRenormDigits`/`geRenormHint`/`geRenormPrefixPh`/`geBatchDoneAll`/`geBatchFiles`/`geNoDiskPath`/`geExtracting`，改占位符：`geBatchProgress`/`geBatchDone`。**新增测试**：`internal/gallery/zip_replace_test.go`（`TestReplaceZipEntries_Store_ReplacesAndPreserves` / `_Deflate_ReplacesAndPreserves` 验证 deflate 实际压缩 + `_MissingKey_NoOp` 空映射/未知键字节等价 + `_CleanedKeyContract`）；`internal/api/gallery/register_test.go`（`TestGalleryRoutes_ZipWriteback_EmptyEntriesNoOp` / `_ReplacesEntry`）。涉及文件：`web/playground/static-pg/gallery-edit.js`、`pg-i18n.js`、`internal/api/gallery/register.go`+`register_test.go`、`internal/gallery/zip_replace.go`+`zip_replace_test.go`、`internal/mediaedit/types.go`+`manager.go`+`manager_test.go`（`StartRequest.OutputName` 借上轮）。文档：PROJECT_MAP §10.9/§10.9a、§24、playground-architecture.md §4.2、§16。
...
- **输出路径（无 OutputDir）**：非覆盖且不指定 `OutputDir` 时在原文件同目录生成 `{base}_{desc}.{ext}`（仅单文件编辑路径；批量转换必带 `OutputDir`），冲突时追加 `_2`/`_3`；覆盖模式（`overwrite:true`）直接写入原文件。
...
- **批量 UX 选项**（仅在「Convert all」勾选且非 overwrite 时出现）：(a) **Rename**（`ge-img-rename` + `ge-img-rename-name`）— 仅在压缩为 ZIP 时显示，自定义压缩包名 stem（回退到原文件夹/压缩包 stem + `_converted.zip`）；(b) **Sequential rename**（`ge-img-renorm` + `ge-img-renorm-prefix` 默认 `img` + `ge-img-renorm-digits` 默认 2）— 按兄弟序顺序将每条 `OutputName` 设为 `prefix` + 左补零序号，超出位数**自动扩位**（`_padNum` 不截断），扩展名仍由服务端 `buildArgs` 追加。
- **replace-original 守卫**：`_startJob` 前置守卫 `overwrite && !canReplace` → 弹 `geNoDiskPath` 拒绝；`canReplace = kind==='backend' || (kind==='zip' && zipAbsPath && zipPath)`，覆盖 fs/plain/FSAA-dropped-zip 等无可写回原文件的情况（覆写临时文件会让用户误以为成功）。
...
| 修改 show-in-gallery 逻辑 | `web/playground/static-pg/gallery-edit.js`（`_addOutputToGallery`：`path` 加目录前缀接入目录桶 + 追加后调 `updateCurrentFolderItems(idx)`/`renderTreePanel()` 重渲缩略图带）+ `web/playground/static-pg/gallery-tree.js`（`getDirPath`/`updateCurrentFolderItems` 提供桶划分与刷新契约） |
| 修改 Same Path + sequential rename | `web/playground/static-pg/gallery-edit.js`（`_refreshBatchUXVisibility` 去掉 renorm 行 `!isOverwrite` gate；`_startBatch` 在 overwrite 分支也送 `outputName`）+ `internal/mediaedit/manager.go`（`Start()` 新增 `OutputName != "" && !Overwrite && OutputDir == ""` 同路径分支用 `relocateOutput(dir(InputPath), OutputName+"_"+desc+ext)`）+ `pg-i18n.js`（`geReplaceOriginal` label 改为 "Same Path"/"同路径"） |
| 修改视频自定义文件名（rename 对等） | `web/playground/static-pg/gallery-edit.js`（共享 dest block 新增 `ge-dest-rename-row` + `ge-dest-rename` 输入框；dest radio onchange 同步可见性仅"另存到目录"时显示；`_startJob` 读取 `ge-dest-rename` 覆盖 `origStem` 作为 `OutputName`） |
| 修改 trim 片段拖动约束 | `web/playground/static-pg/gallery-edit.js`（`_startTrimDrag.onMove` + `_moveNearestHandle` 新增 prevEnd/nextStart 跨片段约束：`seg.start ≥ prevEnd`、`seg.end ≤ nextStart`；保留组内 0.1s 最小宽度） |
| 移除 Show in Gallery 按钮 | `web/playground/static-pg/gallery-edit.js`（单图/compress/non-compress/zip-writeback 四处完成结果区删 `ge-show-btn`/`ge-show-batch-btn` 标记 + `_addOutputToGallery` 绑定；`_addOutputToGallery` 函数保留但不再被调用） |
