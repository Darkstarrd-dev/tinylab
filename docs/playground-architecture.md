...
> **最后核对（2026-08-11，Split Sheet 方案 A 真实像素 Gap 计算与无缝对齐）：** (1) 方案 A 目标画布真实 Gap/Margin：在 `splitImage()`、`commitImageDraft()`、`updateImportPreview()` 及 `estimateFrameCount()` 中，用户输入的 `Center Gap` 和 `Outer Margin` 直接作为缩放目标画布（如 256×256）上的真实像素，不再额外乘以 Scale 造成缝隙缩小；(2) 舞台 100% 预览精准映射：`drawSplitGridOverlay()` 将目标画布 40px Gap/Margin 按 `Gap / Scale` 换算映射至原图坐标系（1024 坐标系下呈现 160px 缝隙），使原图舞台预览框位置与 256×256 切分导出结果精确一致。涉及文件：`web/static/gif-editor/gif-editor.js`、`web/static/gif-editor/gif-editor-import.js`。
