...
> **最后核对（2026-08-22，Gallery 渐进加载：tree 先显 + 首图即显 + 压缩包流式展开）：** 修复 Gallery 粘贴/拖入大量内容（尤其多个 zip 包）时全部处理完才一次性显示、体感等待可达 10s+ 的问题。`gallery-io.js` 加载管线改为三阶段渐进：(1) `walkDir` 保持纯遍历——zip 不再 inline `addArchive` 上传，而是收集 `{kind:'zipfile',file,zipFileHandle}` 交调用方处理（顺带修复 `processHandles`→`processCollectedEntries` 路径上 fs/zipfile 条目被静默丢弃的隐患）；(2) 松散文件在 list-dir/分类完成后立即 `appendItems`/`appendVideoItems`——tree 与第一张主图即刻渲染；(3) 压缩包流式展开：`processZipFilesProgressive`（FSAA blob 包，并发 6 保持 LRU 上传保护）与 `processBackendZipsProgressive`（磁盘包 `zip-from-path`，**并发 12**——会话引用磁盘文件、被 LRU 驱逐后重读磁盘即可重建，无上传风暴，故独立于上传上限调高）逐包取 manifest；增量上屏经 **`queueProgressiveItems` 定时合并**（120ms 窗口，首包立即 flush、导入结束前强制 flush）→ `appendProgressiveItems`（首批建立活动项、后续合并不抢焦点），避免逐包全量 strip/tree 重建的 O(n²) DOM 抖动。共享构造器 `fetchZipManifest`/`pushZipPackItems` 统一 zip item 字段；视频自动切换 guard 计入待展开包数维持旧语义。基准：`autoresearch.sh`（Node VM 驱动真实 gallery-io.js + 固定延迟 mock），60 包×30 图+20 散图场景 ttfi 11272ms→78ms、tree 11227ms→29ms、总耗时 11227ms→991ms、strip/tree 重建 61→6 次（扫描槽位 56k→5.5k）。涉及文件：`web/playground/static-pg/gallery/gallery-io.js`、`autoresearch.sh`、`autoresearch/bench.mjs`。
