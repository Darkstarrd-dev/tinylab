...
> **最后核对（2026-08-09，audit_fix.md Phase A/B/C/F 落地）：** (1) **管理面认证边界（F-04/F-05）**：§4.5 更新——管理密码保护现为 **setup-required 状态机**：未设置密码时 `/api/*` 管理路由（含 Gallery/Editor/Archive/FileTransfer）全部 401 `setup_required`，`POST /api/auth/setup` 完成初始化；已设置密码后所有状态变更请求需会话 + session-bound `X-CSRF-Token` + 本地 Origin + JSON/multipart Content-Type（`internal/api/auth/handler.go::AuthMiddleware`/`csrfChecksPass`/`originAllowed`/`requireJSONBody`）；`web/static/auth.js` 全局 fetch 包装自动注入 CSRF token（覆盖 Playground 与 Gallery 的直连 fetch）。`/v1/*` 与 Playground 静态文件仍不经 AuthMiddleware（本地代理入口，F-12 显式决策）。(2) **Gallery 路径能力合同（F-03/F-28/F-30）**：后端全部迁移——`/api/gallery/open-dir` 原生 picker → owner 绑定 `grantId`；`list-dir`/`file`/`fs`/`open-folder`/`zip-from-path`/`zip-writeback`/`edit/*` 只接受 `grantId`/`assetId`/`sourceId(+entryPath)`，raw path → 410；响应不含绝对路径（`galleryFsEntry{name,rel,size,kind}`）。前端 `web/playground/static-pg/gallery/*.js` 已于 2026-08-09 迁移（`_startJob` 发 `inputAssetId`/`inputGrantId`/`subtitleAssetId`/`outputName`，`zip-outputs`/`edit/zip-writeback`/`/fs`/`/file` 全走 grantId/assetId/sourceId，raw path 调用为零——仅存永不赋值的 `zipAbsPath`/`rootDirPath` 死分支与单 zip extract→edit 的 `data.tempPath` 残留读，后者为非安全功能缺陷待修）。(3) **依赖（F-24/F-25）**：`static-pg/vendor/purify.min.js` → DOMPurify **3.4.13**、`mermaid.min.js` → **11.16.1**（`securityLevel:'strict'`，来源/SHA-256 见 `static-pg/vendor/README.md` + `LICENSE.mermaid`）。(4) **Editor HTML iframe 隔离（F-10）**：`web/static/utility/editor/editor_shell.js` HTML 预览 iframe `sandbox=""`（零权限，预览 .html 不能 fetch `/api/*` 或读 `parent.document`），TOC 经父页 DOMParser 构建；Editor 后端 `/api/editor/*` 改 `fileId`/`pathGrantId` 合同（§17 相关章节同步见 `PROJECT_MAP.md` §10.8）。
...
- 管理面 `/api/*`（含 Gallery/Editor/Archive/FileTransfer）经 `AuthMiddleware`——**2026-08-09（audit F-04）：** 未设置密码时为 **setup-required** 状态（全部管理路由 401 `setup_required`，仅 `/api/auth/status`、`/api/auth/login`、`POST /api/auth/setup` 可用）；已设置密码后，所有 POST/PUT/PATCH/DELETE 还需 session-bound `X-CSRF-Token` + 本地 Origin + JSON/multipart Content-Type（F-05）。
- Playground 静态文件和 `/v1/*` 不经过 `AuthMiddleware`（本地代理入口，无应用层认证；`/v1` 无配置修改能力——F-12 显式兼容决策）。
