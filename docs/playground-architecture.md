...
- **Tree Batch Convert（image 左侧 Tree 顶部按钮）**：`gallery-tree.js` 标准 Header 新增 `Batch Convert`（左对齐）+ `AI Review` 按钮，经 `window.openTreeBatchConvert`（`gallery-edit-batch.js`）打开瘦身版 Image Convert 弹窗——仅保留 Compress/格式/画质/缩放（无 source-info/Set Path/Set Name/Uniform 行，Strip metadata 固定 false）。目标 = `_treeBatchTargets()` 过滤 tree 全部 `galleryState.items` 中 `kind:'backend'`+`grantId` 可写项（fs/plain/zip 跳过并计数 `geTreeBatchSkipped`）；无可写项弹 `geTreeBatchNoWritable`。Start 经 `_startBatch('image_transcode', params, {overwrite:true}, false, writable)` 原地覆盖（`_startBatch` 已透传 `dest.overwrite`，覆盖时不送 `outputName`；跨格式后端自动新扩展名+删旧文件）；Compress 勾选时切非覆盖走既有 zip-outputs 打包链。完成后 `_refreshTreeBatchOutputs()` 同步 rel/path/name（跨格式）+ 吊销 blob URL + 重渲染 thumbs/tree/main。i18n：`geTreeBatchConvert/geTreeBatchSummary/geTreeBatchSkipped/geTreeBatchNoWritable`（en+zh）。
