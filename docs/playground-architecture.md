...
> **最后核对（2026-08-22，Gallery 渐进加载：tree 先显 + 首图即显 + 压缩包流式展开）：** 修复 Gallery 粘贴/拖入大量内容（尤其多个 zip 包）时全部处理完才一次性显示、体感等待可达 10s+ 的问题。`gallery-io.js` 加载管线改为三阶段渐进：(1) `walkDir` 保持纯遍历——zip 不再 inline `addArchive` 上传，而是收集 `{kind:'zipfile',file,zipFileHandle}` 交调用方处理（顺带修复 `processHandles`→`processCollectedEntries` 路径上 fs/zipfile 条目被静默丢弃的隐患）；(2) 松散文件（backend 图片/视频、plain blob、fs 条目）在 list-dir/分类完成后立即 `appendItems`/`appendVideoItems`——tree 与第一张主图即刻渲染；(3) 压缩包经新助手函数流式展开：`processZipFilesProgressive`（FSAA 拖放/粘贴的 blob 包，并发 6）与 `processBackendZipsProgressive`（磁盘包 `zip-from-path`，并发 6）逐包取 manifest，每包完成即经 `appendProgressiveItems`（首批 `appendItems` 建立活动项、后续 `appendNewItems` 合并不抢焦点）增量上屏；`processBackendFileList` 增 `zipJobs` 参数把 zip 条目推迟为 job；`onOpenDirBackend`/`loadBackendGrants`/`processCollectedEntries`/`onDrop`/`onPaste` 全部切换到该契约；共享构造器 `fetchZipManifest`/`pushZipPackItems` 统一 zip item 字段。行为保持：并发上限 6（LRU 会话驱逐保护）、视频自动切换 guard 现计入待展开 zip 包数（`zipFiles.length`）维持旧语义。基准：`autoresearch.sh`（Node VM 加载真实 gallery-io.js + 固定延迟 mock 后端），60 包×30 图 + 20 散图场景 ttfi 11272ms→80ms、tree 11227ms→33ms、总耗时 11227ms→1926ms。涉及文件：`web/playground/static-pg/gallery/gallery-io.js`、`autoresearch.sh`、`autoresearch/bench.mjs`。
...
| 修改 Gallery 导入渐进加载（tree 先显/首图即显/包流式展开） | `web/playground/static-pg/gallery/gallery-io.js`（`walkDir` 纯遍历收 `zipfile`、`appendProgressiveItems`/`processZipFilesProgressive`/`processBackendZipsProgressive`/`fetchZipManifest`/`pushZipPackItems`、`loadBackendGrants`/`onOpenDirBackend`/`processBackendFileList`(zipJobs 参数)/`processCollectedEntries`/`onDrop`/`onPaste` 契约）、`autoresearch/bench.mjs`（管线基准） |
