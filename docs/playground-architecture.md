...
> **最后核对（2026-08-11，GIF Editor 异常修复、排版对齐与功能块重构）：** (1) 侧边栏单图 4 面板规则：导入单图（`kind === 'image'`）时严格限制为 4 个面板，按序保留 `Crop` (1)、`Set Transparency` (2)、`Split Sheet` (3)、`Set Latency` (4)，隐匿 `Overlay`、`Batch Frame Delete` 与 `Reduce Frame`；导入视频/GIF 时隐匿 `Split Sheet`，保留 6 个面板（包含 `Overlay`）；(2) Split Sheet 中心轴平滑缩放与舞台自适应：修正 `getEffectiveDimensions()` 优先取 `canvas.width` 与 `canvas.height`，解决缩放时定位误按原分辨率 `2479` 计算偏移导致画面向左上角偏移收缩的 Bug；拖动 Scale 时画布按中心轴 `(panX, panY)` 居中对齐，舞台呈现保持容器自适应，同时直观表现出低分辨率精细度与像素点颗粒感；(3) 视频 Overlay 展开点击响应修复；(4) Export Sprite Sheet `ReferenceError` 修复；(5) 导入/导出 Spinner z-index 20000 层级置顶与 `commitImportDraft` 自动关弹窗。涉及文件：`web/static/gif-editor/gif-editor-import.js`、`web/static/gif-editor/gif-editor-export.js`、`web/static/gif-editor/gif-editor.js`、`web/static/gif-editor/gif-editor-timeline.js`、`web/static/style.css`、`web/static/i18n.js`。
