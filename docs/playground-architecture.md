...
> **最后核对（2026-07-30 增补#22 路径弹窗复用）：** Gallery Edit 齿轮按钮（`gallery-edit.js:1017`）由 `openDownloadSettingsModal()` 改调 `openPathSettingsModal({sections:{defaultDir:true,ffmpegPath:true}})`，仅显示 defaultDir+ffmpegPath 两项；`#dl-settings-overlay` 轮询不变，关闭后仍触发 `_checkFfmpegStatus()`。

>
> **最后核对（2026-07-30 增补#23 Image 模式端点/协议/GPT 参数）：** Playground Image 模式新增：**(1) 端点控制** - 图片参数区下方新增 endpoint 原生 <select>（generations 默认 / edits），仅 Image 模式可见，text 模式不变；edits 端点发送纯 JSON body（含 image_url data URL），无 multipart 转换。**(2) 双原生 select 选择器** - 模型选择器在 Image 模式替换为 1:3 比例的双 select（protocol 左、model 右），协议列表动态取自 pgImageProtocols()（仅 image 模型的 imgProtocol 值，含 All Protocols），选中协议后过滤模型列表并清除不匹配的 model，选中 model 后协议从 imgProtocol 预填。**(3) 有效协议辅助** - 新增 pgEffectiveProtocol(cfg)（UI 可见性用）与 pgImageProtocols()，pgGetImgProtocol 的 model/default 行为在请求构造中保持不变。**(4) GPT 参数** - quality 扩展为 Auto/Low/Medium/High；新增 n 1..5、response_format (url/b64_json)、output_format (png/jpeg/webp)、output_compression 0..100、user 字段，仅在设置时输出对应字段；WisArt 尺寸（auto, 1024x1024, 1200x675, 928x1664, 3000x1000）纳入内置列表与 Edit 弹窗。**(5) xAI/modelscope** 控件与 payload 保持不变。**(6) 粘贴图片流** (FileReader data:image/...;base64 -> JSON image_url) 在所有协议下保持不变。涉及文件：`web/playground/static-pg/pg-core.js`、`pg-ui.js`、`pg-request.js`、`pg-stream.js`、`pg-i18n.js`、`pg-modal.js`、`docs/playground-architecture.md`、`PROJECT_MAP.md` §24。
