...
| 新增 Tree 顶部 Batch Convert 原地批量 | `web/playground/static-pg/gallery/gallery-tree.js`（标准 Header 左对齐 Batch Convert 按钮 + 点击接 `openTreeBatchConvert`）+ `gallery-edit-batch.js`（`openTreeBatchConvert` 瘦身弹窗（Compress/格式/画质/缩放）/`_treeBatchTargets`（backend+grantId 可写过滤）/`_bindTreeBatchDialog`/`_refreshTreeBatchOutputs`（跨格式 rel/path/name 同步+视图刷新）/`_startBatch` 透传 `dest.overwrite`（覆盖不送 outputName）/`_pollBatchJob` 记 `overwritten`/`_onBatchComplete` grant 回退）+ `gallery-edit-operations.js`（`_checkFfmpegStatus` 兼容 tree Start 禁用）+ `gallery-edit.js`（`_show/_hideProgressSection` 兼容 tree Start 禁用）+ `pg-i18n.js`（`geTreeBatchConvert/geTreeBatchSummary/geTreeBatchSkipped/geTreeBatchNoWritable` en+zh） |
