...
| 修改原地替换（Replace Original） | `web/playground/static-pg/gallery-edit.js`（`_getDestination` 恢复读 `ge-dest` radio；`_startBatch` 补 `canReplace` 守卫；`_refreshBatchUXVisibility` renorm 行恢复 `!samePath` gate + dest radio onchange 联动；批量非压缩完成区 Open Folder 改捕获 `outputPaths[0]`；`_getSiblingImages` `kind:'plain'` 补 `return []`；`_onCompleted` 删 zip 分支死 `logHtml`）+ `internal/mediaedit/manager.go`（`Start()` 覆盖跨格式→`<dir>/<stem><newExt>` + `removeOnSuccess` 删原文件；删 `OutputName != "" && !Overwrite` 死分支；`runJob` 签名加 `removeOnSuccess`）+ `internal/mediaedit/manager_test.go`（`TestManager_TranscodeImage_Overwrite` 改验跨格式契约）+ `pg-i18n.js`（`geReplaceOriginal` en 恢复 "Replace Original File"、zh "原地替换原文件"） |
