...
- **执行项目引用（P0-2）**：create/open-project 成功后写 `tinyrouter.playground.imageBatchActiveProject.v1`（`{schemaVersion:1, projectId, savedAt}`）；Stage 4 Close 与模式切换不清除；仅新建项目（`pgOpenImageBatch`）与恢复引用 snapshot 404 时清除；刷新/重入在 `pgImageBatchOnEnter` 先读引用 → GET snapshot → 成功后置 `uiMode='executing'`/`stage=4`、切单 Pane、开 SSE；EventSource 只订阅，不取消后端任务。
...
- `web/playground/static-pg/playground/pg-image-batch.js`：Batch 三步向导（plan/transform/review）、Stage 4 viewer/sidebar renderer（`pgImageBatchRenderPane`/`pgImageBatchRenderSidebar`/`pgImageBatchRenderCanvas`）、SSE 订阅与 `pgImageBatchCleanup`、`pgImageBatchOnEnter` snapshot-first 重入、`tinyrouter.playground.imageBatchDraft.v1`/`tinyrouter.playground.imageBatchActiveProject.v1` 持久化、显式 Stop immediate/after-current、Prompt×Variant 双层导航；`web/static/api.js` 的 `apiPostTrace()`（`redactTraceValue`/`redactTraceText` 脱敏 + 256 KiB 截断）；
...
| 修改 Image Batch Project 前端生命周期/SSE/重入（08-11 审计修正） | `pg-image-batch.js`（`pgImageBatchCloseUI` 统一 Close、`pgImageBatchStopImmediate`、`pgImageBatchViewPrompt` 导航、active-project/draft localStorage）、`pg-ui.js`（`pgSetMode` Batch 退出时序）、`pg-lifecycle.js`（`pgImageBatchOnEnter` snapshot-first 重入）、`web/static/api.js`（`redactTraceText`/`redactTraceValue`）、本文 §6.1/§7.2、`image_batch_project_flow_review.md` §15.4 |
