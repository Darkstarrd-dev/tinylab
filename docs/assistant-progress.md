...
> 最后核对：2026-08-30（模型设置弹窗+per-preset采样参数+宠物聊天+单文件记忆+闲时10分归纳；详情见 PROJECT_MAP 顶置本轮条目与 §1 模型设置/聊天/记忆三行 + §9）。

## 1. 功能面总览

| 层 | 位置 | 职责 |
|---|---|---|
| 配置 | `internal/config/types.go` (`AssistantConfig`/`AssistantAction`)、`defaults.go`、`persistence.go`（legacy 迁移）；`internal/config/paths.go::ResolveAssistantDir` | `model`（LLM 意图分类模型）、`actions[]`（多 action 精灵动画）、`enabled *bool`（功能开关，nil=开）、`debug`（宠物消息日志门控）；**模型设置与记忆**见独立行 `assistant/presets.go` / `memory`（`{configDir}/assistant/` 下 `model-presets.json` + `memory/<slug>.md`，与 config.yaml 完全分离，不进 config 结构体） |
...
| 意图分发 | `internal/api/assistant/handler.go`（`dispatch`→`classifyIntent`：LLM 优先 `internal/assistant/llm_classifier.go`，无模型/失败回退关键词 `semantics.json`）、`schema.go`、`events.go`（SSE 广播）；`internal/api/assistant/chat.go`（聊天） | `POST /api/assistant/dispatch` 返回命中的工具（保留）；`GET /api/assistant/events` SSE `notify`；**`POST /api/assistant/chat` 聊天**（`ChatClient` 直连本机 `/v1/chat/completions`，`ActivePreset()` + `memory.Read` + `ChatSessions` + `MemorySlug`，`ApplyParams` 仅 enabled 注入） |
| 聊天与记忆 | `internal/assistant/chat.go`（`ChatClient`/`ChatSessions`/`BuildSystemPrompt`，loopback `/v1/chat/completions`、content 兼容 string/array-parts、120s 超时）、`internal/assistant/memory.go`（`MemoryManager`：`{assistantDir}/memory/<slug>.md` 单文件 per-preset、mtime 缓存、`NoteTurn`→`AfterFunc(10m)` 闲时归纳、`Summarize` 闭包由 router 装配、失败保留旧文件）、`internal/config/paths.go::ResolveAssistantDir`（→ `{configDir}/assistant`） | 宠物输入有模型时走 chat（系统提示词 = 身份+`SystemPrompt`+记忆块≤6000 字符），无模型回退 dispatch；会话历史 per-preset `MaxSessionMessages=40` 仅内存；记忆归纳系统提示词 `SummarizeSystemPrompt` + 最近 40 轮≤8000 字符 transcript，归纳模型 = `preset.MemoryModel` 或 `assistant.model`，不注入采样参数，≤800 字 |
| 宠物运行时 | `web/static/sprite-pet.html`（`#pet-bubble-msg` max-height 160px 滚动） + `sprite-pet.js`（`window.__petAssistantModel`、`sendPetIntent` 分流：有模型→`POST /api/assistant/chat` + think/reply/error，空→dispatch 原路径；`loadPetActions` 同回调内 `__petAssistantModel=((s.assistant||{}).model||'')`）、`host_webview_windows.go::openPetWindow/petOnMessage` | 唯一助手表面（App 内 dock 已废弃，勿恢复）；petSM 状态机 + 页面驱动窗口尺寸 |
...
| 模型设置/Chat/Memory（本轮新增） | §1 三新增行 + §9、`internal/assistant/presets.go`+`chat.go`+`memory.go`、`internal/api/assistant/chat.go`+`handler.go:SetChatDeps`/`register.go`、`internal/api/router.go:New()` 接线 + `internal/config/paths.go:ResolveAssistantDir`、`web/settings_assistant_model.js` + `settings_modal.js` 开按钮 + `sprite-pet.js:sendPetIntent` 分流/`loadPetActions:__petAssistantModel` + `sprite-pet.html` 160px + `i18n.js:assistantModelSettings/assistantMS*` + 双 index 挂脚本、`PROJECT_MAP` 本轮顶置条目 |
...
## 9. 模型设置弹窗 + 宠物聊天 + 单文件记忆（2026-08-30）

- **后端**：
  - `internal/assistant/presets.go` — `ParamValue{Enabled,Value}` / `ModelPreset{Name,AssistantName,SystemPrompt,MemoryModel,Params}` / `ModelPresetFile{Active,Presets[]}`；`KnownParams` 8 键、`ValidReasoningEffort` 白名单、`ApplyParams`（仅 enabled 注入：6 数值原样、`reasoning`→`{"enabled":bool}`、`reasoning_effort`→字符串）、`SanitizeParams`（数值 clamp、未知键丢弃、非法 reasoning_effort 报错）、`PresetStore{LoadPresets(path)→缺文件默认 default、Get 深拷贝、Save 校验+fsutil.AtomicWrite 0644、ActivePreset 回退}`。
  - `internal/assistant/chat.go` — `ChatMessage` / `ChatClient{Addr,Model}→POST {Addr}/v1/chat/completions`（仅 `Content-Type`，120s 超时，content 兼容 string/array-parts，HTTP≥400→error、空 content→error）/ `ChatSessions{max 40 FIFO}` / `BuildSystemPrompt`（身份行→SystemPrompt→记忆块 `## 用户记忆…` ≤6000）。
  - `internal/assistant/memory.go` — `SummarizeSystemPrompt`（800 字 markdown 归纳）/ `MemorySlug`（非法→`_`）/ `MemoryManager{Dir={assistantDir}/memory,Idle=10m,History,Summarize,Logf,states map}`：`Read(mtime 缓存读)` / `NoteTurn(Stop 旧 timer+AfterFunc(10m,fire))` / `fire(防重入,snapshot 已有记忆+transcript≤8000→2m ctx Summarize→trim 非空才 AtomicWrite .md + 更新 mtime 缓存，失败 Warn 保留旧文件)` / `Stop` / `buildTranscript(最近 40，回落从头截最旧保留 8000)`。
  - `internal/api/assistant/chat.go` — `POST /api/assistant/chat {message}→{reply,preset}`（空 message 400，未接线/无 assistant.model 503→`assistant chat not configured`，成功 200+`NoteTurn`，失败 502）；`GET/PUT /api/assistant/model-presets→{active,presets}`（PUT 校验失败 400）；按 `cfg.Port≤0→20128` 求 addr；支持 `SetChatAddr` 注入测试。
  - `internal/api/assistant/handler.go` 新增 `presets/memory/sessions/chatAddr` 字段 + `SetChatDeps`/`SetChatAddr`；`internal/api/assistant/register.go` 新增 `POST /chat` + `GET/PUT /model-presets`（`owner.Middleware`）。
  - `internal/api/router.go New()` — `assistantDir=config.ResolveAssistantDir("", filepath.Dir(configPath))` → `LoadPresets(model-presets.json)` + `NewChatSessions()` + `MemoryManager{Idle:10m, History: sessions.Get, Logf: logger.Warn, Summarize: per-preset MemoryModel || assistant.model 的 ChatClient}` → `astHandler.SetChatDeps(...)`。
  - `internal/config/paths.go` — `ResolveAssistantDir(dir,configDir)`（镜像 `ResolveDocDir`，`→{configDir}/assistant`）。
  - 存储与 `config.yaml`/`state.yaml` 完全分离（`{configDir}/assistant/model-presets.json` + `memory/<slug>.md`），当前未显式创建目录（首次 Save/NoteTurn 成功时 `MkdirAll`，路径来自 `ResolveAssistantDir`）。
- **前端**：
  - `web/static/settings/settings_assistant_model.js`（`openAssistantModelSettings` 独立二级弹窗，fetch model-presets→草稿 `__assistantMS{active,presets,sel}`）：overlay `calc(var(--z-modal)+20)`（model picker 为 +30 在其上）、preset 下拉 `renderCustomSelectHtml` + Add(`promptModal`/`prompt`)/Delete(`confirmModal`/`confirm`，禁止删最后一个)/下拉切 stash+load；表单：助手名称 input、记忆整理模型按钮（复用 `openAssistantModelPicker`+✕ 清除跟随主模型）、系统提示词 textarea、8 参数行（toggle-switch + 数值 `input[type=number]` 小数 + top_k `renderStepperHtml`(整数parseInt 安全) + reasoning/–effort `renderCustomSelectHtml`，off 时 disabled/wrap 0.5）；新 preset 缺省 `enabled=false, temperature0.7/top_p1/top_k0/min_p0/presence0/repetition1/reasoning true/effort medium`；Save=`PUT /api/assistant/model-presets`。
  - `web/static/settings/settings_modal.js` — `openAssistantModal` 插入“模型设置”行（`assistantModelSettingsOpen` 按钮）；`web/static/i18n.js` en+cn 新增 `assistantModelSettings/assistantMS*` 12 键；`web/static/index.html`/`index-nopg.html` 在 `settings_assistant.js` 后追加 `settings_assistant_model.js`。
  - `web/static/sprite-pet.js` — `loadPetActions` 同回调内 `__petAssistantModel=((s.assistant||{}).model||'')`；`sendPetIntent` 分流：有模型→`POST /api/assistant/chat {message}→setBubbleMsg(reply)/reply|error`，空→原 dispatch 原样；`web/static/sprite-pet.html` `#pet-bubble-msg` `max-height 50→160px`（长回复气泡内滚动）。
- **陷阱**：`changeStepper/parseInt` 截小数——实现已规避（小数改裸 input，仅 top_k 用 stepper）；受检验 PASSED。
- **验证**：`go vet ./...` 无错；`go test ./internal/assistant ./internal/api/assistant ./internal/config` 3 包通过；全量 `go test ./...` 64 包通过；4 JS `node vm.Script` 语法 ok。
