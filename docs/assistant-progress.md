...
> 最后核对：2026-08-29（Move 共享分组 + Mirror 镜像 + AssistantAction.Mirror + Presets/StateMachine 重整 + Demo 镜像与方向事件；Demo 页下方新增游戏插件区与 `setPaused` 暂停缝——游戏侧架构见 [`gamedemo-progress.md`](gamedemo-progress.md)）。
...
- **事件解析**：`petSM.dispatch(event)` 按 `EVENT_ALIASES` 别名表取第一个已配置的 action：
  ...
  `move_*`（共享，8 向 + idle：`idle/move_left/move_up_left/move_up/move_up_right/move_right/move_down_right/move_down/move_down_left`）与 legacy `walk_*` 双轨别名（`move_*` 优先，`walk_*` 回退链含 `move_*`，兼容旧存量）；
...
- **渲染与镜像**：单 rAF 循环 + 每态 fps 累积；`states[].mirror` 或当次为右向事件回退到左像时（`_mirrorPending`）→ 画布水平镜像（`scale(-1,1)`），否则原样。
...
| 变更 | 同步位置 |
|---|---|
| `AssistantConfig`/`AssistantAction.Mirror` 字段 | §1-§2 配置+状态机行、`internal/config/persistence.go` legacy 迁移、`settings.js`/`settings_assistant.js` 编辑器（Mirror 开关/按帧预览）、PROJECT_MAP 对应条目 |
| petSM/ademoSM 别名表 / 事件接入 / Move 共享分组 | §2/§8、`sprite-pet.js`/`assistant-demo.js` `EVENT_ALIASES`（`move_*` 共享 + `walk_*` 兼容）、`web/sprite-pet-drag.test.js` |
| Mirror 渲染与状态机回退镜像 | §2/§8、`sprite-pet.js`/`assistant-demo.js` 的 `states[].mirror` + `_mirrorPending` 分支、`settings_assistant.js` 的镜像预览/自动配对 |
| Actions 分组/预设/状态机面板与自动镜像策略 | §1/§8、`settings_assistant.js`（`__assistantMoveActions`/`__assistantMoveMirrorPairs` + `assistantAddPreset`/`assistantSaveActionEditor`）与矩阵 `__assistantSMGroups` |
