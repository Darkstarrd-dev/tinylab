...
- **请求追踪（与 Debug Mode 解耦，默认关闭）**：`proxy.Handler` 的 `logRequests` 运行时原子开关（加载自持久化 `cfg.Trace.Enabled`，默认 `false`）控制是否将每个 proxied 请求的完整未截断 body 写入两层 JSONL 追踪日志（`request_log.go`）。追踪日志采用两层 JSONL 格式：`writeRequestLog` 写入 `traces/index-YYYYMMDD.jsonl`（每日轮转，同 reqID 末次写入覆盖）+ `traces/req/<reqID>.jsonl`（追加，仅首次调用写 request 行，后续每次调用写 attempt 行）。敏感请求头（`Authorization`、`X-Api-Key`）的值被遮蔽为 `***`+末 4 字符（保留 scheme 前缀），Base64 图像数据（`data:` URLs、`b64_json` 字段、Anthropic `{type:base64,data:}` 对象）被剥离后再写入。该开关通过 settings API 的 `trace.enabled` 字段运行时切换，独立于 Debug Mode（前端"Request Tracing"行始终可见，不依赖 debugMode）。`TraceMgmtCall` 方法捕获 ManagementClient 路径（probe/combo-speedtest/providers-probe）的调用，`SweepTraces` 后台保留清理（每小时运行，按 retainDays + maxDiskMB 限制）。
- **前端开关可见性**：
